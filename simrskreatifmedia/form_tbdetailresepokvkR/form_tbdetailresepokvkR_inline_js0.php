...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailresepokvkR_inline']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbdetailresepokvkR_inline"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_ambilFormula_okvk_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'ambilFormula_okvk');
}
var var_btn_Validasi_struk_okvk = "<?php echo $this->form_encode_input($this->nmgp_dados_form['trancode_']); ?>";
function sc_btn_Validasi()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Validasi(); }, 500);
      return;
    }
    sc_btn_Validasi_ok();
}
function sc_btn_Validasi_cancel()
{
}
function sc_btn_Validasi_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_cetak_okvk; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "struk_okvk*scin' + var_btn_Validasi_struk_okvk + '*scout"; ?>', '', '_blank', '0', '0', 'cetak_okvk');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailresepokvkR_inline']['where_detal']))
  {
?>  
...
{
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailresepokvkR']['where_detal']))
  {
?>  
  if (parent && parent.nmsc_iframe_liga_form_tbdetailresepokvkR.scInlineFormReceive)
  {
    parent.nmsc_iframe_liga_form_tbdetailresepokvkR.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailresepokvkR_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
...
?>  
  ...
  {
    parent.scInlineFormReceive(oResp, <?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailresepokvkR_inline']['inline_form_seq'] ?>);
    return true;
  }
<?php
  }
