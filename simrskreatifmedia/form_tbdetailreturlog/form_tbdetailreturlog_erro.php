...
$old_error_handler = set_error_handler("form_tbdetailreturlog_scriptcase_error_handler");

function form_tbdetailreturlog_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_tbdetailreturlog']['contr_erro']) && $_SESSION['scriptcase']['form_tbdetailreturlog']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_tbdetailreturlog->contr_form_tbdetailreturlog->Apl_com_erro = true;
            if ($inicial_form_tbdetailreturlog->contr_form_tbdetailreturlog->NM_ajax_flag)
            {
                $inicial_form_tbdetailreturlog->contr_form_tbdetailreturlog->NM_ajax_info['errList']['geral_form_tbdetailreturlog'][] = $msg;
                return;
            }
            form_tbdetailreturlog_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_tbdetailreturlog_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_tbdetailreturlog'])  && '' != $_SESSION['scriptcase']['error_icon']['form_tbdetailreturlog'])  ? $_SESSION['scriptcase']['error_icon']['form_tbdetailreturlog']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_tbdetailreturlog']) && '' != $_SESSION['scriptcase']['error_close']['form_tbdetailreturlog']) ? $_SESSION['scriptcase']['error_close']['form_tbdetailreturlog'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_tbdetailreturlog']) && '' != $_SESSION['scriptcase']['error_span_title']['form_tbdetailreturlog']) ? $_SESSION['scriptcase']['error_span_title']['form_tbdetailreturlog'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_tbdetailreturlog']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_tbdetailreturlog']) ? $_SESSION['scriptcase']['error_icon_title']['form_tbdetailreturlog'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_tbdetailreturlog']['error_buffer'] .= ob_get_flush();
}
