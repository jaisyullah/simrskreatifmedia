...
$old_error_handler = set_error_handler("gantiDokter_scriptcase_error_handler");

function gantiDokter_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['gantiDokter']['contr_erro']) && $_SESSION['scriptcase']['gantiDokter']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_gantiDokter->contr_gantiDokter->Apl_com_erro = true;
            if ($inicial_gantiDokter->contr_gantiDokter->NM_ajax_flag)
            {
                $inicial_gantiDokter->contr_gantiDokter->NM_ajax_info['errList']['geral_gantiDokter'][] = $msg;
                return;
            }
            gantiDokter_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function gantiDokter_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['gantiDokter'])  && '' != $_SESSION['scriptcase']['error_icon']['gantiDokter'])  ? $_SESSION['scriptcase']['error_icon']['gantiDokter']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['gantiDokter']) && '' != $_SESSION['scriptcase']['error_close']['gantiDokter']) ? $_SESSION['scriptcase']['error_close']['gantiDokter'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['gantiDokter']) && '' != $_SESSION['scriptcase']['error_span_title']['gantiDokter']) ? $_SESSION['scriptcase']['error_span_title']['gantiDokter'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['gantiDokter']) && '' != $_SESSION['scriptcase']['error_icon_title']['gantiDokter']) ? $_SESSION['scriptcase']['error_icon_title']['gantiDokter'] : '';
?>
