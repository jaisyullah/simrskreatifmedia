...
class summary_pend_rajal_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['dashboard_info']['under_dashboard'] ? "off" : "on";
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['summary_pend_rajal']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['summary_pend_rajal']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['res_hrz'];
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['contr_total_geral'] == "OK")
      {
          $this->array_total_judul = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['arr_total']['judul'];
      }
      else
      {
          $this->array_total_judul = array();
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['arr_total']['judul'] = $this->array_total_judul;
      $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['json_use_label'])
       {
          $Lab_judul = "judul";
       }
       else
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['labels']['judul']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['labels']['judul'] = "Kategori"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['labels']['jumlah']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['labels']['jumlah'] = "Jumlah"; 
       }
       //-----
       $this->comp_field_nm = array(
           'judul' => 0,
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_order']['judul']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_order']['judul'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_order']['judul'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['summarizing_fields_display']['judul'] as $i_sum => $d_sum)
           {
       ...
       //-----
       foreach ($this->array_total_judul as $i_judul => $d_judul) {
           if (!in_array($i_judul, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_judul[3] ] = $d_judul[2];
               $this->comp_label[0][ $d_judul[3] ] = $d_judul[2];
           }
       }
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_level'] = -1;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_tabular_label'];
       ...
       //-----
       foreach ($this->comp_label[0] as $i_judul => $l_judul) {
           if (isset($this->array_total_judul[$i_judul])) {
               $this->comp_group[$i_judul] = array();
           }
       }
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@summary_pend_rajal@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           '0|1' => array(
               ...
               'label_x'  => $this->getChartText(strip_tags($Lab_judul)),
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_charts'];
	}
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_summary_pend_rajal_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_summary_pend_rajal_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['proc_pdf_vert']){ 
   ...
   {
       if (!$res_chart && $this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['json_use_format']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['json_use_format'])
       {
           return $valor_campo;
       }
       ...
       if ($total == 1)
       {
           nmgp_Form_Num_Val($valor_campo, ".", ",", "0", "S", "12", "Rp", "V:3:12", "-");
       }
       return $valor_campo;
   }
   ...
   function resumo_init()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         $this->monta_html_ini();
         $this->monta_cabecalho();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
   ...
   function inicializa_vars()
   {
      ...
      if ("out" == $this->NM_tipo)
      {
         require_once($this->Ini->path_aplicacao . "summary_pend_rajal_total.class.php"); 
      }
      ...
      $this->Total   = new summary_pend_rajal_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_judul("res");
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['arr_total'][$cmp_gb];
      }
      ksort($this->array_total_judul);
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['SC_Ind_Groupby'];
         $this->Total->$Gb_geral();
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['dashboard_refresh_after_chart'] = 'resumo';
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['doc_word'])
{
   ...
   { 
      ...
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_summary_pend_rajal_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_summary_pend_rajal_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_summary_pend_rajal_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['num_css'] . '.css');
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida("     }\r\n");
           $nm_saida->saida("   </script>\r\n");
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['ajax_nav'] && !$this->Ini->sc_export_ajax && !$this->Ini->Export_html_zip && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['doc_word'])
      {
           $nm_saida->saida("   <script type=\"text/javascript\"> \r\n");
          ...
          if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['grid_pesq']))
          { 
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['dashboard_info']['remove_margin'] ? ' style="margin: 0"' : '';
          $nm_saida->saida(" <BODY class=\"" . $this->css_scGridPage . "\"" . $remove_margin . ">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['embutida'])
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_config_pdf.php?nm_opc=pdf_res&nm_target=0&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=" . s . "&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&summary_export_columns=S&nm_label_group=N&nm_all_cab=S&nm_all_label=S&pdf_zip=N&nm_orient_grid=2&origem=res&language=id&conf_socor=N&script_case_init=" . $this->Ini->sc_page . "&app_name=summary_pend_rajal&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
         if ($NM_Gbtn)
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['xls'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bexcel", "", "", "Rxls_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_config_xls.php?script_case_init=" . $this->Ini->sc_page . "&app_name=summary_pend_rajal&nm_tp_xls=xlsx&nm_tot_xls=S&nm_tot_xls=S&nm_res_cons=s&nm_ini_xls_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&summary_export_columns=S&origem=res&language=&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
         if ($NM_Gbtn)
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['csv'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bcsv", "", "", "Rcsv_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_config_csv.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=S&nm_res_cons=s&nm_ini_csv_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&nm_delim_line=1&nm_delim_col=1&nm_delim_dados=1&nm_label_csv=N&language=&origem=res&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=S&nm_opc=resumo&nm_cor=CO&nm_res_cons=s&nm_ini_prt_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&origem=res&language=id&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bconf_graf", "", "", "Rgraf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_config_graf_flash.php?nome_apl=summary_pend_rajal&campo_apl=nm_resumo_graf&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=id&tp_apl=cns_htm&KeepThis=true&TB_iframe=true&height=450&width=400&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      ...
      if ($this->nmgp_botoes['chart_detail'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("summary_pend_rajal_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
   ...
   function monta_html_fim()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['print_all'])
      {
          $this->grafico_pdf();
      }
      $nm_saida->saida("</TABLE>\r\n");
      $nm_saida->saida("<link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/css/" . $this->Ini->str_schema_all . "_sweetalert.css\" />\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['summary_pend_rajal']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['summary_pend_rajal']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'summary_pend_rajal_config_pivot.php?nome_apl=summary_pend_rajal&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=id&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_summary_pend_rajal_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "summary_pend_rajal/summary_pend_rajal_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
   ...
   {
      global $nm_saida;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['embutida'] || $_SESSION['sc_session'][$this->Ini->sc_page]['summary_pend_rajal']['dashboard_info']['compact_mode'])
      { 
          return;
      } 
      ...
      { 
        ...
        if ($_SESSION['scriptcase']['charset'] != "UTF-8")
        {
          ...
          $judul = $Busca_temp['judul']; 
          $tmp_pos = strpos($judul, "##@@");
          if ($tmp_pos !== false && !is_array($judul))
          {
              $judul = substr($judul, 0, $tmp_pos);
          }
      } 
   ...
   //---- 
   function adiciona_registro($sum_jumlah, $quebra_judul, $quebra_judul_orig)
   {
      //----- Kategori
      if (!isset($this->array_total_judul[$quebra_judul_orig]))
      {
         $this->array_total_judul[$quebra_judul_orig][0] = 1;
         $this->array_total_judul[$quebra_judul_orig][1] = $sum_jumlah;
         $this->array_total_judul[$quebra_judul_orig][2] = $quebra_judul;
         $this->array_total_judul[$quebra_judul_orig][3] = $quebra_judul_orig;
      }
      else
      {
         $this->array_total_judul[$quebra_judul_orig][0]++;
         $this->array_total_judul[$quebra_judul_orig][1] += $sum_jumlah;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('', 'judul');
     $campos_order = $this->Ini->Get_date_order_groupby("judul", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->judul = $rs_res->fields[0] ;  
            $this->jumlah = $rs_res->fields[1] ;  
            $this->jumlah =  str_replace(",", ".", $this->jumlah);
            $this->judul_orig = $this->judul;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->jumlah, $this->judul, $this->judul_orig);
            }
   }
//
