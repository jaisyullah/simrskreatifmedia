...
class chart_bybayar_lab_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_bybayar_lab']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_bybayar_lab']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['tot_geral'] = array() ;  
    if ($res_limit && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['sql_tot_res']))
    {
        $nm_comando = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['sql_tot_res'];
    }
    else
    {
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from (SELECT 	a.trandate AS Tgl, 	b.bayar AS jenis FROM 	tbtranlab a 	LEFT JOIN ( 	SELECT 		x.bayar, 		x.custCode  	FROM 	( SELECT bayar AS bayar, custCode FROM tbantrian UNION ALL SELECT pembayaran AS bayar, custCode FROM tbadmrawatinap ) x  	) b ON b.custCode = a.custcode) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*) from (SELECT 	a.trandate AS Tgl, 	b.bayar AS jenis FROM 	tbtranlab a 	LEFT JOIN ( 	SELECT 		x.bayar, 		x.custCode  	FROM 	( SELECT bayar AS bayar, custCode FROM tbantrian UNION ALL SELECT pembayaran AS bayar, custCode FROM tbadmrawatinap ) x  	) b ON b.custCode = a.custcode) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['contr_total_geral'] = "OK";
   } 

   //-----  jenis
   function quebra_jenis_sc_free_group_by($jenis, $Where_qb, $Cmp_Name) 
   {
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('jenis' => "jenis");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      { 
         $cmd_simp = "select count(*)#@#cmps_quebras#@# from (SELECT 	a.trandate AS Tgl, 	b.bayar AS jenis FROM 	tbtranlab a 	LEFT JOIN ( 	SELECT 		x.bayar, 		x.custCode  	FROM 	( SELECT bayar AS bayar, custCode FROM tbantrian UNION ALL SELECT pembayaran AS bayar, custCode FROM tbadmrawatinap ) x  	) b ON b.custCode = a.custcode) sc_sel_esp " . $where_ok;
         $comando  = "select count(*)#@#cmps_quebras#@# from (";
         $comando .= "select " . $cmps_quebras1 . " from (SELECT 	a.trandate AS Tgl, 	b.bayar AS jenis FROM 	tbtranlab a 	LEFT JOIN ( 	SELECT 		x.bayar, 		x.custCode  	FROM 	( SELECT bayar AS bayar, custCode FROM tbantrian UNION ALL SELECT pembayaran AS bayar, custCode FROM tbadmrawatinap ) x  	) b ON b.custCode = a.custcode) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	a.trandate AS Tgl, 	b.bayar AS jenis FROM 	tbtranlab a 	LEFT JOIN ( 	SELECT 		x.bayar, 		x.custCode  	FROM 	( SELECT bayar AS bayar, custCode FROM tbantrian UNION ALL SELECT pembayaran AS bayar, custCode FROM tbadmrawatinap ) x  	) b ON b.custCode = a.custcode) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_bybayar_lab']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
