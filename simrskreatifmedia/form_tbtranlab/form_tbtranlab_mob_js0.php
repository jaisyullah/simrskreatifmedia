...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtranlab_mob']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbtranlab_mob"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_sc_btn_0_var_lab = "<?php echo $this->form_encode_input($this->nmgp_dados_form['trancode']); ?>";
var var_btn_sc_btn_0_varlab = "<?php echo $this->form_encode_input($this->nmgp_dados_form['trancode']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_pdfreport_tbhasillab_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "var_lab*scin' + var_btn_sc_btn_0_var_lab + '*scoutvarlab*scin' + var_btn_sc_btn_0_varlab + '*scout"; ?>', 'inicio', '_blank', '0', '0', 'pdfreport_tbhasillab');
}
var Crtl_btn_Batal = false;
function sc_btn_Batal()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_Batal(); }, 500);
      return;
    }
    if (Crtl_btn_Batal) {return;}
    scJs_confirm("<?php echo html_entity_decode('Yakin ingin membatalkan Transaksi ?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Batal_ok, sc_btn_Batal_cancel)
}
function sc_btn_Batal_cancel()
{
}
function sc_btn_Batal_ok()
{
    Crtl_btn_Batal = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Batal?@?";
    document.F1.action = "form_tbtranlab_mob.php";
    document.F1.target = "_self";
    document.F1.submit();
}
var var_btn_sc_btn_1_tranlab = "<?php echo $this->form_encode_input($this->nmgp_dados_form['trancode']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_print_lab; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "tranlab*scin' + var_btn_sc_btn_1_tranlab + '*scout"; ?>', '', '_blank', '0', '0', 'print_lab');
}
var var_btn_sc_btn_2_norm = "<?php echo $this->form_encode_input($this->nmgp_dados_form['custcode']); ?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_label; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "norm*scin' + var_btn_sc_btn_2_norm + '*scout"; ?>', '', '_blank', '0', '0', 'label');
}
...
{
  tb_show('', '<?php echo $this->Ini->link_connectLIS_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&script_case_session=<?php echo session_id() ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=480&width=600&modal=true', '');
}
function sc_btn_LIS()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_LIS(); }, 500);
      return;
    }
    sc_btn_LIS_ok();
}
function sc_btn_LIS_cancel()
{
}
function sc_btn_LIS_ok()
{
  tb_show('', '<?php echo $this->Ini->link_sendLIS_edit; ?>?script_case_init=<?php echo $this->Ini->sc_page; ?>&script_case_session=<?php echo session_id() ?>&nmgp_url_saida=modal&nmgp_parms=<?php echo "NM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>&nmgp_outra_jan=true&TB_iframe=true&height=600&width=800&modal=true', '');
}
 NM_tp_critica(1);
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtranlab_mob']['embutida_form']) {
...
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtranlab_mob']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtranlab_mob']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtranlab_mob']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("trancode=disabled;inapcode=disabled;custage=disabled;diag=disabled", "");
   nm_field_disabled("instcode=disabled;struk=disabled;sc_field_1=disabled;sc_field_0=disabled;poli=disabled;doccode=disabled;docname=disabled", "U");
