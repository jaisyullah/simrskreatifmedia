...
class grid_tbadministrasi_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_tbadministrasi_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_tbadministrasi_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_tbadministrasi_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_biayaadm')
      {
          $biayaadm = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_biayaadm($biayaadm);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_tarifadm')
      {
          $tarifadm = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_tarifadm($tarifadm);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_biayaadm($biayaadm)
   {
      $biayaadm = substr($this->Db->qstr($biayaadm), 1, -1);
            $biayaadm_look = substr($this->Db->qstr($biayaadm), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct biayaAdm from " . $this->Ini->nm_tabela . " where  biayaAdm like '%" . $biayaadm . "%' order by biayaAdm"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_tarifadm($tarifadm)
   {
      $tarifadm = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $tarifadm);
      $tarifadm = substr($this->Db->qstr($tarifadm), 1, -1);
            $tarifadm_look = substr($this->Db->qstr($tarifadm), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct tarifAdm from " . $this->Ini->nm_tabela . " where   CAST (tarifAdm AS TEXT)  like '%" . $tarifadm . "%' order by tarifAdm"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct tarifAdm from " . $this->Ini->nm_tabela . " where   CAST (tarifAdm AS VARCHAR)  like '%" . $tarifadm . "%' order by tarifAdm"; 
      }
      else
      {
          $nm_comando = "select distinct tarifAdm from " . $this->Ini->nm_tabela . " where  tarifAdm like '%" . $tarifadm . "%' order by tarifAdm"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Setting Administrasi</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_tbadministrasi/grid_tbadministrasi_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_tbadministrasi_ajax_search.js"></script>
 <script type="text/javascript" src="grid_tbadministrasi_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_tbadministrasi_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbadministrasi']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbadministrasi']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  str_out += 'SC_biayaadm_cond#NMF#' + search_get_sel_txt('SC_biayaadm_cond') + '@NMF@';
  str_out += 'SC_biayaadm#NMF#' + search_get_text('SC_biayaadm') + '@NMF@';
  str_out += 'id_ac_biayaadm#NMF#' + search_get_text('id_ac_biayaadm') + '@NMF@';
  str_out += 'SC_tarifadm_cond#NMF#' + search_get_sel_txt('SC_tarifadm_cond') + '@NMF@';
  str_out += 'SC_tarifadm#NMF#' + search_get_text('SC_tarifadm') + '@NMF@';
  str_out += 'id_ac_tarifadm#NMF#' + search_get_text('id_ac_tarifadm') + '@NMF@';
   ...
   $("#id_ac_biayaadm").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_biayaadm",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_biayaadm").val( $(this).val() );
       }
     }
   });
   $("#id_ac_tarifadm").autocomplete({
     minLength: 1,
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_tarifadm",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Setting Administrasi</div>
...
<?php
   ...
   function monta_form()
   {
      global 
             ...
             $biayaadm_cond, $biayaadm, $biayaadm_autocomp,
             $tarifadm_cond, $tarifadm, $tarifadm_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $id = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['id']; 
          $id_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['id_cond']; 
          $biayaadm = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['biayaadm']; 
          $biayaadm_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['biayaadm_cond']; 
          $tarifadm = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['tarifadm']; 
          $tarifadm_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['tarifadm_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['NM_operador']; 
      } 
      if (!isset($id_cond) || empty($id_cond))
      {
         $id_cond = "gt";
      }
      if (!isset($biayaadm_cond) || empty($biayaadm_cond))
      {
         $biayaadm_cond = "qp";
      }
      if (!isset($tarifadm_cond) || empty($tarifadm_cond))
      {
         $tarifadm_cond = "gt";
      }
      ...
      $str_hide_biayaadm = (in_array($biayaadm_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_tarifadm = (in_array($tarifadm_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      if (!isset($biayaadm) || $biayaadm == "")
      {
          $biayaadm = "";
      }
      if (isset($biayaadm) && !empty($biayaadm))
      {
         $tmp_pos = strpos($biayaadm, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $biayaadm = substr($biayaadm, 0, $tmp_pos);
         }
      }
      if (!isset($tarifadm) || $tarifadm == "")
      {
          $tarifadm = "";
      }
      if (isset($tarifadm) && !empty($tarifadm))
      {
         $tmp_pos = strpos($tarifadm, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $tarifadm = substr($tarifadm, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
...
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_id" name="id_autocomp" size="5"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 5, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_biayaadm_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['biayaadm'])) ? $this->New_label['biayaadm'] : "Biaya Adm"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_biayaadm_cond" name="biayaadm_cond" onChange="nm_campos_between(document.getElementById('id_vis_biayaadm'), this, 'biayaadm')">
       <OPTION value="qp" <?php if ("qp" == $biayaadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $biayaadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $biayaadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $biayaadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_biayaadm" <?php echo $str_hide_biayaadm?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['biayaadm'])) ? $this->New_label['biayaadm'] : "Biaya Adm";
 $nmgp_tab_label .= "biayaadm?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $biayaadm_look = substr($this->Db->qstr($biayaadm), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct biayaAdm from " . $this->Ini->nm_tabela . " where biayaAdm = '$biayaadm_look' order by biayaAdm"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$biayaadm]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$biayaadm];
      }
      else
      {
          $sAutocompValue = $biayaadm;
      }
?>
<INPUT  type="text" id="SC_biayaadm" name="biayaadm" value="<?php echo NM_encode_input($biayaadm) ?>"  size=50 alt="{datatype: 'text', maxLength: 255, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_biayaadm" name="biayaadm_autocomp" size="50"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 255, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_tarifadm_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['tarifadm'])) ? $this->New_label['tarifadm'] : "Administrasi"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_tarifadm_cond" name="tarifadm_cond" onChange="nm_campos_between(document.getElementById('id_vis_tarifadm'), this, 'tarifadm')">
       <OPTION value="gt" <?php if ("gt" == $tarifadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $tarifadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $tarifadm_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_tarifadm" <?php echo $str_hide_tarifadm?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['tarifadm'])) ? $this->New_label['tarifadm'] : "Administrasi";
 $nmgp_tab_label .= "tarifadm?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $tarifadm_look = substr($this->Db->qstr($tarifadm), 1, -1); 
      $nmgp_def_dados = array(); 
   if (is_numeric($tarifadm))
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct tarifAdm from " . $this->Ini->nm_tabela . " where tarifAdm = $tarifadm_look order by tarifAdm"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct tarifAdm from " . $this->Ini->nm_tabela . " where tarifAdm = $tarifadm_look order by tarifAdm"; 
      ...
      if (isset($nmgp_def_dados[0][$tarifadm]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$tarifadm];
      }
      else
      {
            nmgp_Form_Num_Val($tarifadm, ".", "", "0", "S", "2", "", "N:2", "-") ; 
          $sAutocompValue = $tarifadm;
      }
?>
<INPUT  type="text" id="SC_tarifadm" name="tarifadm" value="<?php echo NM_encode_input($tarifadm) ?>" size=8 alt="{datatype: 'integer', maxLength: 8, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_tarifadm" name="tarifadm_autocomp" size="8"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 8, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
...
?>
<?php
   if (is_file("grid_tbadministrasi_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tbadministrasi']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tbadministrasi']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.biayaadm_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_biayaadm'), document.F1.biayaadm_cond, 'biayaadm');
   document.F1.biayaadm.value = "";
   document.F1.biayaadm_autocomp.value = "";
   document.F1.tarifadm_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_tarifadm'), document.F1.tarifadm_cond, 'tarifadm');
   document.F1.tarifadm.value = "";
   document.F1.tarifadm_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_tbadministrasi";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_biayaadm_cond'] = 'cond';
      $tp_fields['SC_biayaadm'] = 'text_aut';
      $tp_fields['id_ac_biayaadm'] = 'text_aut';
      $tp_fields['SC_tarifadm_cond'] = 'cond';
      $tp_fields['SC_tarifadm'] = 'text_aut';
      $tp_fields['id_ac_tarifadm'] = 'text_aut';
   ...
   function trata_campos()
   {
      global $id_cond, $id, $id_autocomp,
             $biayaadm_cond, $biayaadm, $biayaadm_autocomp,
             $tarifadm_cond, $tarifadm, $tarifadm_autocomp, $nmgp_tab_label;
      ...
      if (!empty($biayaadm_autocomp) && empty($biayaadm))
      {
          $biayaadm = $biayaadm_autocomp;
      }
      if (!empty($tarifadm_autocomp) && empty($tarifadm))
      {
          $tarifadm = $tarifadm_autocomp;
      }
      ...
      $biayaadm_cond_salva = $biayaadm_cond; 
      if (!isset($biayaadm_input_2) || $biayaadm_input_2 == "")
      {
          $biayaadm_input_2 = $biayaadm;
      }
      $tarifadm_cond_salva = $tarifadm_cond; 
      if (!isset($tarifadm_input_2) || $tarifadm_input_2 == "")
      {
          $tarifadm_input_2 = $tarifadm;
      }
      if ($id_cond != "in")
      {
      ...
      if ($tarifadm_cond != "in")
      {
          nm_limpa_numero($tarifadm, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['id'] = $id; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['id_cond'] = $id_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['biayaadm'] = $biayaadm; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['biayaadm_cond'] = $biayaadm_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['tarifadm'] = $tarifadm; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['tarifadm_cond'] = $tarifadm_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca'] = $Temp_Busca;
      }
      ...
      if ($tarifadm_cond != "in" && $tarifadm_cond != "bw" && !empty($tarifadm) && !is_numeric($tarifadm))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Administrasi";
      }
      if ($tarifadm_cond == "bw" && ((!empty($tarifadm) && !is_numeric($tarifadm)) || (!empty($tarifadm_input_2) && !is_numeric($tarifadm_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Administrasi";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['biayaadm'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['biayaadm'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['biayaadm'] = $biayaadm;
      }
      $tarifadm_look = substr($this->Db->qstr($tarifadm), 1, -1); 
      $nmgp_def_dados = array(); 
   if (is_numeric($tarifadm))
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['tarifadm'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['tarifadm'] = $cmp1;
      }
      else
      {
          $cmp1 = $tarifadm;
          nmgp_Form_Num_Val($cmp1, ".", "", "0", "S", "2", "", "N:2", "-") ; 
          $this->cmp_formatado['tarifadm'] = $cmp1;
      }
      ...
      //----- $biayaadm
      $this->Date_part = false;
      if (isset($biayaadm) || $biayaadm_cond == "nu" || $biayaadm_cond == "nn" || $biayaadm_cond == "ep" || $biayaadm_cond == "ne")
      {
         $this->monta_condicao("biayaAdm", $biayaadm_cond, $biayaadm, "", "biayaadm");
      }

      //----- $tarifadm
      $this->Date_part = false;
      if (isset($tarifadm) || $tarifadm_cond == "nu" || $tarifadm_cond == "nn" || $tarifadm_cond == "ep" || $tarifadm_cond == "ne")
      {
         $this->monta_condicao("tarifAdm", $tarifadm_cond, $tarifadm, "", "tarifadm");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbadministrasi']['cond_pesq'] .= $this->NM_operador;
