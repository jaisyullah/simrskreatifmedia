...
$old_error_handler = set_error_handler("etiket_scriptcase_error_handler");

function etiket_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['etiket']['contr_erro']) && $_SESSION['scriptcase']['etiket']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_etiket->contr_etiket->Apl_com_erro = true;
            if ($inicial_etiket->contr_etiket->NM_ajax_flag)
            {
                $inicial_etiket->contr_etiket->NM_ajax_info['errList']['geral_etiket'][] = $msg;
                return;
            }
            etiket_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function etiket_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['etiket'])  && '' != $_SESSION['scriptcase']['error_icon']['etiket'])  ? $_SESSION['scriptcase']['error_icon']['etiket']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['etiket']) && '' != $_SESSION['scriptcase']['error_close']['etiket']) ? $_SESSION['scriptcase']['error_close']['etiket'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['etiket']) && '' != $_SESSION['scriptcase']['error_span_title']['etiket']) ? $_SESSION['scriptcase']['error_span_title']['etiket'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['etiket']) && '' != $_SESSION['scriptcase']['error_icon_title']['etiket']) ? $_SESSION['scriptcase']['error_icon_title']['etiket'] : '';
?>
