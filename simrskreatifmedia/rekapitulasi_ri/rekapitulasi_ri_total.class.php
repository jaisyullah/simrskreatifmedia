...
class rekapitulasi_ri_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['rekapitulasi_ri']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['rekapitulasi_ri']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_byDate($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['contr_total_geral'] = "OK";
   } 

   //-----  b_tglkeluar
   function quebra_b_tglkeluar_byDate($b_tglkeluar, $arg_sum_b_tglkeluar) 
   {
      global $tot_b_tglkeluar;
      $tot_b_tglkeluar = array() ;  
      $tot_b_tglkeluar[0] = $b_tglkeluar ; 
   }
   //-----  a_tglmasuk
   function quebra_a_tglmasuk_byDate($b_tglkeluar, $a_tglmasuk, $arg_sum_b_tglkeluar, $arg_sum_a_tglmasuk) 
   {
      global $tot_a_tglmasuk;
      $tot_a_tglmasuk = array() ;  
      $tot_a_tglmasuk[0] = $a_tglmasuk ; 
   }

   //----- 
   function Calc_resumo_byDate($destino_resumo)
   {
      global $nm_lang, $usia;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('b_tglkeluar' => "b.tglKeluar",'a_tglmasuk' => "a.tglMasuk");
      $cmps_quebra_atual = array("b_tglkeluar","a_tglmasuk");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_byDate($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_byDate($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['rekapitulasi_ri']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['b_tglkeluar']['reg'] = "S";
      $format_dimensions['b_tglkeluar']['msk'] = "";
      $format_dimensions['a_tglmasuk']['reg'] = "S";
      $format_dimensions['a_tglmasuk']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'byDate', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_byDate($where_ok);
