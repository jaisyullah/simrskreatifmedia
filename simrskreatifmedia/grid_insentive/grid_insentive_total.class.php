...
class grid_insentive_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_insentive']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_insentive']['campos_busca']))
      { 
   ...
   function quebra_geral_byTanggal($res_limit=false)
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['contr_total_geral'] = "OK";
   } 

   //-----  tgltindakan
   function quebra_tgltindakan_byTanggal($tgltindakan, $arg_sum_tgltindakan) 
   {
      global $tot_tgltindakan;
      $tot_tgltindakan = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(Insentive) from (SELECT 	a.Kode, 	a.Tindakan, 	a.Insentive, 	a.Diskon, 	a.tglTindakan, 	concat( 		d.gelar, 		d.`name`, 		', ', 		d.spec 	) AS dokCode, 	concat( 		c.`name`, 		', ', 		c.salut, 		' (', 		c.custCode, 		')' 	) AS Pasien, 	a.Deskripsi FROM 	( 		SELECT 			a.tranCode AS Kode, 			a.tindakan AS Tindakan, 			f.tarif*(80/100) AS Insentive, 			a.diskon AS Diskon, 			a.tglTindakan AS tglTindakan, 			b.dokter AS dokCode, 			b.custCode AS namaPasien, 			'Tind. Poli' Deskripsi, 			a.id AS noID 		FROM 			tbtindakanrawatjalan a 		LEFT JOIN tbdetailrawatjalan b ON b.tranCode = a.tranCode 		LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 		LEFT JOIN tbdoctor d ON d.docCode = b.dokter 		LEFT JOIN tbpoli e ON e.polyCode = d.poli 		LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 		AND c.jenisTindakan = e.`name` 		GROUP BY 			a.id 		UNION ALL 			SELECT 				a.tranCode AS Kode, 				a.tindakan AS Tindakan, 				g.tarif*(80/100) AS Insentive, 				a.diskon AS Diskon, 				a.tglTindakan AS tglTindakan, 				b.dokter AS dokCode, 				b.custCode AS namaPasien, 				'Visite Ranap' Deskripsi, 				a.id AS noID 			FROM 				tbtindakanrawatinap a 			LEFT JOIN tbdetailrawatinap b ON b.tranCode = a.tranCode 			LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 			LEFT JOIN tbdoctor d ON d.docCode = b.dokter 			LEFT JOIN tbpoli e ON e.polyCode = d.poli 			LEFT JOIN tbadmrawatinap f ON f.tranCode = b.tranCode 			LEFT JOIN tbtarifri g ON g.kodeTindakan = c.kodeTindakan 			AND c.jenisTindakan = e.`name` 			AND g.kelas = f.kelas 			GROUP BY 				a.id 			UNION ALL 				SELECT 					a.tranCode AS Kode, 					a.tindakan AS Tindakan, 					f.tarif*(80/100) AS Insentive, 					a.diskon AS Diskon, 					a.tglTindakan AS tglTindakan, 					a.pelaksana AS dokCode, 					b.custCode AS namaPasien, 					'IGD' Deskripsi, 					a.id AS noID 				FROM 					tbtindakanigd a 				LEFT JOIN tbdetailigd b ON b.tranCode = a.tranCode 				LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 				LEFT JOIN tbdoctor d ON d.docCode = b.dokter 				LEFT JOIN tbpoli e ON e.polyCode = d.poli 				LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 				AND c.jenisTindakan = e.`name` 				GROUP BY 					a.id 				UNION ALL 					SELECT 						a.trancode AS Kode, 						a.actname AS Tindakan, 						f.tarif AS Insentive, 						a.disc AS Diskon, 						b.tranDate AS tglTindakan, 						b.custCode AS namaPasien, 						a. CODE AS dokCode, 						'Tind. OK' Deskripsi, 						a.id AS noID 					FROM 						tbtim a 					LEFT JOIN tbdetailok b ON b.trancode = a.trancode 					LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 					LEFT JOIN tbdoctor d ON d.docCode = a.`code` 					LEFT JOIN tbpoli e ON e.polyCode = d.poli 					LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 					AND c.jenisTindakan = e.`name` 					AND f.kelas = b.class 					GROUP BY 						a.id 					UNION ALL 						SELECT 							a.trancode AS Kode, 							a.actname AS Tindakan, 							f.tarif*(80/100) AS Insentive, 							a.disc AS Diskon, 							b.tranDate AS tglTindakan, 							a. CODE AS dokCode, 							b.custCode AS namaPasien, 							'Tind. VK' Deskripsi, 							a.id AS noID 						FROM 							tbtim a 						LEFT JOIN tbdetailvk b ON b.trancode = a.trancode 						LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 						LEFT JOIN tbdoctor d ON d.docCode = a.`code` 						LEFT JOIN tbpoli e ON e.polyCode = d.poli 						LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 						AND c.jenisTindakan = e.`name` 						AND f.kelas = b.class 						GROUP BY 							a.id 	) a LEFT JOIN tbtindakan b ON b.kodeTindakan = a.Tindakan LEFT JOIN tbcustomer c ON c.custCode = a.namaPasien LEFT JOIN tbdoctor d ON d.docCode = a.dokCode WHERE 	a.Tindakan NOT LIKE 'SARANA%' ) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['where_pesq']; 
      } 
      ...
      { 
         $nm_comando .= " where " .  $this->Sc_groupby_tgltindakan . $arg_sum_tgltindakan ; 
      } 
      else 
      { 
         $nm_comando .= " and " .  $this->Sc_groupby_tgltindakan . $arg_sum_tgltindakan ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tgltindakan[0] = NM_encode_input(sc_strip_script($tgltindakan)) ; 
      $tot_tgltindakan[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tgltindakan[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 


   //----- 
   function Calc_resumo_byTanggal($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tgltindakan' => "tglTindakan");
      $cmps_quebra_atual = array("tgltindakan");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_byTanggal($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_byTanggal($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('insentive' => 'N','diskon' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(Insentive)#@#cmps_quebras#@# from (SELECT 	a.Kode, 	a.Tindakan, 	a.Insentive, 	a.Diskon, 	a.tglTindakan, 	concat( 		d.gelar, 		d.`name`, 		', ', 		d.spec 	) AS dokCode, 	concat( 		c.`name`, 		', ', 		c.salut, 		' (', 		c.custCode, 		')' 	) AS Pasien, 	a.Deskripsi FROM 	( 		SELECT 			a.tranCode AS Kode, 			a.tindakan AS Tindakan, 			f.tarif*(80/100) AS Insentive, 			a.diskon AS Diskon, 			a.tglTindakan AS tglTindakan, 			b.dokter AS dokCode, 			b.custCode AS namaPasien, 			'Tind. Poli' Deskripsi, 			a.id AS noID 		FROM 			tbtindakanrawatjalan a 		LEFT JOIN tbdetailrawatjalan b ON b.tranCode = a.tranCode 		LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 		LEFT JOIN tbdoctor d ON d.docCode = b.dokter 		LEFT JOIN tbpoli e ON e.polyCode = d.poli 		LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 		AND c.jenisTindakan = e.`name` 		GROUP BY 			a.id 		UNION ALL 			SELECT 				a.tranCode AS Kode, 				a.tindakan AS Tindakan, 				g.tarif*(80/100) AS Insentive, 				a.diskon AS Diskon, 				a.tglTindakan AS tglTindakan, 				b.dokter AS dokCode, 				b.custCode AS namaPasien, 				'Visite Ranap' Deskripsi, 				a.id AS noID 			FROM 				tbtindakanrawatinap a 			LEFT JOIN tbdetailrawatinap b ON b.tranCode = a.tranCode 			LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 			LEFT JOIN tbdoctor d ON d.docCode = b.dokter 			LEFT JOIN tbpoli e ON e.polyCode = d.poli 			LEFT JOIN tbadmrawatinap f ON f.tranCode = b.tranCode 			LEFT JOIN tbtarifri g ON g.kodeTindakan = c.kodeTindakan 			AND c.jenisTindakan = e.`name` 			AND g.kelas = f.kelas 			GROUP BY 				a.id 			UNION ALL 				SELECT 					a.tranCode AS Kode, 					a.tindakan AS Tindakan, 					f.tarif*(80/100) AS Insentive, 					a.diskon AS Diskon, 					a.tglTindakan AS tglTindakan, 					a.pelaksana AS dokCode, 					b.custCode AS namaPasien, 					'IGD' Deskripsi, 					a.id AS noID 				FROM 					tbtindakanigd a 				LEFT JOIN tbdetailigd b ON b.tranCode = a.tranCode 				LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 				LEFT JOIN tbdoctor d ON d.docCode = b.dokter 				LEFT JOIN tbpoli e ON e.polyCode = d.poli 				LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 				AND c.jenisTindakan = e.`name` 				GROUP BY 					a.id 				UNION ALL 					SELECT 						a.trancode AS Kode, 						a.actname AS Tindakan, 						f.tarif AS Insentive, 						a.disc AS Diskon, 						b.tranDate AS tglTindakan, 						b.custCode AS namaPasien, 						a. CODE AS dokCode, 						'Tind. OK' Deskripsi, 						a.id AS noID 					FROM 						tbtim a 					LEFT JOIN tbdetailok b ON b.trancode = a.trancode 					LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 					LEFT JOIN tbdoctor d ON d.docCode = a.`code` 					LEFT JOIN tbpoli e ON e.polyCode = d.poli 					LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 					AND c.jenisTindakan = e.`name` 					AND f.kelas = b.class 					GROUP BY 						a.id 					UNION ALL 						SELECT 							a.trancode AS Kode, 							a.actname AS Tindakan, 							f.tarif*(80/100) AS Insentive, 							a.disc AS Diskon, 							b.tranDate AS tglTindakan, 							a. CODE AS dokCode, 							b.custCode AS namaPasien, 							'Tind. VK' Deskripsi, 							a.id AS noID 						FROM 							tbtim a 						LEFT JOIN tbdetailvk b ON b.trancode = a.trancode 						LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 						LEFT JOIN tbdoctor d ON d.docCode = a.`code` 						LEFT JOIN tbpoli e ON e.polyCode = d.poli 						LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 						AND c.jenisTindakan = e.`name` 						AND f.kelas = b.class 						GROUP BY 							a.id 	) a LEFT JOIN tbtindakan b ON b.kodeTindakan = a.Tindakan LEFT JOIN tbcustomer c ON c.custCode = a.namaPasien LEFT JOIN tbdoctor d ON d.docCode = a.dokCode WHERE 	a.Tindakan NOT LIKE 'SARANA%' ) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select Insentive as SC_metric1, " . $cmps_quebras1 . " from (SELECT 	a.Kode, 	a.Tindakan, 	a.Insentive, 	a.Diskon, 	a.tglTindakan, 	concat( 		d.gelar, 		d.`name`, 		', ', 		d.spec 	) AS dokCode, 	concat( 		c.`name`, 		', ', 		c.salut, 		' (', 		c.custCode, 		')' 	) AS Pasien, 	a.Deskripsi FROM 	( 		SELECT 			a.tranCode AS Kode, 			a.tindakan AS Tindakan, 			f.tarif*(80/100) AS Insentive, 			a.diskon AS Diskon, 			a.tglTindakan AS tglTindakan, 			b.dokter AS dokCode, 			b.custCode AS namaPasien, 			'Tind. Poli' Deskripsi, 			a.id AS noID 		FROM 			tbtindakanrawatjalan a 		LEFT JOIN tbdetailrawatjalan b ON b.tranCode = a.tranCode 		LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 		LEFT JOIN tbdoctor d ON d.docCode = b.dokter 		LEFT JOIN tbpoli e ON e.polyCode = d.poli 		LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 		AND c.jenisTindakan = e.`name` 		GROUP BY 			a.id 		UNION ALL 			SELECT 				a.tranCode AS Kode, 				a.tindakan AS Tindakan, 				g.tarif*(80/100) AS Insentive, 				a.diskon AS Diskon, 				a.tglTindakan AS tglTindakan, 				b.dokter AS dokCode, 				b.custCode AS namaPasien, 				'Visite Ranap' Deskripsi, 				a.id AS noID 			FROM 				tbtindakanrawatinap a 			LEFT JOIN tbdetailrawatinap b ON b.tranCode = a.tranCode 			LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 			LEFT JOIN tbdoctor d ON d.docCode = b.dokter 			LEFT JOIN tbpoli e ON e.polyCode = d.poli 			LEFT JOIN tbadmrawatinap f ON f.tranCode = b.tranCode 			LEFT JOIN tbtarifri g ON g.kodeTindakan = c.kodeTindakan 			AND c.jenisTindakan = e.`name` 			AND g.kelas = f.kelas 			GROUP BY 				a.id 			UNION ALL 				SELECT 					a.tranCode AS Kode, 					a.tindakan AS Tindakan, 					f.tarif*(80/100) AS Insentive, 					a.diskon AS Diskon, 					a.tglTindakan AS tglTindakan, 					a.pelaksana AS dokCode, 					b.custCode AS namaPasien, 					'IGD' Deskripsi, 					a.id AS noID 				FROM 					tbtindakanigd a 				LEFT JOIN tbdetailigd b ON b.tranCode = a.tranCode 				LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 				LEFT JOIN tbdoctor d ON d.docCode = b.dokter 				LEFT JOIN tbpoli e ON e.polyCode = d.poli 				LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 				AND c.jenisTindakan = e.`name` 				GROUP BY 					a.id 				UNION ALL 					SELECT 						a.trancode AS Kode, 						a.actname AS Tindakan, 						f.tarif AS Insentive, 						a.disc AS Diskon, 						b.tranDate AS tglTindakan, 						b.custCode AS namaPasien, 						a. CODE AS dokCode, 						'Tind. OK' Deskripsi, 						a.id AS noID 					FROM 						tbtim a 					LEFT JOIN tbdetailok b ON b.trancode = a.trancode 					LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 					LEFT JOIN tbdoctor d ON d.docCode = a.`code` 					LEFT JOIN tbpoli e ON e.polyCode = d.poli 					LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 					AND c.jenisTindakan = e.`name` 					AND f.kelas = b.class 					GROUP BY 						a.id 					UNION ALL 						SELECT 							a.trancode AS Kode, 							a.actname AS Tindakan, 							f.tarif*(80/100) AS Insentive, 							a.disc AS Diskon, 							b.tranDate AS tglTindakan, 							a. CODE AS dokCode, 							b.custCode AS namaPasien, 							'Tind. VK' Deskripsi, 							a.id AS noID 						FROM 							tbtim a 						LEFT JOIN tbdetailvk b ON b.trancode = a.trancode 						LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 						LEFT JOIN tbdoctor d ON d.docCode = a.`code` 						LEFT JOIN tbpoli e ON e.polyCode = d.poli 						LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 						AND c.jenisTindakan = e.`name` 						AND f.kelas = b.class 						GROUP BY 							a.id 	) a LEFT JOIN tbtindakan b ON b.kodeTindakan = a.Tindakan LEFT JOIN tbcustomer c ON c.custCode = a.namaPasien LEFT JOIN tbdoctor d ON d.docCode = a.dokCode WHERE 	a.Tindakan NOT LIKE 'SARANA%' ) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	a.Kode, 	a.Tindakan, 	a.Insentive, 	a.Diskon, 	a.tglTindakan, 	concat( 		d.gelar, 		d.`name`, 		', ', 		d.spec 	) AS dokCode, 	concat( 		c.`name`, 		', ', 		c.salut, 		' (', 		c.custCode, 		')' 	) AS Pasien, 	a.Deskripsi FROM 	( 		SELECT 			a.tranCode AS Kode, 			a.tindakan AS Tindakan, 			f.tarif*(80/100) AS Insentive, 			a.diskon AS Diskon, 			a.tglTindakan AS tglTindakan, 			b.dokter AS dokCode, 			b.custCode AS namaPasien, 			'Tind. Poli' Deskripsi, 			a.id AS noID 		FROM 			tbtindakanrawatjalan a 		LEFT JOIN tbdetailrawatjalan b ON b.tranCode = a.tranCode 		LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 		LEFT JOIN tbdoctor d ON d.docCode = b.dokter 		LEFT JOIN tbpoli e ON e.polyCode = d.poli 		LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 		AND c.jenisTindakan = e.`name` 		GROUP BY 			a.id 		UNION ALL 			SELECT 				a.tranCode AS Kode, 				a.tindakan AS Tindakan, 				g.tarif*(80/100) AS Insentive, 				a.diskon AS Diskon, 				a.tglTindakan AS tglTindakan, 				b.dokter AS dokCode, 				b.custCode AS namaPasien, 				'Visite Ranap' Deskripsi, 				a.id AS noID 			FROM 				tbtindakanrawatinap a 			LEFT JOIN tbdetailrawatinap b ON b.tranCode = a.tranCode 			LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 			LEFT JOIN tbdoctor d ON d.docCode = b.dokter 			LEFT JOIN tbpoli e ON e.polyCode = d.poli 			LEFT JOIN tbadmrawatinap f ON f.tranCode = b.tranCode 			LEFT JOIN tbtarifri g ON g.kodeTindakan = c.kodeTindakan 			AND c.jenisTindakan = e.`name` 			AND g.kelas = f.kelas 			GROUP BY 				a.id 			UNION ALL 				SELECT 					a.tranCode AS Kode, 					a.tindakan AS Tindakan, 					f.tarif*(80/100) AS Insentive, 					a.diskon AS Diskon, 					a.tglTindakan AS tglTindakan, 					a.pelaksana AS dokCode, 					b.custCode AS namaPasien, 					'IGD' Deskripsi, 					a.id AS noID 				FROM 					tbtindakanigd a 				LEFT JOIN tbdetailigd b ON b.tranCode = a.tranCode 				LEFT JOIN tbtindakan c ON c.namaTindakan = a.tindakan 				LEFT JOIN tbdoctor d ON d.docCode = b.dokter 				LEFT JOIN tbpoli e ON e.polyCode = d.poli 				LEFT JOIN tbtarifrj f ON f.kodeTindakan = c.kodeTindakan 				AND c.jenisTindakan = e.`name` 				GROUP BY 					a.id 				UNION ALL 					SELECT 						a.trancode AS Kode, 						a.actname AS Tindakan, 						f.tarif AS Insentive, 						a.disc AS Diskon, 						b.tranDate AS tglTindakan, 						b.custCode AS namaPasien, 						a. CODE AS dokCode, 						'Tind. OK' Deskripsi, 						a.id AS noID 					FROM 						tbtim a 					LEFT JOIN tbdetailok b ON b.trancode = a.trancode 					LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 					LEFT JOIN tbdoctor d ON d.docCode = a.`code` 					LEFT JOIN tbpoli e ON e.polyCode = d.poli 					LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 					AND c.jenisTindakan = e.`name` 					AND f.kelas = b.class 					GROUP BY 						a.id 					UNION ALL 						SELECT 							a.trancode AS Kode, 							a.actname AS Tindakan, 							f.tarif*(80/100) AS Insentive, 							a.disc AS Diskon, 							b.tranDate AS tglTindakan, 							a. CODE AS dokCode, 							b.custCode AS namaPasien, 							'Tind. VK' Deskripsi, 							a.id AS noID 						FROM 							tbtim a 						LEFT JOIN tbdetailvk b ON b.trancode = a.trancode 						LEFT JOIN tbtindakan c ON c.namaTindakan = a.actname 						LEFT JOIN tbdoctor d ON d.docCode = a.`code` 						LEFT JOIN tbpoli e ON e.polyCode = d.poli 						LEFT JOIN tbtarifri f ON f.kodeTindakan = c.kodeTindakan 						AND c.jenisTindakan = e.`name` 						AND f.kelas = b.class 						GROUP BY 							a.id 	) a LEFT JOIN tbtindakan b ON b.kodeTindakan = a.Tindakan LEFT JOIN tbcustomer c ON c.custCode = a.namaPasien LEFT JOIN tbdoctor d ON d.docCode = a.dokCode WHERE 	a.Tindakan NOT LIKE 'SARANA%' ) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_insentive']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tgltindakan']['reg'] = "S";
      $format_dimensions['tgltindakan']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'byTanggal', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_byTanggal($where_ok);
