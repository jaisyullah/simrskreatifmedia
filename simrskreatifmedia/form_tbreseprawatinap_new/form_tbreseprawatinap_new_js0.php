...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbreseprawatinap_new"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_obat_ranap; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_struknya*scinResep*scout"; ?>', '', '_blank', '0', '0', 'obat_ranap');
}
var Crtl_btn_Verif = false;
function sc_btn_Verif()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_Verif(); }, 500);
      return;
    }
    if (Crtl_btn_Verif) {return;}
    scJs_confirm("<?php echo html_entity_decode('Verif Resep ?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_Verif_ok, sc_btn_Verif_cancel)
}
function sc_btn_Verif_cancel()
{
}
function sc_btn_Verif_ok()
{
    Crtl_btn_Verif = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?Verif?@?";
    document.F1.action = "./";
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbreseprawatinap_new']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tbreseprawatinap_new_submit_form(y, z);
