...
//  
   function lookup_pemeriksaan(&$conteudo , $trancode, &$nm_array_retorno_lookup) 
   {
      $nm_array_retorno_lookup = array();
      $conteudo = "";
      $nm_comando = "SELECT a.nama 
FROM tbradiologi a left join tbdetailrad b on b.radcode = a.radcode
WHERE b.trancode = '" . substr($this->Db->qstr($trancode), 1 , -1) . "' 
ORDER BY nama" ; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rx = $this->Db->Execute($nm_comando)) 
