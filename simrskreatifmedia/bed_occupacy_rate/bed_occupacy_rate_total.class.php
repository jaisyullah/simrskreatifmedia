...
class bed_occupacy_rate_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['bed_occupacy_rate']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['bed_occupacy_rate']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['tot_geral'] = array() ;  
    if ($res_limit && isset($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['sql_tot_res']))
    {
        $nm_comando = $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['sql_tot_res'];
    }
    else
    {
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(BOR) from (SELECT 	a.tglKeluar, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(BOR) from (SELECT 	a.tglKeluar, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['contr_total_geral'] = "OK";
   } 

   //-----  tglkeluar
   function quebra_tglkeluar_sc_free_group_by($tglkeluar, $Where_qb, $Cmp_Name) 
   {
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['where_pesq'])) 
      { 
      ...
      $tot_tglkeluar[0] = NM_encode_input(sc_strip_script($tglkeluar)) ; 
      $tot_tglkeluar[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tglkeluar[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_tglkeluar;
   } 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tglkeluar' => "tglKeluar");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('bor' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(BOR)#@#cmps_quebras#@# from (SELECT 	a.tglKeluar, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select BOR as SC_metric1, " . $cmps_quebras1 . " from (SELECT 	a.tglKeluar, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	a.tglKeluar, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['bed_occupacy_rate']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tglkeluar']['reg'] = "N";
      $format_dimensions['tglkeluar']['msk'] = "F/Y";
      while (!$rt->EOF)
      {
          $sql_where = "";
