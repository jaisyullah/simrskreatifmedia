...
$old_error_handler = set_error_handler("import_pasien_scriptcase_error_handler");

function import_pasien_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['import_pasien']['contr_erro']) && $_SESSION['scriptcase']['import_pasien']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_import_pasien->contr_import_pasien->Apl_com_erro = true;
            if ($inicial_import_pasien->contr_import_pasien->NM_ajax_flag)
            {
                $inicial_import_pasien->contr_import_pasien->NM_ajax_info['errList']['geral_import_pasien'][] = $msg;
                return;
            }
            import_pasien_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function import_pasien_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['import_pasien'])  && '' != $_SESSION['scriptcase']['error_icon']['import_pasien'])  ? $_SESSION['scriptcase']['error_icon']['import_pasien']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['import_pasien']) && '' != $_SESSION['scriptcase']['error_close']['import_pasien']) ? $_SESSION['scriptcase']['error_close']['import_pasien'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['import_pasien']) && '' != $_SESSION['scriptcase']['error_span_title']['import_pasien']) ? $_SESSION['scriptcase']['error_span_title']['import_pasien'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['import_pasien']) && '' != $_SESSION['scriptcase']['error_icon_title']['import_pasien']) ? $_SESSION['scriptcase']['error_icon_title']['import_pasien'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['import_pasien']['error_buffer'] .= ob_get_flush();
}
