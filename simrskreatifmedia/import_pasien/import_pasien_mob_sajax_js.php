  ...
  function scAjaxUpdateErrors(sType)
  {
    ...
    if (oResp["errList"])
    {
      for (iFieldErrors = 0; iFieldErrors < oResp["errList"].length; iFieldErrors++)
      {
        ...
        if ("geral_import_pasien_mob" == sTestField)
        {
...
//    return document.F1.elements[sFieldName].value.replace(/[+]/g, "__NM_PLUS__");
  ...
  function scMoveMasterDetail(widget)
  {
	...
	function scAjax_displayEmptyForm() {
        ...
        sc_hide_import_pasien_mob_form();
	}

  // ---------- Validate xls_file
  function do_ajax_import_pasien_mob_validate_xls_file()
  {
    var nomeCampo_xls_file = "xls_file";
    var var_xls_file = scAjaxGetFieldText(nomeCampo_xls_file);
    var var_script_case_init = document.F1.script_case_init.value;
    x_ajax_import_pasien_mob_validate_xls_file(var_xls_file, var_script_case_init, do_ajax_import_pasien_mob_validate_xls_file_cb);
  } // do_ajax_import_pasien_mob_validate_xls_file

  function do_ajax_import_pasien_mob_validate_xls_file_cb(sResp)
  {
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    sFieldValid = "xls_file";
    ...
    scAjaxSetMaster();
    scAjaxSetFocus();
  } // do_ajax_import_pasien_mob_validate_xls_file_cb
...
function scJs_sweetalert_params(params) {
  ...
  // ---------- Form
  function do_ajax_import_pasien_mob_submit_form()
  {
    if (scEventControl_active("")) {
      setTimeout(function() { do_ajax_import_pasien_mob_submit_form(); }, 500);
      return;
    }
    scAjaxHideMessage();
    var var_xls_file = scAjaxGetFieldText("xls_file");
    var var_xls_file_ul_name = scAjaxSpecCharProtect(document.F1.xls_file_ul_name.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_xls_file_ul_type = document.F1.xls_file_ul_type.value;
    var var_xls_file_salva = scAjaxSpecCharProtect(document.F1.xls_file_salva.value);//.replace(/[+]/g, "__NM_PLUS__");
    var var_xls_file_limpa = document.F1.xls_file_limpa.checked ? "S" : "N";
    ...
    scAjaxProcOn();
    x_ajax_import_pasien_mob_submit_form(var_xls_file, var_xls_file_ul_name, var_xls_file_ul_type, var_xls_file_salva, var_xls_file_limpa, var_nm_form_submit, var_nmgp_url_saida, var_nmgp_opcao, var_nmgp_ancora, var_nmgp_num_form, var_nmgp_parms, var_script_case_init, var_csrf_token, do_ajax_import_pasien_mob_submit_form_cb);
  } // do_ajax_import_pasien_mob_submit_form

  function do_ajax_import_pasien_mob_submit_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
...
?>
    ...
    if (!scAjaxHasError())
    {
      ...
      if (document.F1.xls_file_limpa && document.F1.xls_file_limpa.checked)
      {
        var oImgTemp = {0: {"value" : ""}};
        scAjaxSetFieldDocument("xls_file", oImgTemp, "", "");
      }
      else if (document.getElementById("id_ajax_doc_xls_file") && "" != document.getElementById("id_ajax_doc_xls_file").innerHTML)
      {
        document.getElementById("id_ajax_doc_xls_file").style.display = "";
      }
    }
    scAjaxShowDebug();
    scAjaxSetDisplay();
    scAjaxSetLabel();
    scAjaxSetReadonly();
    scAjaxAlert(do_ajax_import_pasien_mob_submit_form_cb_after_alert);
  } // do_ajax_import_pasien_mob_submit_form_cb
  function do_ajax_import_pasien_mob_submit_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
    scAjaxRedir();
  ...
  } // do_ajax_import_pasien_mob_submit_form_cb_after_alert

  var scStatusDetail = {};

  function do_ajax_import_pasien_mob_navigate_form()
  {
    if (scRefreshTable())
    {
      return;
    }
  } // do_ajax_import_pasien_mob_navigate_form

  var scMasterDetailParentIframe = "<?php echo $_SESSION['sc_session'][$this->Ini->sc_page]['import_pasien_mob']['dashboard_info']['parent_widget'] ?>";
...
?>
  function do_ajax_import_pasien_mob_navigate_form_cb(sResp)
  {
    scAjaxProcOff();
    oResp = scAjaxResponse(sResp);
    scAjaxRedir();
    ...
    scAjaxSetBtnVars();
    $('.sc-js-ui-statusimg').css('display', 'none');
    scAjaxAlert(do_ajax_import_pasien_mob_navigate_form_cb_after_alert);
  } // do_ajax_import_pasien_mob_navigate_form_cb
  function do_ajax_import_pasien_mob_navigate_form_cb_after_alert() {
    scAjaxMessage();
    scAjaxJavascript();
    scAjaxSetFocus();
<?php
...
{
?>
    do_ajax_import_pasien_mob_restore_buttons();
<?php
}
?>
    if (hasJsFormOnload)
  ...
  } // do_ajax_import_pasien_mob_navigate_form_cb_after_alert
  function sc_hide_import_pasien_mob_form()
  {
  ...
  ajax_field_list[0] = "xls_file";

  var ajax_block_list = new Array();
  ajax_block_list[0] = "0";

  var ajax_error_list = {
    "xls_file": {"label": "File Excel", "valid": new Array(), "onblur": new Array(), "onchange": new Array(), "onclick": new Array(), "onfocus": new Array(), "timeout": 5}
  };
  ...
  ajax_field_mult["xls_file"][1] = "xls_file";

  var ajax_field_id = {
    "xls_file": new Array("hidden_field_label_xls_file", "hidden_field_data_xls_file")
  };
