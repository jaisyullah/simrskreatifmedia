...
class chart_byjenis_ranap_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_byjenis_ranap']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_byjenis_ranap']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['tot_geral'] = array() ;  
    if ($res_limit && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['sql_tot_res']))
    {
        $nm_comando = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['sql_tot_res'];
    }
    else
    {
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['contr_total_geral'] = "OK";
   } 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('jenis' => "IF
	( date_format( a.TglMasuk,\"%Y-%m-%d\" ) = date_format( b.registerDate,\"%Y-%m-%d\" ),\"Pasien Baru\",\"Pasien Lama\" )");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_byjenis_ranap']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
