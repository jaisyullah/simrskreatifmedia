...
class grid_lap_demo_age_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_lap_demo_age']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_lap_demo_age']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_range_umur($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(jumlah) from (SELECT CASE 		 	WHEN 		umur <= 5 THEN 			'0 - 5'  			WHEN umur BETWEEN 6  			AND 19 THEN 				'6 - 19'  				WHEN umur BETWEEN 20  				AND 39 THEN 					'20 - 39'  					WHEN umur BETWEEN 41  					AND 49 THEN 						'41 - 49'  						WHEN umur BETWEEN 51  						AND 60 THEN 							'51 - 60'  							WHEN umur > 60 THEN 							'> dari 60'  							WHEN umur IS NULL THEN 							'(NULL)'  						END AS range_umur, 						COUNT( * ) AS jumlah  					FROM 						( SELECT custCode, NAME, birthDate, TIMESTAMPDIFF( YEAR, birthDate, CURDATE( ) ) AS umur FROM tbcustomer ) AS dummy_table  					GROUP BY 						range_umur  				) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(jumlah) from (SELECT CASE 		 	WHEN 		umur <= 5 THEN 			'0 - 5'  			WHEN umur BETWEEN 6  			AND 19 THEN 				'6 - 19'  				WHEN umur BETWEEN 20  				AND 39 THEN 					'20 - 39'  					WHEN umur BETWEEN 41  					AND 49 THEN 						'41 - 49'  						WHEN umur BETWEEN 51  						AND 60 THEN 							'51 - 60'  							WHEN umur > 60 THEN 							'> dari 60'  							WHEN umur IS NULL THEN 							'(NULL)'  						END AS range_umur, 						COUNT( * ) AS jumlah  					FROM 						( SELECT custCode, NAME, birthDate, TIMESTAMPDIFF( YEAR, birthDate, CURDATE( ) ) AS umur FROM tbcustomer ) AS dummy_table  					GROUP BY 						range_umur  				) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['contr_total_geral'] = "OK";
   } 

   //-----  range_umur
   function quebra_range_umur_range_umur($range_umur, $arg_sum_range_umur) 
   {
      global $tot_range_umur;
      $tot_range_umur = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(jumlah) from (SELECT CASE 		 	WHEN 		umur <= 5 THEN 			'0 - 5'  			WHEN umur BETWEEN 6  			AND 19 THEN 				'6 - 19'  				WHEN umur BETWEEN 20  				AND 39 THEN 					'20 - 39'  					WHEN umur BETWEEN 41  					AND 49 THEN 						'41 - 49'  						WHEN umur BETWEEN 51  						AND 60 THEN 							'51 - 60'  							WHEN umur > 60 THEN 							'> dari 60'  							WHEN umur IS NULL THEN 							'(NULL)'  						END AS range_umur, 						COUNT( * ) AS jumlah  					FROM 						( SELECT custCode, NAME, birthDate, TIMESTAMPDIFF( YEAR, birthDate, CURDATE( ) ) AS umur FROM tbcustomer ) AS dummy_table  					GROUP BY 						range_umur  				) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['where_pesq']; 
      } 
      ...
      { 
         $nm_comando .= " where range_umur" . $arg_sum_range_umur ; 
      } 
      else 
      { 
         $nm_comando .= " and range_umur" . $arg_sum_range_umur ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_range_umur[0] = sc_strip_script($range_umur) ; 
      $tot_range_umur[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_range_umur[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 


   //----- 
   function Calc_resumo_range_umur($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('range_umur' => "range_umur");
      $cmps_quebra_atual = array("range_umur");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_range_umur($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_range_umur($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      { 
         $cmd_simp = "select count(*), sum(jumlah)#@#cmps_quebras#@# from (SELECT CASE 		 	WHEN 		umur <= 5 THEN 			'0 - 5'  			WHEN umur BETWEEN 6  			AND 19 THEN 				'6 - 19'  				WHEN umur BETWEEN 20  				AND 39 THEN 					'20 - 39'  					WHEN umur BETWEEN 41  					AND 49 THEN 						'41 - 49'  						WHEN umur BETWEEN 51  						AND 60 THEN 							'51 - 60'  							WHEN umur > 60 THEN 							'> dari 60'  							WHEN umur IS NULL THEN 							'(NULL)'  						END AS range_umur, 						COUNT( * ) AS jumlah  					FROM 						( SELECT custCode, NAME, birthDate, TIMESTAMPDIFF( YEAR, birthDate, CURDATE( ) ) AS umur FROM tbcustomer ) AS dummy_table  					GROUP BY 						range_umur  				) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select jumlah as SC_metric1, " . $cmps_quebras1 . " from (SELECT CASE 		 	WHEN 		umur <= 5 THEN 			'0 - 5'  			WHEN umur BETWEEN 6  			AND 19 THEN 				'6 - 19'  				WHEN umur BETWEEN 20  				AND 39 THEN 					'20 - 39'  					WHEN umur BETWEEN 41  					AND 49 THEN 						'41 - 49'  						WHEN umur BETWEEN 51  						AND 60 THEN 							'51 - 60'  							WHEN umur > 60 THEN 							'> dari 60'  							WHEN umur IS NULL THEN 							'(NULL)'  						END AS range_umur, 						COUNT( * ) AS jumlah  					FROM 						( SELECT custCode, NAME, birthDate, TIMESTAMPDIFF( YEAR, birthDate, CURDATE( ) ) AS umur FROM tbcustomer ) AS dummy_table  					GROUP BY 						range_umur  				) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT CASE 		 	WHEN 		umur <= 5 THEN 			'0 - 5'  			WHEN umur BETWEEN 6  			AND 19 THEN 				'6 - 19'  				WHEN umur BETWEEN 20  				AND 39 THEN 					'20 - 39'  					WHEN umur BETWEEN 41  					AND 49 THEN 						'41 - 49'  						WHEN umur BETWEEN 51  						AND 60 THEN 							'51 - 60'  							WHEN umur > 60 THEN 							'> dari 60'  							WHEN umur IS NULL THEN 							'(NULL)'  						END AS range_umur, 						COUNT( * ) AS jumlah  					FROM 						( SELECT custCode, NAME, birthDate, TIMESTAMPDIFF( YEAR, birthDate, CURDATE( ) ) AS umur FROM tbcustomer ) AS dummy_table  					GROUP BY 						range_umur  				) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_demo_age']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['range_umur']['reg'] = "S";
      $format_dimensions['range_umur']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'range_umur', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "range_umur") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_range_umur($where_ok);
