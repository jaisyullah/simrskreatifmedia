...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbtracerrm_multi"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
var var_btn_label_nostruk = "<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dados_form'][$sc_seq_vert]['nostruk_'])?>";
var var_btn_label_nama = "<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dados_form'][$sc_seq_vert]['sc_field_1_'])?>";
var var_btn_label_rm = "<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dados_form'][$sc_seq_vert]['rm_'])?>";
var var_btn_label_unit = "<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dados_form'][$sc_seq_vert]['lokasi_'])?>";
var var_btn_label_tgllahir = "<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['dados_form'][$sc_seq_vert]['usia_'])?>";
function sc_btn_label()
{
    if (scEventControl_active_all()) {
      setTimeout(function() { sc_btn_label(); }, 500);
      return;
    }
    sc_btn_label_ok();
}
function sc_btn_label_cancel()
{
}
function sc_btn_label_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_label_tracer; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "nostruk*scin' + var_btn_label_nostruk + '*scoutnama*scin' + var_btn_label_nama + '*scoutrm*scin' + var_btn_label_rm + '*scoutunit*scin' + var_btn_label_unit + '*scouttgllahir*scin' + var_btn_label_tgllahir + '*scout"; ?>', '', '_blank', '0', '0', 'label_tracer');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbtracerrm_multi']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tbtracerrm_multi_submit_form(y, z);
...
{
   nm_field_disabled("rm_=disabled;jenis_=disabled;sc_field_1_=disabled;usia_=disabled;alamat_=disabled;lokasi_=disabled", "");
