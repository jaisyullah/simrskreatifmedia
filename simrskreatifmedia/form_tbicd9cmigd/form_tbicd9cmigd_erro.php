...
$old_error_handler = set_error_handler("form_tbicd9cmigd_scriptcase_error_handler");

function form_tbicd9cmigd_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['form_tbicd9cmigd']['contr_erro']) && $_SESSION['scriptcase']['form_tbicd9cmigd']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_form_tbicd9cmigd->contr_form_tbicd9cmigd->Apl_com_erro = true;
            if ($inicial_form_tbicd9cmigd->contr_form_tbicd9cmigd->NM_ajax_flag)
            {
                $inicial_form_tbicd9cmigd->contr_form_tbicd9cmigd->NM_ajax_info['errList']['geral_form_tbicd9cmigd'][] = $msg;
                return;
            }
            form_tbicd9cmigd_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function form_tbicd9cmigd_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['form_tbicd9cmigd'])  && '' != $_SESSION['scriptcase']['error_icon']['form_tbicd9cmigd'])  ? $_SESSION['scriptcase']['error_icon']['form_tbicd9cmigd']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['form_tbicd9cmigd']) && '' != $_SESSION['scriptcase']['error_close']['form_tbicd9cmigd']) ? $_SESSION['scriptcase']['error_close']['form_tbicd9cmigd'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['form_tbicd9cmigd']) && '' != $_SESSION['scriptcase']['error_span_title']['form_tbicd9cmigd']) ? $_SESSION['scriptcase']['error_span_title']['form_tbicd9cmigd'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['form_tbicd9cmigd']) && '' != $_SESSION['scriptcase']['error_icon_title']['form_tbicd9cmigd']) ? $_SESSION['scriptcase']['error_icon_title']['form_tbicd9cmigd'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['form_tbicd9cmigd']['error_buffer'] .= ob_get_flush();
}
