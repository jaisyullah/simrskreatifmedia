...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailrawatjalan_old']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbdetailrawatjalan_old"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_HTA_struckno = "<?php echo $this->form_encode_input($this->nmgp_dados_form['nostruk']); ?>";
function sc_btn_HTA()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_HTA(); }, 500);
      return;
    }
    sc_btn_HTA_ok();
}
function sc_btn_HTA_cancel()
{
}
function sc_btn_HTA_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_control_hta_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "struckno*scin' + var_btn_HTA_struckno + '*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'control_hta');
}
...
{ 
   var sob_iframe = '';
<?php 
if ($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailrawatjalan_old']['embutida_form']) {
...
if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailrawatjalan_old']['sc_modal']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailrawatjalan_old']['sc_modal']) {
    echo "   sob_iframe += 'parent.';\r\n";
}
?>
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbdetailrawatjalan_old']['where_detal']))
  {
?>  
...
{
   nm_field_disabled("trancode=disabled;hta=disabled;tp=disabled;uk=disabled", "");
