...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbresependoscopy"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbresependoscopy']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbresependoscopy']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbresependoscopy']['dashboard_info']['parent_widget'] . "']\")";
...
?> 
var var_btn_sc_btn_1_v_trancode = "<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbresependoscopy']['dados_form'][$sc_seq_vert]['trancode_'])?>";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_ambilFormula_endos_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "v_trancode*scin' + var_btn_sc_btn_1_v_trancode + '*scoutNM_btn_insert*scinS*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinS*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'ambilFormula_endos');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbresependoscopy']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tbresependoscopy_submit_form(y, z);
...
{
   nm_field_disabled("biaya_=disabled;trandate_=disabled", "");
