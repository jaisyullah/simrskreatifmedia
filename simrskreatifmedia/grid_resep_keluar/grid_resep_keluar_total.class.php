...
class grid_resep_keluar_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_resep_keluar']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_resep_keluar']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_item($res_limit=false)
   {
      global $nada, $nm_lang , $item;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from (SELECT 	a.item, 	a.tranDate  FROM 	( 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatinap  	) a  WHERE 	a.jumlah = '0') nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*) from (SELECT 	a.item, 	a.tranDate  FROM 	( 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatinap  	) a  WHERE 	a.jumlah = '0') nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['contr_total_geral'] = "OK";
   } 

   //-----  trandate
   function quebra_trandate_item($trandate, $arg_sum_trandate) 
   {
      global $tot_trandate, $item;
      $tot_trandate = array() ;  
      $tot_trandate[0] = $trandate ; 
   }
   //-----  item
   function quebra_item_item($trandate, $item, $arg_sum_trandate, $arg_sum_item) 
   {
   ...
   //----- 
   function Calc_resumo_item($destino_resumo)
   {
      global $nm_lang, $item;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('trandate' => "tranDate",'item' => "item");
      $cmps_quebra_atual = array("trandate","item");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_item($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_item($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      { 
         $cmd_simp = "select count(*)#@#cmps_quebras#@# from (SELECT 	a.item, 	a.tranDate  FROM 	( 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatinap  	) a  WHERE 	a.jumlah = '0') sc_sel_esp " . $where_ok;
         $comando  = "select count(*)#@#cmps_quebras#@# from (";
         $comando .= "select " . $cmps_quebras1 . " from (SELECT 	a.item, 	a.tranDate  FROM 	( 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatinap  	) a  WHERE 	a.jumlah = '0') sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	a.item, 	a.tranDate  FROM 	( 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		jumlah, 		tranDate  	FROM 		tbreseprawatinap  	) a  WHERE 	a.jumlah = '0') sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_resep_keluar']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['trandate']['reg'] = "S";
      $format_dimensions['trandate']['msk'] = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'item', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "item") {
                  $this->Lookup->lookup_item_item($$Cada_dim,  $item);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_item($where_ok);
