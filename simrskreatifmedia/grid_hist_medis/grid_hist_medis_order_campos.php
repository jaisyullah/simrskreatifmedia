...
<?php
   ...
   $Ord_Cmp = new grid_hist_medis_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_hist_medis_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["deskripsi"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['labels']["deskripsi"])) ? $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['labels']["deskripsi"] : "Deskripsi";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       {
           $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
       }
   }
?>
    <script language="javascript"> 
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_hist_medis_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select'];
...
<?php
  $disp_rest = "none";
  if (isset($_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_hist_medis']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
