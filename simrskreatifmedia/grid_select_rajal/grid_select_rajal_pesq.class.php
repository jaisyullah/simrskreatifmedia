...
class grid_select_rajal_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_select_rajal_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_select_rajal_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_select_rajal_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_queue')
      {
          $queue = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_queue($queue);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_struckno')
      {
          $struckno = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_struckno($struckno);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_custcode($custcode)
   {
      ...
      $nm_comando = "select distinct custCode from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and  custCode like '%" . $custcode . "%' order by custCode"; 
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_doctor($doctor)
   {
      ...
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct doctor from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and  doctor like '%" . $doctor . "%' order by doctor"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_queue($queue)
   {
      $queue = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $queue);
      $queue = substr($this->Db->qstr($queue), 1, -1);
            $queue_look = substr($this->Db->qstr($queue), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and   CAST (queue AS TEXT)  like '%" . $queue . "%' order by queue"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and   CAST (queue AS VARCHAR)  like '%" . $queue . "%' order by queue"; 
      }
      else
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and  queue like '%" . $queue . "%' order by queue"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function lookup_ajax_struckno($struckno)
   {
      $struckno = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $struckno);
      $struckno = substr($this->Db->qstr($struckno), 1, -1);
            $struckno_look = substr($this->Db->qstr($struckno), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and   CAST (struckNo AS TEXT)  like '%" . $struckno . "%' order by struckNo"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and   CAST (struckNo AS VARCHAR)  like '%" . $struckno . "%' order by struckNo"; 
      }
      else
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and  struckNo like '%" . $struckno . "%' order by struckNo"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Antrian</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_select_rajal/grid_select_rajal_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_select_rajal_ajax_search.js"></script>
 <script type="text/javascript" src="grid_select_rajal_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_select_rajal_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_select_rajal']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_select_rajal']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  str_out += 'SC_queue_cond#NMF#' + search_get_sel_txt('SC_queue_cond') + '@NMF@';
  str_out += 'SC_queue#NMF#' + search_get_text('SC_queue') + '@NMF@';
  str_out += 'id_ac_queue#NMF#' + search_get_text('id_ac_queue') + '@NMF@';
  str_out += 'SC_struckno_cond#NMF#' + search_get_sel_txt('SC_struckno_cond') + '@NMF@';
  str_out += 'SC_struckno#NMF#' + search_get_text('SC_struckno') + '@NMF@';
  str_out += 'id_ac_struckno#NMF#' + search_get_text('id_ac_struckno') + '@NMF@';
   ...
   $("#id_ac_queue").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_queue",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_queue").val( $(this).val() );
       }
     }
   });
   $("#id_ac_struckno").autocomplete({
     minLength: 1,
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_struckno",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Antrian</div>
...
<?php
   ...
   function monta_form()
   {
      global 
             ...
             $queue_cond, $queue, $queue_autocomp,
             $struckno_cond, $struckno, $struckno_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $custcode = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['custcode']; 
          $custcode_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['custcode_cond']; 
          $doctor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['doctor']; 
          $doctor_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['doctor_cond']; 
          $queue = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['queue']; 
          $queue_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['queue_cond']; 
          $struckno = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['struckno']; 
          $struckno_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['struckno_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['NM_operador']; 
      } 
      ...
      if (!isset($queue_cond) || empty($queue_cond))
      {
         $queue_cond = "gt";
      }
      if (!isset($struckno_cond) || empty($struckno_cond))
      {
         $struckno_cond = "qp";
      }
      ...
      $str_hide_queue = (in_array($queue_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_struckno = (in_array($struckno_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      {
         $tmp_pos = strpos($queue, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $queue = substr($queue, 0, $tmp_pos);
         }
      }
      if (!isset($struckno) || $struckno == "")
      {
          $struckno = "";
      }
      if (isset($struckno) && !empty($struckno))
      {
         $tmp_pos = strpos($struckno, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $struckno = substr($struckno, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
...
<?php
      ...
      $custcode_look = substr($this->Db->qstr($custcode), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct custCode from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and custCode = '$custcode_look' order by custCode"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
...
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_custcode" name="custcode_autocomp" size="8"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 8, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_doctor_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['doctor'])) ? $this->New_label['doctor'] : "Dokter"; ?></TD>
...
<?php
      ...
      $doctor_look = substr($this->Db->qstr($doctor), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct doctor from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and doctor = '$doctor_look' order by doctor"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
...
?>
<INPUT  type="text" id="SC_doctor" name="doctor" value="<?php echo NM_encode_input($doctor) ?>"  size=8 alt="{datatype: 'text', maxLength: 8, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_doctor" name="doctor_autocomp" size="8"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 8, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_queue_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['queue'])) ? $this->New_label['queue'] : "No. Antrian"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_queue_cond" name="queue_cond" onChange="nm_campos_between(document.getElementById('id_vis_queue'), this, 'queue')">
       <OPTION value="gt" <?php if ("gt" == $queue_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $queue_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $queue_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_queue" <?php echo $str_hide_queue?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['queue'])) ? $this->New_label['queue'] : "No. Antrian";
 $nmgp_tab_label .= "queue?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and queue = $queue_look order by queue"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and queue = $queue_look order by queue"; 
      ...
      if (isset($nmgp_def_dados[0][$queue]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$queue];
      }
      else
      {
            nmgp_Form_Num_Val($queue, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $queue;
      }
?>
<INPUT  type="text" id="SC_queue" name="queue" value="<?php echo NM_encode_input($queue) ?>" size=11 alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_queue" name="queue_autocomp" size="11"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_struckno_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['struckno'])) ? $this->New_label['struckno'] : "No. Struk"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_struckno_cond" name="struckno_cond" onChange="nm_campos_between(document.getElementById('id_vis_struckno'), this, 'struckno')">
       <OPTION value="qp" <?php if ("qp" == $struckno_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $struckno_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $struckno_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_struckno" <?php echo $str_hide_struckno?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['struckno'])) ? $this->New_label['struckno'] : "No. Struk";
 $nmgp_tab_label .= "struckno?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $struckno_look = substr($this->Db->qstr($struckno), 1, -1); 
      $nmgp_def_dados = array(); 
   if (is_numeric($struckno))
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and struckNo = $struckno_look order by struckNo"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where not poly = 'POL004' and status = 'Antre' AND regDate = CURDATE() and doctor = '" . $_SESSION['v_dokter_terpilih'] . "' and struckNo = $struckno_look order by struckNo"; 
      ...
      if (isset($nmgp_def_dados[0][$struckno]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$struckno];
      }
      else
      {
            nmgp_Form_Num_Val($struckno, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $struckno;
      }
?>
<INPUT  type="text" id="SC_struckno" name="struckno" value="<?php echo NM_encode_input($struckno) ?>" size=10 alt="{datatype: 'integer', maxLength: 10, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_struckno" name="struckno_autocomp" size="10"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 10, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
...
?>
<?php
   if (is_file("grid_select_rajal_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_select_rajal']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_select_rajal']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.queue_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_queue'), document.F1.queue_cond, 'queue');
   document.F1.queue.value = "";
   document.F1.queue_autocomp.value = "";
   document.F1.struckno_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_struckno'), document.F1.struckno_cond, 'struckno');
   document.F1.struckno.value = "";
   document.F1.struckno_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_select_rajal";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_queue_cond'] = 'cond';
      $tp_fields['SC_queue'] = 'text_aut';
      $tp_fields['id_ac_queue'] = 'text_aut';
      $tp_fields['SC_struckno_cond'] = 'cond';
      $tp_fields['SC_struckno'] = 'text_aut';
      $tp_fields['id_ac_struckno'] = 'text_aut';
   ...
   {
      global $custcode_cond, $custcode, $custcode_autocomp,
             $doctor_cond, $doctor, $doctor_autocomp,
             $queue_cond, $queue, $queue_autocomp,
             $struckno_cond, $struckno, $struckno_autocomp, $nmgp_tab_label;
      ...
      if (!empty($queue_autocomp) && empty($queue))
      {
          $queue = $queue_autocomp;
      }
      if (!empty($struckno_autocomp) && empty($struckno))
      {
          $struckno = $struckno_autocomp;
      }
      ...
      $queue_cond_salva = $queue_cond; 
      if (!isset($queue_input_2) || $queue_input_2 == "")
      {
          $queue_input_2 = $queue;
      }
      $struckno_cond_salva = $struckno_cond; 
      if (!isset($struckno_input_2) || $struckno_input_2 == "")
      {
          $struckno_input_2 = $struckno;
      }
      if ($queue_cond != "in")
      {
          nm_limpa_numero($queue, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      if ($struckno_cond != "in")
      {
          nm_limpa_numero($struckno, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['custcode'] = $custcode; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['custcode_cond'] = $custcode_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['doctor'] = $doctor; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['doctor_cond'] = $doctor_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['queue'] = $queue; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['queue_cond'] = $queue_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['struckno'] = $struckno; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['struckno_cond'] = $struckno_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca'] = $Temp_Busca;
      }
      if ($queue_cond != "in" && $queue_cond != "bw" && !empty($queue) && !is_numeric($queue))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : No. Antrian";
      }
      if ($queue_cond == "bw" && ((!empty($queue) && !is_numeric($queue)) || (!empty($queue_input_2) && !is_numeric($queue_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : No. Antrian";
      }
      if ($struckno_cond != "in" && $struckno_cond != "bw" && !empty($struckno) && !is_numeric($struckno))
      {
      ...
      if ($struckno_cond == "bw" && ((!empty($struckno) && !is_numeric($struckno)) || (!empty($struckno_input_2) && !is_numeric($struckno_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : No. Struk";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['struckno'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['struckno'] = $cmp1;
      }
      else
      {
          $cmp1 = $struckno;
      ...
      //----- $queue
      $this->Date_part = false;
      if (isset($queue) || $queue_cond == "nu" || $queue_cond == "nn" || $queue_cond == "ep" || $queue_cond == "ne")
      {
         $this->monta_condicao("queue", $queue_cond, $queue, "", "queue");
      }

      //----- $struckno
      $this->Date_part = false;
      if (isset($struckno) || $struckno_cond == "nu" || $struckno_cond == "nn" || $struckno_cond == "ep" || $struckno_cond == "ne")
      {
         $this->monta_condicao("struckNo", $struckno_cond, $struckno, "", "struckno");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_rajal']['cond_pesq'] .= $this->NM_operador;
