...
$old_error_handler = set_error_handler("setTindakan_scriptcase_error_handler");

function setTindakan_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['setTindakan']['contr_erro']) && $_SESSION['scriptcase']['setTindakan']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_setTindakan->contr_setTindakan->Apl_com_erro = true;
            if ($inicial_setTindakan->contr_setTindakan->NM_ajax_flag)
            {
                $inicial_setTindakan->contr_setTindakan->NM_ajax_info['errList']['geral_setTindakan'][] = $msg;
                return;
            }
            setTindakan_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function setTindakan_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['setTindakan'])  && '' != $_SESSION['scriptcase']['error_icon']['setTindakan'])  ? $_SESSION['scriptcase']['error_icon']['setTindakan']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['setTindakan']) && '' != $_SESSION['scriptcase']['error_close']['setTindakan']) ? $_SESSION['scriptcase']['error_close']['setTindakan'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['setTindakan']) && '' != $_SESSION['scriptcase']['error_span_title']['setTindakan']) ? $_SESSION['scriptcase']['error_span_title']['setTindakan'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['setTindakan']) && '' != $_SESSION['scriptcase']['error_icon_title']['setTindakan']) ? $_SESSION['scriptcase']['error_icon_title']['setTindakan'] : '';
?>
