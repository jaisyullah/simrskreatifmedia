...
class grid_tbbu_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_tbbu_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_tbbu_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_tbbu_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_id_bu')
      {
          $id_bu = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_id_bu($id_bu);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_nama_bu')
      {
          $nama_bu = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_nama_bu($nama_bu);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_alamat_bu')
      {
          $alamat_bu = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_alamat_bu($alamat_bu);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_id_bu($id_bu)
   {
      $id_bu = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $id_bu);
      $id_bu = substr($this->Db->qstr($id_bu), 1, -1);
            $id_bu_look = substr($this->Db->qstr($id_bu), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct id_bu from " . $this->Ini->nm_tabela . " where   CAST (id_bu AS TEXT)  like '%" . $id_bu . "%' order by id_bu"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct id_bu from " . $this->Ini->nm_tabela . " where   CAST (id_bu AS VARCHAR)  like '%" . $id_bu . "%' order by id_bu"; 
      }
      else
      {
          $nm_comando = "select distinct id_bu from " . $this->Ini->nm_tabela . " where  id_bu like '%" . $id_bu . "%' order by id_bu"; 
      }
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_nama_bu($nama_bu)
   {
      $nama_bu = substr($this->Db->qstr($nama_bu), 1, -1);
            $nama_bu_look = substr($this->Db->qstr($nama_bu), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct nama_bu from " . $this->Ini->nm_tabela . " where  nama_bu like '%" . $nama_bu . "%' order by nama_bu"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_alamat_bu($alamat_bu)
   {
      $alamat_bu = substr($this->Db->qstr($alamat_bu), 1, -1);
            $alamat_bu_look = substr($this->Db->qstr($alamat_bu), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct alamat_bu from " . $this->Ini->nm_tabela . " where  alamat_bu like '%" . $alamat_bu . "%' order by alamat_bu"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Perusahaan</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_tbbu/grid_tbbu_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_tbbu_ajax_search.js"></script>
 <script type="text/javascript" src="grid_tbbu_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_tbbu_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbbu']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbbu']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_id_bu_cond#NMF#' + search_get_sel_txt('SC_id_bu_cond') + '@NMF@';
  str_out += 'SC_id_bu#NMF#' + search_get_text('SC_id_bu') + '@NMF@';
  str_out += 'id_ac_id_bu#NMF#' + search_get_text('id_ac_id_bu') + '@NMF@';
  str_out += 'SC_nama_bu_cond#NMF#' + search_get_sel_txt('SC_nama_bu_cond') + '@NMF@';
  str_out += 'SC_nama_bu#NMF#' + search_get_text('SC_nama_bu') + '@NMF@';
  str_out += 'id_ac_nama_bu#NMF#' + search_get_text('id_ac_nama_bu') + '@NMF@';
  str_out += 'SC_alamat_bu_cond#NMF#' + search_get_sel_txt('SC_alamat_bu_cond') + '@NMF@';
  str_out += 'SC_alamat_bu#NMF#' + search_get_text('SC_alamat_bu') + '@NMF@';
  str_out += 'id_ac_alamat_bu#NMF#' + search_get_text('id_ac_alamat_bu') + '@NMF@';
   ...
   $("#id_ac_id_bu").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          ...
          nmgp_parms: "NM_ajax_opcao?#?autocomp_id_bu",
          max_itens: "10",
          cod_desc: "N",
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_id_bu").val( $(this).val() );
       }
     }
   });
   $("#id_ac_nama_bu").autocomplete({
     minLength: 1,
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_nama_bu",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_nama_bu").val( $(this).val() );
       }
     }
   });
   $("#id_ac_alamat_bu").autocomplete({
     minLength: 1,
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_alamat_bu",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_alamat_bu").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     focus: function (event, ui) {
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Perusahaan</div>
...
<?php
   ...
   {
      global 
             $id_bu_cond, $id_bu, $id_bu_autocomp,
             $nama_bu_cond, $nama_bu, $nama_bu_autocomp,
             $alamat_bu_cond, $alamat_bu, $alamat_bu_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $id_bu = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['id_bu']; 
          $id_bu_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['id_bu_cond']; 
          $nama_bu = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['nama_bu']; 
          $nama_bu_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['nama_bu_cond']; 
          $alamat_bu = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['alamat_bu']; 
          $alamat_bu_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['alamat_bu_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['NM_operador']; 
      } 
      if (!isset($id_bu_cond) || empty($id_bu_cond))
      {
         $id_bu_cond = "gt";
      }
      if (!isset($nama_bu_cond) || empty($nama_bu_cond))
      {
         $nama_bu_cond = "qp";
      }
      if (!isset($alamat_bu_cond) || empty($alamat_bu_cond))
      {
         $alamat_bu_cond = "qp";
      }
      ...
      $str_hide_id_bu = (in_array($id_bu_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_nama_bu = (in_array($nama_bu_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_alamat_bu = (in_array($alamat_bu_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      if (!isset($id_bu) || $id_bu == "")
      {
          $id_bu = "";
      }
      if (isset($id_bu) && !empty($id_bu))
      {
         $tmp_pos = strpos($id_bu, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $id_bu = substr($id_bu, 0, $tmp_pos);
         }
      }
      if (!isset($nama_bu) || $nama_bu == "")
      {
          $nama_bu = "";
      }
      if (isset($nama_bu) && !empty($nama_bu))
      {
         $tmp_pos = strpos($nama_bu, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $nama_bu = substr($nama_bu, 0, $tmp_pos);
         }
      }
      if (!isset($alamat_bu) || $alamat_bu == "")
      {
          $alamat_bu = "";
      }
      if (isset($alamat_bu) && !empty($alamat_bu))
      {
         $tmp_pos = strpos($alamat_bu, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $alamat_bu = substr($alamat_bu, 0, $tmp_pos);
         }
      }
?>
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD id='SC_id_bu_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['id_bu'])) ? $this->New_label['id_bu'] : "ID Perusahaan"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_id_bu_cond" name="id_bu_cond" onChange="nm_campos_between(document.getElementById('id_vis_id_bu'), this, 'id_bu')">
       <OPTION value="gt" <?php if ("gt" == $id_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $id_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $id_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_id_bu" <?php echo $str_hide_id_bu?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['id_bu'])) ? $this->New_label['id_bu'] : "ID Perusahaan";
 $nmgp_tab_label .= "id_bu?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct id_bu from " . $this->Ini->nm_tabela . " where id_bu = $id_bu_look order by id_bu"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct id_bu from " . $this->Ini->nm_tabela . " where id_bu = $id_bu_look order by id_bu"; 
      ...
      if (isset($nmgp_def_dados[0][$id_bu]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$id_bu];
      }
      else
      {
            nmgp_Form_Num_Val($id_bu, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $id_bu;
      }
?>
<INPUT  type="text" id="SC_id_bu" name="id_bu" value="<?php echo NM_encode_input($id_bu) ?>" size=5 alt="{datatype: 'integer', maxLength: 5, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_id_bu" name="id_bu_autocomp" size="5"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 5, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_nama_bu_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['nama_bu'])) ? $this->New_label['nama_bu'] : "Nama Perusahaan"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_nama_bu_cond" name="nama_bu_cond" onChange="nm_campos_between(document.getElementById('id_vis_nama_bu'), this, 'nama_bu')">
       <OPTION value="qp" <?php if ("qp" == $nama_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $nama_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $nama_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $nama_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_nama_bu" <?php echo $str_hide_nama_bu?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['nama_bu'])) ? $this->New_label['nama_bu'] : "Nama Perusahaan";
 $nmgp_tab_label .= "nama_bu?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $nama_bu_look = substr($this->Db->qstr($nama_bu), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct nama_bu from " . $this->Ini->nm_tabela . " where nama_bu = '$nama_bu_look' order by nama_bu"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$nama_bu]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$nama_bu];
      }
      else
      {
          $sAutocompValue = $nama_bu;
      }
?>
<INPUT  type="text" id="SC_nama_bu" name="nama_bu" value="<?php echo NM_encode_input($nama_bu) ?>"  size=50 alt="{datatype: 'text', maxLength: 150, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_nama_bu" name="nama_bu_autocomp" size="50"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 150, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_alamat_bu_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['alamat_bu'])) ? $this->New_label['alamat_bu'] : "Alamat Perusahaan"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_alamat_bu_cond" name="alamat_bu_cond" onChange="nm_campos_between(document.getElementById('id_vis_alamat_bu'), this, 'alamat_bu')">
       <OPTION value="qp" <?php if ("qp" == $alamat_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $alamat_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $alamat_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $alamat_bu_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_alamat_bu" <?php echo $str_hide_alamat_bu?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['alamat_bu'])) ? $this->New_label['alamat_bu'] : "Alamat Perusahaan";
 $nmgp_tab_label .= "alamat_bu?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $alamat_bu_look = substr($this->Db->qstr($alamat_bu), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct alamat_bu from " . $this->Ini->nm_tabela . " where alamat_bu = '$alamat_bu_look' order by alamat_bu"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$alamat_bu]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$alamat_bu];
      }
      else
      {
          $sAutocompValue = $alamat_bu;
      }
?>
<INPUT  type="text" id="SC_alamat_bu" name="alamat_bu" value="<?php echo NM_encode_input($alamat_bu) ?>"  size=50 alt="{datatype: 'text', maxLength: 255, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_alamat_bu" name="alamat_bu_autocomp" size="50"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 255, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tbbu']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tbbu']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.id_bu_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_id_bu'), document.F1.id_bu_cond, 'id_bu');
   document.F1.id_bu.value = "";
   document.F1.id_bu_autocomp.value = "";
   document.F1.nama_bu_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_nama_bu'), document.F1.nama_bu_cond, 'nama_bu');
   document.F1.nama_bu.value = "";
   document.F1.nama_bu_autocomp.value = "";
   document.F1.alamat_bu_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_alamat_bu'), document.F1.alamat_bu_cond, 'alamat_bu');
   document.F1.alamat_bu.value = "";
   document.F1.alamat_bu_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_tbbu";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_id_bu_cond'] = 'cond';
      $tp_fields['SC_id_bu'] = 'text_aut';
      $tp_fields['id_ac_id_bu'] = 'text_aut';
      $tp_fields['SC_nama_bu_cond'] = 'cond';
      $tp_fields['SC_nama_bu'] = 'text_aut';
      $tp_fields['id_ac_nama_bu'] = 'text_aut';
      $tp_fields['SC_alamat_bu_cond'] = 'cond';
      $tp_fields['SC_alamat_bu'] = 'text_aut';
      $tp_fields['id_ac_alamat_bu'] = 'text_aut';
   ...
   {
      global $id_bu_cond, $id_bu, $id_bu_autocomp,
             $nama_bu_cond, $nama_bu, $nama_bu_autocomp,
             $alamat_bu_cond, $alamat_bu, $alamat_bu_autocomp, $nmgp_tab_label;

      $C_formatado = true;
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($id_bu_autocomp) && empty($id_bu))
      {
          $id_bu = $id_bu_autocomp;
      }
      if (!empty($nama_bu_autocomp) && empty($nama_bu))
      {
          $nama_bu = $nama_bu_autocomp;
      }
      if (!empty($alamat_bu_autocomp) && empty($alamat_bu))
      {
          $alamat_bu = $alamat_bu_autocomp;
      }
      $id_bu_cond_salva = $id_bu_cond; 
      if (!isset($id_bu_input_2) || $id_bu_input_2 == "")
      {
          $id_bu_input_2 = $id_bu;
      }
      $nama_bu_cond_salva = $nama_bu_cond; 
      if (!isset($nama_bu_input_2) || $nama_bu_input_2 == "")
      {
          $nama_bu_input_2 = $nama_bu;
      }
      $alamat_bu_cond_salva = $alamat_bu_cond; 
      if (!isset($alamat_bu_input_2) || $alamat_bu_input_2 == "")
      {
          $alamat_bu_input_2 = $alamat_bu;
      }
      if ($id_bu_cond != "in")
      {
          nm_limpa_numero($id_bu, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['id_bu'] = $id_bu; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['id_bu_cond'] = $id_bu_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['nama_bu'] = $nama_bu; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['nama_bu_cond'] = $nama_bu_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['alamat_bu'] = $alamat_bu; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['alamat_bu_cond'] = $alamat_bu_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca'] = $Temp_Busca;
      }
      if ($id_bu_cond != "in" && $id_bu_cond != "bw" && !empty($id_bu) && !is_numeric($id_bu))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : ID Perusahaan";
      }
      if ($id_bu_cond == "bw" && ((!empty($id_bu) && !is_numeric($id_bu)) || (!empty($id_bu_input_2) && !is_numeric($id_bu_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : ID Perusahaan";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['nama_bu'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['nama_bu'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['nama_bu'] = $nama_bu;
      }
      $alamat_bu_look = substr($this->Db->qstr($alamat_bu), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct alamat_bu from " . $this->Ini->nm_tabela . " where alamat_bu = '$alamat_bu_look' order by alamat_bu"; 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['alamat_bu'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['alamat_bu'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['alamat_bu'] = $alamat_bu;
      }

      //----- $id_bu
      $this->Date_part = false;
      if (isset($id_bu) || $id_bu_cond == "nu" || $id_bu_cond == "nn" || $id_bu_cond == "ep" || $id_bu_cond == "ne")
      {
         $this->monta_condicao("id_bu", $id_bu_cond, $id_bu, "", "id_bu");
      }

      //----- $nama_bu
      $this->Date_part = false;
      if (isset($nama_bu) || $nama_bu_cond == "nu" || $nama_bu_cond == "nn" || $nama_bu_cond == "ep" || $nama_bu_cond == "ne")
      {
         $this->monta_condicao("nama_bu", $nama_bu_cond, $nama_bu, "", "nama_bu");
      }

      //----- $alamat_bu
      $this->Date_part = false;
      if (isset($alamat_bu) || $alamat_bu_cond == "nu" || $alamat_bu_cond == "nn" || $alamat_bu_cond == "ep" || $alamat_bu_cond == "ne")
      {
         $this->monta_condicao("alamat_bu", $alamat_bu_cond, $alamat_bu, "", "alamat_bu");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbu']['cond_pesq'] .= $this->NM_operador;
