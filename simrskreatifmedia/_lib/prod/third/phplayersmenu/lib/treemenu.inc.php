	...
	)
{
	...
	$_SESSION['nm_session']['menu_tree']['pastas'] = array();
	...
	for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {
		...
		if ($cnt<$this->_lastItem[$menu_name] && $this->tree[$cnt]['level']<$this->tree[$cnt+1]['level']) {
			...
			if ($this->tree[$cnt]['expanded'] != 1) {
				...
				$_SESSION['nm_session']['menu_tree']['pastas'][] = $cnt;
			} else {
...
*/
	...
	$this->_treeMenuToggleFunction[$menu_name] = $toggle_function;
	$this->_treeMenuToggle[$menu_name] = $toggle;
	
	$this->_treeMenu[$menu_name] =  $this->_treeMenu[$menu_name];
...
function printTreeMenuToggle($menu_name)
{
	print $this->_treeMenuToggle[$menu_name];
}
function printTreeMenuToggleFunction($menu_name)
{
