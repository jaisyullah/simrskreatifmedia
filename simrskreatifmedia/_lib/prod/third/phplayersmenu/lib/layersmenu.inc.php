        ...
        )
{
        ...
        for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {        // this counter scans all nodes of the new menu
                ...
                if ($this->tree[$cnt]['child_of_root_node']) {
                        ...
                        if ($this->tree[$cnt]['not_a_leaf']) {
                                $this->tree[$cnt]['onmouseover'] = ' onmouseover="moveLayerX1(' . "'" . $this->tree[$cnt]['layer_label'] . "', '" . $menu_name . "') ; LMPopUp('" . $this->tree[$cnt]['layer_label'] . "'" . ', false);NM_show_menu(\'' . $menu_name . $this->tree[$cnt]['layer_label'] . '\');"';
                        } else {
                        ...
                        if ($this->tree[$cnt]['not_a_leaf']) {
                                $this->tree[$cnt]['onmouseover'] = ' onmouseover="moveLayerX(' . "'" . $this->tree[$cnt]['layer_label'] . "') ; moveLayerY('" . $this->tree[$cnt]['layer_label'] . "') ; LMPopUp('" . $this->tree[$cnt]['layer_label'] . "'". ', false);NM_show_menu(\'ref' . $this->tree[$cnt]['layer_label'] . '\');"';
                        } else {
                                $this->tree[$cnt]['onmouseover'] = ' onmouseover="LMPopUp(' . "'" . $this->tree[$this->tree[$cnt]['father_node']]['layer_label'] . "'" . ', true);NM_show_menu(\'ref' . $this->tree[$cnt]['layer_label'] . '\');"';
                        }
                        $t_sub->setVar(array(
                                'imgwww'        => $this->imgwww,
                                'transparent'        => $this->transparentIcon,
