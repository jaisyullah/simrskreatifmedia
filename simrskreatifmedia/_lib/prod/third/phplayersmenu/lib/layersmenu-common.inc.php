...
function setMenuStructureFile($tree_file)
{
        ...
        while ($buffer = fgets($fd, 4096)) {
                $buffer = strtr($buffer,chr(13), '');	// Microsoft Stupidity Suppression
...
{
        $this->menuStructure = strtr($tree_string, chr(13), '');        // Microsoft Stupidity Suppression
        if ($this->menuStructure == '') {
        ...
        )
{
        for ($cnt=$this->_firstItem[$menu_name]; $cnt<=$this->_lastItem[$menu_name]; $cnt++) {        // this counter scans all nodes of the new menu
                ...
                $this->tree[$cnt]['parsed_href'] = (trim($this->tree[$cnt]['href']) == '') ? '#' : $this->prependedUrl . $this->tree[$cnt]['href'];
