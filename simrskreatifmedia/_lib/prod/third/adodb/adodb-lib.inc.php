			...
			$size=0, $selectAttr='',$compareFields0=true)
{
	...
	while(!$zthis->EOF) {
		...
		if ($hasvalue) 
			$value = " value='".htmlspecialchars($zval2, ENT_COMPAT, "ISO-8859-1")."'";
		
		if (is_array($defstr))  {
			
			if (in_array($selected,$defstr)) 
				$s .= "\n<option selected='selected'$value>".htmlspecialchars($zval, ENT_COMPAT, "ISO-8859-1").'</option>';
			else 
				$s .= "\n<option".$value.'>'.htmlspecialchars($zval, ENT_COMPAT, "ISO-8859-1").'</option>';
		}
		else {
			...
			$size=0, $selectAttr='',$compareFields0=true)
{
	...
	while(!$zthis->EOF) {
        ...
        if ($optgroup != $group) {
            ...
            if ($firstgroup) {
                $firstgroup = false;
                $s .="\n<optgroup label='". htmlspecialchars($group, ENT_COMPAT, "ISO-8859-1") ."'>";
            } else {
                $s .="\n</optgroup>";
                $s .="\n<optgroup label='". htmlspecialchars($group, ENT_COMPAT, "ISO-8859-1") ."'>";
            }
...
function _adodb_debug_execute(&$zthis, $sql, $inputarr)
{
	...
	*/
	...
	if ($inBrowser) {
		if ($ss) {
			$ss = '<code>'.htmlspecialchars($ss, ENT_COMPAT, "ISO-8859-1").'</code>';
		}
		if ($zthis->debug === -1)
			ADOConnection::outp( "<br />\n($dbt): ".htmlspecialchars($sqlTxt, ENT_COMPAT, "ISO-8859-1")." &nbsp; $ss\n<br />\n",false);
		else 
		...
		if (isset($arr['args']))
		 foreach($arr['args'] as $v) {
			if (is_null($v)) $args[] = 'null';
				...
				$v = (string) @$v;
				$str = htmlspecialchars(substr($v,0,$MAXSTRLEN), ENT_COMPAT, "ISO-8859-1");
