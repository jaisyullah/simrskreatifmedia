...
function adodb_log_sql(&$connx,$sql,$inputarr)
{
	...
	global $ADODB_LOG_CONN;
		...
		if (!$rs) {
			...
			$errN = $connx->ErrorNo();
			$conn->lastInsID = 0;
			$tracer = substr('ERROR: '.htmlspecialchars($errM, ENT_COMPAT, "ISO-8859-1"),0,250);
		} else {
			$tracer = '';
			$errM = '';
			$errN = 0;
		...
		if (isset($_SERVER['HTTP_HOST'])) {
			...
			if (isset($_SERVER['PHP_SELF'])) $tracer .= htmlspecialchars($_SERVER['PHP_SELF'], ENT_COMPAT, "ISO-8859-1");
		} else
			if (isset($_SERVER['PHP_SELF'])) $tracer .= '<br>'.htmlspecialchars($_SERVER['PHP_SELF'], ENT_COMPAT, "ISO-8859-1");
...
<table border=1 bgcolor=white><tr><td><b>Avg Time</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
			...
			while (!$rs->EOF) {
				...
				$s .= "<tr><td>".adodb_round($rs->fields[0],6)."<td align=right>".$rs->fields[2]."<td><font size=-1>".$prefix.htmlspecialchars($sql, ENT_COMPAT, "ISO-8859-1").$suffix."</font>".
...
<table border=1 bgcolor=white><tr><td><b>Load</b><td><b>Count</b><td><b>SQL</b><td><b>Max</b><td><b>Min</b></tr>\n";
	...
	<a href=?do=stats><b>Performance Stats</b></a> &nbsp; <a href=?do=viewsql><b>View SQL</b></a>
	 ...
	 "</tr></table>";
		...
		case 'stats':
			...
			break;
		case 'poll':
			$self = htmlspecialchars($_SERVER['PHP_SELF'], ENT_COMPAT, "ISO-8859-1");
	...
	*/
	...
	function DoSQLForm()
	{
		$PHP_SELF = htmlspecialchars($_SERVER['PHP_SELF'], ENT_COMPAT, "ISO-8859-1");
...
<input type="submit" value=" Run SQL Below " name="RUN"><input type=hidden name=do value=dosql>
  ...
  <tr>
  <td colspan=2><textarea rows=<?php print $rows; ?> name="sql" cols="80"><?php print htmlspecialchars($sql, ENT_COMPAT, "ISO-8859-1") ?></textarea>
  </td>
  </tr>
 </table>
</form>
...
<?php
		...
		foreach($sqla as $sqls) {
			...
			if ($print) {
				print "<p>".htmlspecialchars($sqls, ENT_COMPAT, "ISO-8859-1")."</p>";
				flush();
			}
			$savelog = $this->conn->LogSQL(false);
