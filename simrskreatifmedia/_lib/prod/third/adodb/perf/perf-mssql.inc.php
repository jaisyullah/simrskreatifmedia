...
class perf_mssql extends adodb_perf{
	...
	function Explain($sql,$partial=false)
	{
		...
		$s = '<p><b>Explain</b>: '.htmlspecialchars($sql, ENT_COMPAT, "ISO-8859-1").'</p>';
		...
		if ($rs) {
			...
			while (!$rs->EOF) {
				$s .= '<tr><td>'.round($rs->fields[8],1).'<td>'.round($rs->fields[9],3).'<td align=right>'.round($rs->fields[10],3).'<td nowrap><pre>'.htmlspecialchars($rs->fields[0], ENT_COMPAT, "ISO-8859-1")."</td></pre></tr>\n"; ## NOTE CORRUPT </td></pre> tag is intentional!!!!
				$rs->MoveNext();
			}
			$s .= '</table>';
