...
class grid_tbpayment_xls
{
   ...
   function monta_xls()
   {
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida']) {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['opcao'] = "";
      }
   }
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          { 
              ...
              $this->Res_xls = new grid_tbpayment_res_xls();
              $this->prep_modulos("Res_xls");
          } 
          $this->Arquivo    = "sc_xls";
          $this->Arquivo   .= "_" . date("YmdHis") . "_" . rand(0, 1000);
          $this->Arq_zip    = $this->Arquivo . "_grid_tbpayment.zip";
          $this->Arquivo   .= "_grid_tbpayment" . $this->Xls_tp;
          $this->Tit_doc    = "grid_tbpayment" . $this->Xls_tp;
          $this->Tit_zip    = "grid_tbpayment.zip";
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_name']);
          $this->Xls_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
          $this->Zip_f = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arq_zip;
      }
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida_label']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida_label'])
      { 
          $this->count_span = 0;
          $this->Xls_row++;
          $this->proc_label();
     ...
     $nm_houve_quebra = "N";
         ...
         if (isset($this->NM_Row_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida'])
         { 
      ...
      $this->xls_set_style();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida'] && $prim_reg)
      { 
          $this->proc_label();
          $this->xls_sub_cons_copy_label($this->Xls_row);
      ...
      if (isset($this->NM_Col_din) && !$_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida'])
      { 
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_totals()
   {
         ...
         if (is_numeric($this->totals))
         {
             $this->NM_ctrl_style[$current_cell_ref]['format'] = '#,##0';
         }
         $this->Nm_ActiveSheet->setCellValue($current_cell_ref . $this->Xls_row, $this->totals);
         $this->Xls_col++;
   }
   //----- id
   ...
   //----- totals
   function NM_sub_cons_totals()
   {
         if (!NM_is_utf8($this->totals))
         {
             $this->totals = sc_convert_encoding($this->totals, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['data']   = $this->totals;
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['align']  = "right";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['type']   = "num";
         $this->arr_export['lines'][$this->Xls_row][$this->Xls_col]['format'] = "#,##0";
         $this->Xls_col++;
   ...
   {
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['nolabel']) || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['nolabel'])
       {
   ...
   function quebra_geral_sc_free_total_bot()
   {
       ...
       $mens_tot_base = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['tot_geral'][0];
       $prim_lin_tot  = true;
     foreach ($NM_tipos_tot as $Tipo_Sum => $Tit_sum)
     {
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['field_order'] as $Cada_cmp)
       {
           if (isset($NM_cmp_tot['totals'][$Tipo_Sum]) && $Cada_cmp == "totals" && (!isset($this->NM_cmp_hidden['totals']) || $this->NM_cmp_hidden['totals'] != "off"))
           {
               ...
               $Vl_Tot = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['tot_geral'][$NM_cmp_tot['totals'][$Tipo_Sum]];
               $prim_cmp = false;
           ...
           elseif (!isset($this->NM_cmp_hidden[$Cada_cmp]) || $this->NM_cmp_hidden[$Cada_cmp] != "off")
           {
               ...
               elseif ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['embutida']) {
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_file']);
      if (is_file($this->Xls_f))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_file'] = $this->Xls_f;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - History Pembayaran Poli :: Excel</TITLE>
...
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbpayment']['xls_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
