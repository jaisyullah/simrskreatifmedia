...
class grid_tbruang_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_tbruang_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_tbruang_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_tbruang_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_idruang')
      {
          $idruang = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_idruang($idruang);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_namaruang')
      {
          $namaruang = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_namaruang($namaruang);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_idruang($idruang)
   {
      $idruang = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $idruang);
      $idruang = substr($this->Db->qstr($idruang), 1, -1);
            $idruang_look = substr($this->Db->qstr($idruang), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct idRuang from " . $this->Ini->nm_tabela . " where   CAST (idRuang AS TEXT)  like '%" . $idruang . "%' order by idRuang"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct idRuang from " . $this->Ini->nm_tabela . " where   CAST (idRuang AS VARCHAR)  like '%" . $idruang . "%' order by idRuang"; 
      }
      else
      {
          $nm_comando = "select distinct idRuang from " . $this->Ini->nm_tabela . " where  idRuang like '%" . $idruang . "%' order by idRuang"; 
      }
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_namaruang($namaruang)
   {
      $namaruang = substr($this->Db->qstr($namaruang), 1, -1);
            $namaruang_look = substr($this->Db->qstr($namaruang), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct namaRuang from " . $this->Ini->nm_tabela . " where  namaRuang like '%" . $namaruang . "%' order by namaRuang"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Manajemen Ruangan</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_tbruang/grid_tbruang_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_tbruang_ajax_search.js"></script>
 <script type="text/javascript" src="grid_tbruang_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_tbruang_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbruang']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbruang']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_idruang_cond#NMF#' + search_get_sel_txt('SC_idruang_cond') + '@NMF@';
  str_out += 'SC_idruang#NMF#' + search_get_text('SC_idruang') + '@NMF@';
  str_out += 'id_ac_idruang#NMF#' + search_get_text('id_ac_idruang') + '@NMF@';
  str_out += 'SC_namaruang_cond#NMF#' + search_get_sel_txt('SC_namaruang_cond') + '@NMF@';
  str_out += 'SC_namaruang#NMF#' + search_get_text('SC_namaruang') + '@NMF@';
  str_out += 'id_ac_namaruang#NMF#' + search_get_text('id_ac_namaruang') + '@NMF@';
   ...
   $("#id_ac_idruang").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          ...
          nmgp_parms: "NM_ajax_opcao?#?autocomp_idruang",
          max_itens: "10",
          cod_desc: "N",
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_idruang").val( $(this).val() );
       }
     }
   });
   $("#id_ac_namaruang").autocomplete({
     minLength: 1,
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_namaruang",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     select: function (event, ui) {
       $("#SC_namaruang").val(ui.item.value);
       $(this).val(ui.item.label);
       event.preventDefault();
     },
     focus: function (event, ui) {
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - Manajemen Ruangan</div>
...
<?php
   ...
   {
      global 
             $idruang_cond, $idruang, $idruang_autocomp,
             $namaruang_cond, $namaruang, $namaruang_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $idruang = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['idruang']; 
          $idruang_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['idruang_cond']; 
          $namaruang = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['namaruang']; 
          $namaruang_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['namaruang_cond']; 
          $kelas = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['kelas']; 
          $kelas_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['kelas_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['NM_operador']; 
      } 
      if (!isset($idruang_cond) || empty($idruang_cond))
      {
         $idruang_cond = "gt";
      }
      if (!isset($namaruang_cond) || empty($namaruang_cond))
      {
         $namaruang_cond = "qp";
      }
      ...
      $str_hide_idruang = (in_array($idruang_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_namaruang = (in_array($namaruang_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_kelas = (in_array($kelas_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      if (!isset($idruang) || $idruang == "")
      {
          $idruang = "";
      }
      if (isset($idruang) && !empty($idruang))
      {
         $tmp_pos = strpos($idruang, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $idruang = substr($idruang, 0, $tmp_pos);
         }
      }
      if (!isset($namaruang) || $namaruang == "")
      {
          $namaruang = "";
      }
      if (isset($namaruang) && !empty($namaruang))
      {
         $tmp_pos = strpos($namaruang, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $namaruang = substr($namaruang, 0, $tmp_pos);
         }
      }
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD id='SC_idruang_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['idruang'])) ? $this->New_label['idruang'] : "ID Ruang"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_idruang_cond" name="idruang_cond" onChange="nm_campos_between(document.getElementById('id_vis_idruang'), this, 'idruang')">
       <OPTION value="gt" <?php if ("gt" == $idruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $idruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $idruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_idruang" <?php echo $str_hide_idruang?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['idruang'])) ? $this->New_label['idruang'] : "ID Ruang";
 $nmgp_tab_label .= "idruang?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
      ...
      $idruang_look = substr($this->Db->qstr($idruang), 1, -1); 
      $nmgp_def_dados = array(); 
   if (is_numeric($idruang))
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct idRuang from " . $this->Ini->nm_tabela . " where idRuang = $idruang_look order by idRuang"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct idRuang from " . $this->Ini->nm_tabela . " where idRuang = $idruang_look order by idRuang"; 
      ...
      if (isset($nmgp_def_dados[0][$idruang]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$idruang];
      }
      else
      {
            nmgp_Form_Num_Val($idruang, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $idruang;
      }
?>
<INPUT  type="text" id="SC_idruang" name="idruang" value="<?php echo NM_encode_input($idruang) ?>" size=3 alt="{datatype: 'integer', maxLength: 3, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_idruang" name="idruang_autocomp" size="3"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 3, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_namaruang_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['namaruang'])) ? $this->New_label['namaruang'] : "Nama Ruang"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_namaruang_cond" name="namaruang_cond" onChange="nm_campos_between(document.getElementById('id_vis_namaruang'), this, 'namaruang')">
       <OPTION value="qp" <?php if ("qp" == $namaruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $namaruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $namaruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $namaruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_namaruang" <?php echo $str_hide_namaruang?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['namaruang'])) ? $this->New_label['namaruang'] : "Nama Ruang";
 $nmgp_tab_label .= "namaruang?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $namaruang_look = substr($this->Db->qstr($namaruang), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct namaRuang from " . $this->Ini->nm_tabela . " where namaRuang = '$namaruang_look' order by namaRuang"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$namaruang]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$namaruang];
      }
      else
      {
          $sAutocompValue = $namaruang;
      }
?>
<INPUT  type="text" id="SC_namaruang" name="namaruang" value="<?php echo NM_encode_input($namaruang) ?>"  size=25 alt="{datatype: 'text', maxLength: 25, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_namaruang" name="namaruang_autocomp" size="25"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 25, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<?php
   if (is_file("grid_tbruang_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tbruang']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tbruang']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.idruang_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_idruang'), document.F1.idruang_cond, 'idruang');
   document.F1.idruang.value = "";
   document.F1.idruang_autocomp.value = "";
   document.F1.namaruang_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_namaruang'), document.F1.namaruang_cond, 'namaruang');
   document.F1.namaruang.value = "";
   document.F1.namaruang_autocomp.value = "";
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_tbruang";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_idruang_cond'] = 'cond';
      $tp_fields['SC_idruang'] = 'text_aut';
      $tp_fields['id_ac_idruang'] = 'text_aut';
      $tp_fields['SC_namaruang_cond'] = 'cond';
      $tp_fields['SC_namaruang'] = 'text_aut';
      $tp_fields['id_ac_namaruang'] = 'text_aut';
   ...
   {
      global $idruang_cond, $idruang, $idruang_autocomp,
             $namaruang_cond, $namaruang, $namaruang_autocomp,
      ...
      if (!empty($idruang_autocomp) && empty($idruang))
      {
          $idruang = $idruang_autocomp;
      }
      if (!empty($namaruang_autocomp) && empty($namaruang))
      {
          $namaruang = $namaruang_autocomp;
      }
      if (!empty($kelas_autocomp) && empty($kelas))
      {
          $kelas = $kelas_autocomp;
      }
      $idruang_cond_salva = $idruang_cond; 
      if (!isset($idruang_input_2) || $idruang_input_2 == "")
      {
          $idruang_input_2 = $idruang;
      }
      $namaruang_cond_salva = $namaruang_cond; 
      if (!isset($namaruang_input_2) || $namaruang_input_2 == "")
      {
          $namaruang_input_2 = $namaruang;
      }
      ...
      if ($idruang_cond != "in")
      {
          nm_limpa_numero($idruang, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['idruang'] = $idruang; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['idruang_cond'] = $idruang_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['namaruang'] = $namaruang; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['namaruang_cond'] = $namaruang_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['kelas'] = $kelas; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['kelas_cond'] = $kelas_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca'] = $Temp_Busca;
      }
      if ($idruang_cond != "in" && $idruang_cond != "bw" && !empty($idruang) && !is_numeric($idruang))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : ID Ruang";
      }
      if ($idruang_cond == "bw" && ((!empty($idruang) && !is_numeric($idruang)) || (!empty($idruang_input_2) && !is_numeric($idruang_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : ID Ruang";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['idruang'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['idruang'] = $cmp1;
      }
      else
      {
          $cmp1 = $idruang;
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['namaruang'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['namaruang'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['namaruang'] = $namaruang;
      }
      $kelas_look = substr($this->Db->qstr($kelas), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct kelas from " . $this->Ini->nm_tabela . " where kelas = '$kelas_look' order by kelas"; 
      ...
      if (isset($idruang) || $idruang_cond == "nu" || $idruang_cond == "nn" || $idruang_cond == "ep" || $idruang_cond == "ne")
      {
         $this->monta_condicao("idRuang", $idruang_cond, $idruang, "", "idruang");
      }

      //----- $namaruang
      $this->Date_part = false;
      if (isset($namaruang) || $namaruang_cond == "nu" || $namaruang_cond == "nn" || $namaruang_cond == "ep" || $namaruang_cond == "ne")
      {
         $this->monta_condicao("namaRuang", $namaruang_cond, $namaruang, "", "namaruang");
      }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbruang']['cond_pesq'] .= $this->NM_operador;
