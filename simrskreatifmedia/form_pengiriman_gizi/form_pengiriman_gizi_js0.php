...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_pengiriman_gizi']['parms']); ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
  <input type="hidden" name="script_case_session" value="<?php echo $this->form_encode_input(session_id()); ?>"> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_pengiriman_gizi"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
var var_btn_sticker_pasien_id_pengiriman = "<?php echo $this->form_encode_input($this->nmgp_dados_form['id']); ?>";
function sc_btn_sticker_pasien()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_sticker_pasien(); }, 500);
      return;
    }
    sc_btn_sticker_pasien_ok();
}
function sc_btn_sticker_pasien_cancel()
{
}
function sc_btn_sticker_pasien_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_cetak_sticker_batch_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "id_pengiriman*scin' + var_btn_sticker_pasien_id_pengiriman + '*scout"; ?>', 'inicio', '_self', '0', '0', 'cetak_sticker_batch');
}
...
{
  nm_gp_submit('<?php echo $this->Ini->link_get_list_pasien_ranap_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo ""; ?>', '', '_self', '0', '0', 'get_list_pasien_ranap');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_pengiriman_gizi']['where_detal']))
  {
?>  
