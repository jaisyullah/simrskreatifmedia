...
class chart_detail_dokter_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_detail_dokter']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_detail_dokter']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['where_pesq']; 
      } 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['contr_total_geral'] = "OK";
   } 

   //-----  regdate
   function quebra_regdate_sc_free_group_by($regdate, $Where_qb, $Cmp_Name) 
   {
   ...
   //-----  doctor
   function quebra_doctor_sc_free_group_by($doctor, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_doctor = array() ;  
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('regdate' => "regDate",'doctor' => "doctor");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      $cmp_sql_tp_num = array('queue' => 'N','viaphone' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_detail_dokter']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['regdate']['reg'] = "N";
      $format_dimensions['regdate']['msk'] = "F/Y";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "doctor") {
                  $this->Lookup->lookup_sc_free_group_by_doctor($$Cada_dim,  $doctor);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
