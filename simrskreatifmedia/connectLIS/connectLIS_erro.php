...
$old_error_handler = set_error_handler("connectLIS_scriptcase_error_handler");

function connectLIS_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['connectLIS']['contr_erro']) && $_SESSION['scriptcase']['connectLIS']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_connectLIS->contr_connectLIS->Apl_com_erro = true;
            if ($inicial_connectLIS->contr_connectLIS->NM_ajax_flag)
            {
                $inicial_connectLIS->contr_connectLIS->NM_ajax_info['errList']['geral_connectLIS'][] = $msg;
                return;
            }
            connectLIS_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function connectLIS_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['connectLIS'])  && '' != $_SESSION['scriptcase']['error_icon']['connectLIS'])  ? $_SESSION['scriptcase']['error_icon']['connectLIS']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['connectLIS']) && '' != $_SESSION['scriptcase']['error_close']['connectLIS']) ? $_SESSION['scriptcase']['error_close']['connectLIS'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['connectLIS']) && '' != $_SESSION['scriptcase']['error_span_title']['connectLIS']) ? $_SESSION['scriptcase']['error_span_title']['connectLIS'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['connectLIS']) && '' != $_SESSION['scriptcase']['error_icon_title']['connectLIS']) ? $_SESSION['scriptcase']['error_icon_title']['connectLIS'] : '';
?>
