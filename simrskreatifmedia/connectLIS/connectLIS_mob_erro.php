...
$old_error_handler = set_error_handler("connectLIS_mob_scriptcase_error_handler");

function connectLIS_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['connectLIS_mob']['contr_erro']) && $_SESSION['scriptcase']['connectLIS_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_connectLIS_mob->contr_connectLIS_mob->Apl_com_erro = true;
            if ($inicial_connectLIS_mob->contr_connectLIS_mob->NM_ajax_flag)
            {
                $inicial_connectLIS_mob->contr_connectLIS_mob->NM_ajax_info['errList']['geral_connectLIS_mob'][] = $msg;
                return;
            }
            connectLIS_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function connectLIS_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['connectLIS_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['connectLIS_mob'])  ? $_SESSION['scriptcase']['error_icon']['connectLIS_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['connectLIS_mob']) && '' != $_SESSION['scriptcase']['error_close']['connectLIS_mob']) ? $_SESSION['scriptcase']['error_close']['connectLIS_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['connectLIS_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['connectLIS_mob']) ? $_SESSION['scriptcase']['error_span_title']['connectLIS_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['connectLIS_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['connectLIS_mob']) ? $_SESSION['scriptcase']['error_icon_title']['connectLIS_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['connectLIS']['error_buffer'] .= ob_get_flush();
}
