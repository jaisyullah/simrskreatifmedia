...
class kartu_stok_xml
{
   ...
   function inicializa_vars()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['embutida'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_label']))
      {
          $this->Xml_tag_label = $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_label'];
          $this->New_Format    = true;
      }
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_return']);
          if ($this->Tem_xml_res) {
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<kartu_stok>\r\n";
         }
         else
         {
             $this->xml_registro = "<kartu_stok";
      ...
      { 
          ...
          { 
              ...
              $this->Res = new kartu_stok_res_xml();
              $this->prep_modulos("Res");
              $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_res_grid'] = true;
              $this->Res->monta_xml();
          } 
          if (!$this->Ini->sc_export_ajax) {
          ...
          { 
              ...
              { 
                  $str_zip   = "";
                  $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_res_file']['xml'];
                  ...
                  unlink($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_res_file']['xml']);
              }
              if ($this->Grava_view)
              {
                  $str_zip    = "";
                  ...
                  { 
                      $str_zip   = "";
                      $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_res_file']['view'];
                      $Arq_input = (FALSE !== strpos($Arq_res, ' ')) ? " \"" . $Arq_res . "\"" :  $Arq_res;
                      if (FALSE !== strpos(strtolower(php_uname()), 'windows')) 
                      {
                          $str_zip = "zip.exe " . strtoupper($Parm_pass) . " -j -u " . $this->Xml_password . " " . $Zip_f . " " . $Arq_input;
                      ...
                      unlink($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_res_file']['view']);
                  }
              } 
              else 
              {
              ...
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_res_grid']);
          } 
      }
   ...
   function NM_export_kadaluarsa()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->kadaluarsa) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->kadaluarsa) . "\"";
         }
   }
   //----- transaksi
   function NM_export_transaksi()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->transaksi) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->transaksi) . "\"";
         }
   }
   //----- trancode
   ...
   function NM_export_kumulatif()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->kumulatif) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->kumulatif) . "\"";
         }
   }
   //----- cumulatif
   function NM_export_cumulatif()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->cumulatif) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->cumulatif) . "\"";
         }
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE>Kartu Stok Farmasi :: XML</TITLE>
...
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['kartu_stok']['xml_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
