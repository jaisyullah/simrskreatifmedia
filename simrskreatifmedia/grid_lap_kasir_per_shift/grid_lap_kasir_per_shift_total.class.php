...
class grid_lap_kasir_per_shift_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_lap_kasir_per_shift']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_lap_kasir_per_shift']['campos_busca']))
      { 
   ...
   function quebra_geral_Jenis($res_limit=false)
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['contr_total_geral'] = "OK";
   } 

   //-----  jenis
   function quebra_jenis_Jenis($jenis, $arg_sum_jenis) 
   {
      global $tot_jenis, $custcode;
      $tot_jenis = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(Nominal) from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq']; 
      ...
      { 
         $nm_comando .= " where Jenis" . $arg_sum_jenis ; 
      } 
      else 
      { 
         $nm_comando .= " and Jenis" . $arg_sum_jenis ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_jenis[0] = sc_strip_script($jenis) ; 
      $tot_jenis[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_jenis[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 

   //-----  kasir
   function quebra_kasir_Jenis($jenis, $kasir, $arg_sum_jenis, $arg_sum_kasir) 
   {
      global $tot_kasir, $custcode;
      $tot_kasir = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(Nominal) from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq'])) 
      { 
         $nm_comando .= " where Jenis" . $arg_sum_jenis . " and kasir" . $arg_sum_kasir ; 
      } 
      else 
      { 
         $nm_comando .= " and Jenis" . $arg_sum_jenis . " and kasir" . $arg_sum_kasir ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_kasir[0] = sc_strip_script($kasir) ; 
      $tot_kasir[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_kasir[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 

   //-----  paiddate
   function quebra_paiddate_Jenis($jenis, $kasir, $paiddate, $arg_sum_jenis, $arg_sum_kasir, $arg_sum_paiddate) 
   {
      global $tot_paiddate, $custcode;
      $tot_paiddate = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(Nominal) from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq'])) 
      { 
         $nm_comando .= " where Jenis" . $arg_sum_jenis . " and kasir" . $arg_sum_kasir . " and " .  $this->Sc_groupby_paiddate . $arg_sum_paiddate ; 
      } 
      else 
      { 
         $nm_comando .= " and Jenis" . $arg_sum_jenis . " and kasir" . $arg_sum_kasir . " and " .  $this->Sc_groupby_paiddate . $arg_sum_paiddate ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_paiddate[0] = NM_encode_input(sc_strip_script($paiddate)) ; 
      $tot_paiddate[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_paiddate[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 

   //-----  custcode
   function quebra_custcode_Jenis($jenis, $kasir, $paiddate, $custcode, $arg_sum_jenis, $arg_sum_kasir, $arg_sum_paiddate, $arg_sum_custcode) 
   {
      global $tot_custcode, $custcode;
      $tot_custcode = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(Nominal) from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['where_pesq'])) 
      { 
         $nm_comando .= " where Jenis" . $arg_sum_jenis . " and kasir" . $arg_sum_kasir . " and " .  $this->Sc_groupby_paiddate . $arg_sum_paiddate . " and custCode" . $arg_sum_custcode ; 
      } 
      else 
      { 
         $nm_comando .= " and Jenis" . $arg_sum_jenis . " and kasir" . $arg_sum_kasir . " and " .  $this->Sc_groupby_paiddate . $arg_sum_paiddate . " and custCode" . $arg_sum_custcode ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_custcode[0] = sc_strip_script($custcode) ; 
      $tot_custcode[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_custcode[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
   } 


   //----- 
   function Calc_resumo_Jenis($destino_resumo)
   {
      global $nm_lang, $custcode;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('jenis' => "Jenis",'kasir' => "kasir",'paiddate' => "paidDate",'custcode' => "custCode");
      $cmps_quebra_atual = array("jenis","kasir","paiddate","custcode");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_Jenis($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_Jenis($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('nominal' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(Nominal)#@#cmps_quebras#@# from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select Nominal as SC_metric1, " . $cmps_quebras1 . " from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	c.custCode, IF 	( 		c.deposit > 0, 		c.deposit, 	IF 		( c.jmlTagihan <= c.jmlBayar, c.jmlTagihan, c.jmlBayar )  	) AS Nominal, IF 	( 		c.deposit > 0, 		'DEPOSIT', 		( 		IF 			( c.jenisPayment LIKE \"KARTU%\", concat( ' EDC : ', COALESCE ( c.edc1, '' ) ), c.jenisPayment )  		)  	) AS Jenis, 	c.kasir,         c.paidDate, 	c.tranCode, 	c.noPayment FROM 	( 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Poli' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'IGD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_igd UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		IF 		( hrsDibayar > 0, ( jmlBayar - hrsDibayar ), jmlBayar ) AS jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Rawat Inap' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_ri         WHERE 		( jmlBayar != '0' OR deposit != '0' ) UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'LAB' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_lab UNION ALL 	SELECT 		custCode, 		jmlTagihan, 		jmlBayar, 		deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'RAD' AS Kategori,                 paidDate, 		tranCode, 		noPayment  	FROM 		tbpayment_rad UNION ALL 	SELECT                 custCode, 		jmlTagihan, 		jmlBayar, 		0 AS deposit, 		tranDate, 		jenisPayment, 		edc1, 		kasir, 		'Obat' AS Kategori,                 paidDate, 		tranCode, 		noPayment 	FROM 		tbpayment_obat UNION ALL 	SELECT 		custcode AS custCode, 		total AS jmlTagihan, 		total AS jmlBayar, 		0 AS deposit, 		trandate AS tranDate, 		jenisPayment,                 edc1, 		kasir, 		'Lain-lain' AS Kategori,                 trandate as paidDate, 		tranCode, 		tranCode as noPayment  	FROM 		tbpendapatanlain  	) c  WHERE 	(c.jmlBayar != '0' OR c.deposit != '0')) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_lap_kasir_per_shift']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['jenis']['reg'] = "S";
      $format_dimensions['jenis']['msk'] = "";
      $format_dimensions['kasir']['reg'] = "S";
      $format_dimensions['kasir']['msk'] = "";
      $format_dimensions['paiddate']['reg'] = "S";
      $format_dimensions['paiddate']['msk'] = "";
      $format_dimensions['custcode']['reg'] = "S";
      $format_dimensions['custcode']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'Jenis', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              ...
              if ($Cada_dim == "custcode") {
                  $this->Lookup->lookup_Jenis_custcode($$Cada_dim,  $custcode);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_Jenis($where_ok);
