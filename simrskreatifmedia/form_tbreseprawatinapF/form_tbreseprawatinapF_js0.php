...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbreseprawatinapF"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
<?php
  if (isset($this->NM_ajax_info['masterValue']) && !empty($this->NM_ajax_info['masterValue']))
  {
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbreseprawatinapF']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbreseprawatinapF']['dashboard_info']['under_dashboard']) {
          echo "  var dbParentFrame = $(parent.document).find(\"[name='" . $_SESSION['sc_session'][$this->Ini->sc_page]['form_tbreseprawatinapF']['dashboard_info']['parent_widget'] . "']\")";
...
{
  nm_gp_submit('<?php echo $this->Ini->link_ambilFormula_edit; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_v_trancode*scinv_tranCode*scoutNM_btn_insert*scinN*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scout"; ?>', '', '_self', '0', '0', 'ambilFormula');
}
 NM_tp_critica(1);
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbreseprawatinapF']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tbreseprawatinapF_submit_form(y, z);
...
{
   nm_field_disabled("biaya_=disabled", "");
