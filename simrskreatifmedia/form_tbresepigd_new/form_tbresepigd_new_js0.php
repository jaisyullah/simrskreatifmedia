...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_tbresepigd_new"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
{
  nm_gp_submit('<?php echo $this->Ini->link_obat_igd; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "SC_glo_par_struknya*scinv_tranCode*scout"; ?>', '', '_blank', '0', '0', 'obat_igd');
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_tbresepigd_new']['where_detal']))
  {
?>  
...
function scForm_inline_submit(y, z) {
	Nm_Proc_Atualiz = true;
	do_ajax_form_tbresepigd_new_submit_form(y, z);
