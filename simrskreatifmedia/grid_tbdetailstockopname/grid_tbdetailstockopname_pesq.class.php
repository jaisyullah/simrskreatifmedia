...
class grid_tbdetailstockopname_pesq
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_tbdetailstockopname_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_tbdetailstockopname_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_tbdetailstockopname_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_stokawal')
      {
          $stokawal = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_stokawal($stokawal);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_socode')
      {
          $socode = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_socode($socode);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_stokawal($stokawal)
   {
      $stokawal = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $stokawal);
      $stokawal = substr($this->Db->qstr($stokawal), 1, -1);
            $stokawal_look = substr($this->Db->qstr($stokawal), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct stokAwal from " . $this->Ini->nm_tabela . " where   CAST (stokAwal AS TEXT)  like '%" . $stokawal . "%' order by stokAwal"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct stokAwal from " . $this->Ini->nm_tabela . " where   CAST (stokAwal AS VARCHAR)  like '%" . $stokawal . "%' order by stokAwal"; 
      }
      else
      {
          $nm_comando = "select distinct stokAwal from " . $this->Ini->nm_tabela . " where  stokAwal like '%" . $stokawal . "%' order by stokAwal"; 
      }
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_socode($socode)
   {
      $socode = substr($this->Db->qstr($socode), 1, -1);
            $socode_look = substr($this->Db->qstr($socode), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct soCode from " . $this->Ini->nm_tabela . " where  soCode like '%" . $socode . "%' order by soCode"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function monta_condicao($nome, $condicao, $campo, $campo2 = "", $nome_campo="", $tp_campo="")
   {
      ...
      else
      {
         ...
         $nome_sum = "tbdetailstockopname.$nome";
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - tbdetailstockopname</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_tbdetailstockopname/grid_tbdetailstockopname_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_tbdetailstockopname_ajax_search.js"></script>
 <script type="text/javascript" src="grid_tbdetailstockopname_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_tbdetailstockopname_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbdetailstockopname']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbdetailstockopname']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_stokawal_cond#NMF#' + search_get_sel_txt('SC_stokawal_cond') + '@NMF@';
  str_out += 'SC_stokawal#NMF#' + search_get_text('SC_stokawal') + '@NMF@';
  str_out += 'id_ac_stokawal#NMF#' + search_get_text('id_ac_stokawal') + '@NMF@';
  ...
  str_out += 'SC_socode_cond#NMF#' + search_get_sel_txt('SC_socode_cond') + '@NMF@';
  str_out += 'SC_socode#NMF#' + search_get_text('SC_socode') + '@NMF@';
  str_out += 'id_ac_socode#NMF#' + search_get_text('id_ac_socode') + '@NMF@';
   ...
   $("#id_ac_socode").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_socode",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
   ...
   $("#id_ac_stokawal").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_stokawal",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - tbdetailstockopname</div>
...
<?php
   ...
   {
      global 
             $stokawal_cond, $stokawal, $stokawal_autocomp,
             $id_cond, $id, $id_autocomp,
             $socode_cond, $socode, $socode_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $stokawal = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['stokawal']; 
          $stokawal_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['stokawal_cond']; 
          $id = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['id']; 
          $id_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['id_cond']; 
          $socode = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['socode']; 
          $socode_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['socode_cond']; 
          $namaitem = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['namaitem']; 
          $namaitem_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['namaitem_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['NM_operador']; 
      } 
      if (!isset($stokawal_cond) || empty($stokawal_cond))
      {
         $stokawal_cond = "gt";
      }
      ...
      if (!isset($socode_cond) || empty($socode_cond))
      {
         $socode_cond = "qp";
      }
      ...
      $str_hide_stokawal = (in_array($stokawal_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_id = (in_array($id_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_socode = (in_array($socode_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_namaitem = (in_array($namaitem_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      if (!isset($stokawal) || $stokawal == "")
      {
          $stokawal = "";
      }
      if (isset($stokawal) && !empty($stokawal))
      {
         $tmp_pos = strpos($stokawal, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $stokawal = substr($stokawal, 0, $tmp_pos);
         }
      }
      if (!isset($id) || $id == "")
      {
      ...
      if (!isset($socode) || $socode == "")
      {
          $socode = "";
      }
      if (isset($socode) && !empty($socode))
      {
         $tmp_pos = strpos($socode, "##@@");
         if ($tmp_pos === false)
         { }
         else
         {
         $socode = substr($socode, 0, $tmp_pos);
         }
      }
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr>
      ...
      <TD id='SC_stokawal_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['stokawal'])) ? $this->New_label['stokawal'] : "Stok Awal"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_stokawal_cond" name="stokawal_cond" onChange="nm_campos_between(document.getElementById('id_vis_stokawal'), this, 'stokawal')">
       <OPTION value="gt" <?php if ("gt" == $stokawal_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $stokawal_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $stokawal_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_stokawal" <?php echo $str_hide_stokawal?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['stokawal'])) ? $this->New_label['stokawal'] : "Stok Awal";
 $nmgp_tab_label .= "stokawal?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct stokAwal from " . $this->Ini->nm_tabela . " where stokAwal = $stokawal_look order by stokAwal"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct stokAwal from " . $this->Ini->nm_tabela . " where stokAwal = $stokawal_look order by stokAwal"; 
      ...
      if (isset($nmgp_def_dados[0][$stokawal]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$stokawal];
      }
      else
      {
            nmgp_Form_Num_Val($stokawal, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $stokawal;
      }
?>
<INPUT  type="text" id="SC_stokawal" name="stokawal" value="<?php echo NM_encode_input($stokawal) ?>" size=4 alt="{datatype: 'integer', maxLength: 4, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_stokawal" name="stokawal_autocomp" size="4"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 4, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
...
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_id" name="id_autocomp" size="5"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 5, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_socode_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['socode'])) ? $this->New_label['socode'] : "So Code"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_socode_cond" name="socode_cond" onChange="nm_campos_between(document.getElementById('id_vis_socode'), this, 'socode')">
       <OPTION value="qp" <?php if ("qp" == $socode_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $socode_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $socode_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $socode_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_socode" <?php echo $str_hide_socode?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['socode'])) ? $this->New_label['socode'] : "So Code";
 $nmgp_tab_label .= "socode?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $socode_look = substr($this->Db->qstr($socode), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct soCode from " . $this->Ini->nm_tabela . " where soCode = '$socode_look' order by soCode"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$socode]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$socode];
      }
      else
      {
          $sAutocompValue = $socode;
      }
?>
<INPUT  type="text" id="SC_socode" name="socode" value="<?php echo NM_encode_input($socode) ?>"  size=15 alt="{datatype: 'text', maxLength: 15, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_socode" name="socode_autocomp" size="15"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 15, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<INPUT  type="text" id="SC_namaitem" name="namaitem" value="<?php echo NM_encode_input($namaitem) ?>"  size=15 alt="{datatype: 'text', maxLength: 15, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_namaitem" name="namaitem_autocomp" size="15"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 15, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<?php
   if (is_file("grid_tbdetailstockopname_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tbdetailstockopname']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tbdetailstockopname']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.stokawal_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_stokawal'), document.F1.stokawal_cond, 'stokawal');
   document.F1.stokawal.value = "";
   document.F1.stokawal_autocomp.value = "";
   ...
   document.F1.socode_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_socode'), document.F1.socode_cond, 'socode');
   document.F1.socode.value = "";
   document.F1.socode_autocomp.value = "";
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_tbdetailstockopname";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function salva_filtro($nmgp_save_origem)
   {
      global $NM_filters_save, $nmgp_save_name, $nmgp_save_option, $script_case_init;
          ...
          $NM_patch .= "grid_tbdetailstockopname/";
          if (!is_dir($NM_patch))
          {
              $NMdir = mkdir($NM_patch, 0755);
          }
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_stokawal_cond'] = 'cond';
      $tp_fields['SC_stokawal'] = 'text_aut';
      $tp_fields['id_ac_stokawal'] = 'text_aut';
      $tp_fields['SC_id_cond'] = 'cond';
      $tp_fields['SC_id'] = 'text_aut';
      $tp_fields['id_ac_id'] = 'text_aut';
      $tp_fields['SC_socode_cond'] = 'cond';
      $tp_fields['SC_socode'] = 'text_aut';
      $tp_fields['id_ac_socode'] = 'text_aut';
   ...
   {
      global $stokawal_cond, $stokawal, $stokawal_autocomp,
             $id_cond, $id, $id_autocomp,
             $socode_cond, $socode, $socode_autocomp,
             $namaitem_cond, $namaitem, $namaitem_autocomp, $nmgp_tab_label;

      $C_formatado = true;
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($stokawal_autocomp) && empty($stokawal))
      {
          $stokawal = $stokawal_autocomp;
      }
      if (!empty($id_autocomp) && empty($id))
      {
          $id = $id_autocomp;
      }
      if (!empty($socode_autocomp) && empty($socode))
      {
          $socode = $socode_autocomp;
      }
      if (!empty($namaitem_autocomp) && empty($namaitem))
      {
          $namaitem = $namaitem_autocomp;
      }
      $stokawal_cond_salva = $stokawal_cond; 
      if (!isset($stokawal_input_2) || $stokawal_input_2 == "")
      {
          $stokawal_input_2 = $stokawal;
      }
      ...
      $socode_cond_salva = $socode_cond; 
      if (!isset($socode_input_2) || $socode_input_2 == "")
      {
          $socode_input_2 = $socode;
      }
      ...
      if ($stokawal_cond != "in")
      {
          nm_limpa_numero($stokawal, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      if ($id_cond != "in")
      {
          nm_limpa_numero($id, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['stokawal'] = $stokawal; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['stokawal_cond'] = $stokawal_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['id'] = $id; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['id_cond'] = $id_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['socode'] = $socode; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['socode_cond'] = $socode_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['namaitem'] = $namaitem; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['namaitem_cond'] = $namaitem_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca'] = $Temp_Busca;
      }
      if ($stokawal_cond != "in" && $stokawal_cond != "bw" && !empty($stokawal) && !is_numeric($stokawal))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Stok Awal";
      }
      if ($stokawal_cond == "bw" && ((!empty($stokawal) && !is_numeric($stokawal)) || (!empty($stokawal_input_2) && !is_numeric($stokawal_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Stok Awal";
      }
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['stokawal'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['stokawal'] = $cmp1;
      }
      else
      {
          $cmp1 = $stokawal;
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['socode'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['socode'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['socode'] = $socode;
      }
      $namaitem_look = substr($this->Db->qstr($namaitem), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct namaItem from " . $this->Ini->nm_tabela . " where namaItem = '$namaitem_look' order by namaItem"; 
      ...
      if (isset($stokawal) || $stokawal_cond == "nu" || $stokawal_cond == "nn" || $stokawal_cond == "ep" || $stokawal_cond == "ne")
      {
         $this->monta_condicao("stokAwal", $stokawal_cond, $stokawal, "", "stokawal");
      }
      ...
      //----- $socode
      $this->Date_part = false;
      if (isset($socode) || $socode_cond == "nu" || $socode_cond == "nn" || $socode_cond == "ep" || $socode_cond == "ne")
      {
         $this->monta_condicao("soCode", $socode_cond, $socode, "", "socode");
      }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbdetailstockopname']['cond_pesq'] .= $this->NM_operador;
