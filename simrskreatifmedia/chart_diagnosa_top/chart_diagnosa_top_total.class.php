...
class chart_diagnosa_top_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_diagnosa_top']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_diagnosa_top']['campos_busca']))
      { 
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*) from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_total_geral'] = "OK";
   } 

   //-----  icd1
   function quebra_icd1_sc_free_group_by($icd1, $Where_qb, $Cmp_Name) 
   {
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo)
   {
      global $nm_lang;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('icd1' => "icd1");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      { 
         $cmd_simp = "select count(*)#@#cmps_quebras#@# from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') sc_sel_esp " . $where_ok;
         $comando  = "select count(*)#@#cmps_quebras#@# from (";
         $comando .= "select " . $cmps_quebras1 . " from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['icd1']['reg'] = "S";
      $format_dimensions['icd1']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "icd1") {
                  $this->Lookup->lookup_sc_free_group_by_icd1($$Cada_dim,  $icd1);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
