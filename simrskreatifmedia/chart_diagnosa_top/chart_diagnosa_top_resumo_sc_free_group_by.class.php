...
class chart_diagnosa_top_resumo
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['under_dashboard'] ? "off" : "on";

      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['under_dashboard'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['maximized']) {
          $tmpDashboardApp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['dashboard_app'];
          if (isset($_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_diagnosa_top'])) {
              $tmpDashboardButtons = $_SESSION['scriptcase']['dashboard_toolbar'][$tmpDashboardApp]['chart_diagnosa_top'];
      ...
      if (isset($_SESSION['scriptcase']['sc_apl_conf']['chart_diagnosa_top']['btn_display']) && !empty($_SESSION['scriptcase']['sc_apl_conf']['chart_diagnosa_top']['btn_display']))
      {
          foreach ($_SESSION['scriptcase']['sc_apl_conf']['chart_diagnosa_top']['btn_display'] as $NM_cada_btn => $NM_cada_opc)
          {
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_step']     = 'image';
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_step'] = 'end';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['chart_diagnosa_top']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
      ...
      $this->sc_where_orig   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_orig'];
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_pesq'];
      $this->sc_where_filtro = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_pesq_filtro'];
     if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['campos_busca']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['campos_busca']))
     { 
         $Busca_temp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['campos_busca'];
       ...
       if ($tmp_pos !== false && !is_array($this->icd1))
       {
         ...
         {
             ob_start();
             $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_combination_type'] = $_POST['fc_chart_type'];
         }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys']))
       {
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_x_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys']             = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_order'] = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['comb_field_display_type']  = array();
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_display_values']     = array();

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_combination_type']        = 'bar2d';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_combination_order_rule']  = 'desc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_combination_order_field'] = '1';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_combination_order_rule_groupby']["icd1"]  = 'desc';
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_combination_order_field_groupby']["icd1"] = '1';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys'][0] = 0;
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['comb_field_display_type'][2] = '';

           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_display_values'][2] = null;


           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_order']['sc_free_group_by'][] = 2;
       }

      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pesq_tab_label']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pesq_tab_label'] = "";
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pesq_tab_label'] .= "icd1?#?" . "Icd 1" . "?@?";
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_search_add']))
      {
          $this->monta_css();
          $nmgp_tab_label = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pesq_tab_label'];
          ...
          $Seq_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_search_add']['seq'];
          $Cmp_grid      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_search_add']['cmp'];
          ...
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_search_add']);
          exit;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dyn_search_aut_comp']))
      {
          ob_start();
          $NM_func_aut_comp = "lookup_ajax_" . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dyn_search_aut_comp']['cmp'];
          ...
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dyn_search_aut_comp']);
          exit;
      }
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['res_hrz'];
      if (isset($_SESSION['scriptcase']['sc_aba_iframe']) && !$this->NM_export)
      {
          foreach ($_SESSION['scriptcase']['sc_aba_iframe'] as $aba => $apls_aba)
          {
              if (in_array("chart_diagnosa_top", $apls_aba))
              {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['iframe_menu'] && !$this->NM_export && (!isset($_SESSION['scriptcase']['menu_mobile']) || empty($_SESSION['scriptcase']['menu_mobile'])))
      {
          $this->aba_iframe = true;
      }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_total_geral'] == "OK")
      {
          foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
          {
              $Arr_tot_name = "array_total_" . $cmp_gb;
              $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_total'][$cmp_gb];
          }
      }
      else
      {
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['tot_geral'][1];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['tot_geral'][1] == 0)
      {
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_total']['icd1'] = $this->array_total_icd1;
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['json_use_label'])
       {
          $Lab_icd1 = "icd1";
       }
       else
       ...
       $prep_field = array();
       $prep_field['icd1'] = $Lab_icd1;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['labels']['icd1'] = $Lab_icd1; 
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
          $Str_gb .= ($Str_gb == "") ? "" : ",";
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['labels']['icd1']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['labels']['icd1'] = "Icd 1"; 
       }
       //-----
       $ix = 0;
       $this->comp_field_nm = array();
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_order']['sc_free_group_by']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_order']['sc_free_group_by'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_order']['sc_free_group_by'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'] as $i_sum => $d_sum)
           {
      ...
      eval ($Str_gb);
       ...
       $Arr_parms = array();
       $Arr_parms['icd1']['alin'] = "''";
       $Arr_parms['icd1']['link'] = "S";
       $Arr_parms['icd1']['fill'] = "true";
       $Arr_parms['icd1']['order'] = "label";
       $Arr_parms['icd1']['order_start'] = 'asc';
       $Arr_parms['icd1']['order_invert'] = 'false';
       $Arr_parms['icd1']['order_enabled'] = 'true';
       //-----
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
       {
       ...
       //-----
       $prep_links_fl = array();
       $prep_links_fl['icd1'] = array(0 => 'icd1', 1 => '@aspass@');
       $Str_gb = "";
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_cmp'] as $cmp_gb => $col_sql)
       {
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_col'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_level'] = 0;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_sort'] = '';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_col']  = 0;
           }
           ...
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_tabular_label'];
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_diagnosa_top@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function makeTabular(&$a_data)
   {
       ...
       {
           $a_labels = array();
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['proc_pdf_vert'])
   {
     $this->comp_tab_hover = true;
   }
   ...
   function buildChart()
   {
       ...
       $prep_chart_config = array(
           ...
           '1' => array(
               ...
               'label_x'  => $this->getChartText(strip_tags($Lab_icd1)),
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_order']['sc_free_group_by'] as $i_summ_order)
       {
                   ...
                   foreach ($this->sc_comb_quick_index[$i_index_summ] as $i_index_comb_groupby)
                   {
                       ...
                       if (!isset($a_chart_combinations[$s_comb_index]))
                       {
                           $a_chart_combinations[$s_comb_index] = array(
                               ...
                               'label_x'     => '',
                               'label_y'     => '',
                               'xml_pholder' => sizeof($this->comp_y_axys) > $i_index_group + 1 ? 'sc_chart_diagnosa_top_' . session_id() . '_!!!C_' . ($i_index_group + 1) . '_' . $s_index_comb . ('' == $s_index_comb ? '' : '_') . '__SC_PLACEHOLDER__!!!.json' : '',
                               'xml'         => 'sc_chart_diagnosa_top_' . session_id() . '_!!!C_' . $i_index_group . '_' . $s_index_comb . '!!!.json',
                           );
                       }
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'];
	}
	...
	function filterChartsBySummarization() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao)) {
			...
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_fields_display']['sc_free_group_by'] as $fieldIndex => $fieldData) {
	...
	function filterChartsByMultiseries() {
		...
		if ($applyFilter && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_drill_down']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['summarizing_drill_down'])) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'] as $chartIndex => $chartData) {
				$indexParts = explode('|', $chartIndex);
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_chart_diagnosa_top_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_chart_diagnosa_top_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['embutida'])
                   {
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['proc_pdf_vert']){ 
   ...
   {
       if (!$res_chart && $this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['json_use_format']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['json_use_format'])
       {
           return $valor_campo;
       }
   ...
   function resumo_init()
   {
      ...
      $this->monta_html_ini();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
      {
      ...
      { 
         ...
         { 
             $seq_search = 1;
             foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq'] as $cmp => $def)
             {
                $Seq_grid      = $seq_search;
                $Cmp_grid      = $cmp;
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_diagnosa_top']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css_bw']['chart_diagnosa_top']) . "_";
               } 
           } 
           else 
           { 
               if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_diagnosa_top']))
               {
                   $compl_css = str_replace(".", "_", $_SESSION['sc_session'][$this->Ini->sc_page]['SC_sub_css']['chart_diagnosa_top']) . "_";
               } 
           }
       }
   ...
   function resumo_final()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['ajax_nav'])
      {
   ...
   {
      $this->Tot_ger = false;
      require_once($this->Ini->path_aplicacao . "chart_diagnosa_top_total.class.php"); 
      $this->Print_All = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['print_all'];
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['doc_word'] || $this->Ini->sc_export_ajax_img)
      { 
      ...
      $this->Total   = new chart_diagnosa_top_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      ...
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['arr_total'][$cmp_gb];
      }
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_refresh_after_chart'] = 'resumo';
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['doc_word'])
{
      ...
      { 
           ...
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_diagnosa_top_ajax.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida(" </script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_diagnosa_top_jquery.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"chart_diagnosa_top_message.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput.js\"></script>\r\n");
           $nm_saida->saida(" <script type=\"text/javascript\" src=\"../_lib/lib/js/jquery.scInput2.js\"></script>\r\n");
   ...
   { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css'] = rand(0, 1000);
      }
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_diagnosa_top_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_chart_diagnosa_top_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_diagnosa_top_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css'] . '.css');
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "chart_diagnosa_top/chart_diagnosa_top_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida("         dataType: \"html\",\r\n");
           $nm_saida->saida("         url: \"chart_diagnosa_top_save_grid.php\",\r\n");
           ...
           $nm_saida->saida("     }\r\n");
           $nm_saida->saida("   </script>\r\n");
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['ajax_nav'] && !$this->Ini->sc_export_ajax && !$this->Ini->Export_html_zip && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['doc_word'])
      {
           $nm_saida->saida("   <script type=\"text/javascript\"> \r\n");
          ...
          if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq']))
          { 
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['dashboard_info']['remove_margin'] ? ' style="margin: 0"' : '';
          $nm_saida->saida(" <BODY class=\"" . $this->css_scGridPage . "\"" . $remove_margin . ">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['embutida'])
      {
      ...
      $nm_saida->saida("function scChartImageExportShow(exportType) {\r\n");
          $nm_saida->saida("  scShowCombinationImage(" . count($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['Labels_GB']) . ", exportType, '');\r\n");
      $nm_saida->saida("}\r\n");
...
;
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_diagnosa_top/chart_diagnosa_top_config_pdf.php?nm_opc=pdf&nm_target=0&nm_cor=cor&papel=1&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=" . s . "&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=chart&nm_all_modules=chart&password=n&summary_export_columns=N&pdf_zip=N&origem=chart&language=id&conf_socor=N&is_chart_app=Y&nm_label_group=S&nm_all_cab=N&nm_all_label=N&nm_orient_grid=2&script_case_init=" . $this->Ini->sc_page . "&app_name=chart_diagnosa_top&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['print'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bprint", "", "", "Rprint_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "chart_diagnosa_top/chart_diagnosa_top_config_print.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=N&nm_opc=resumo&nm_cor=PB&nm_res_cons=s&nm_ini_prt_res=&nm_all_modules=chart&password=n&origem=chart&language=id&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      if ($this->nmgp_botoes['filter'] == 'on' && !$this->Ini->SC_Link_View && !$this->grid_emb_form)
      {
         ...
         if (is_file("chart_diagnosa_top_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
            $Arq_WebHelp = file("chart_diagnosa_top_help.txt"); 
   ...
   function monta_html_fim()
   {
      ...
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_diagnosa_top']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['chart_diagnosa_top']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
$nm_saida->saida("       scJs_alert('" . $mensagem . "', $jsonParams);\r\n");
      ...
      elseif ($this->Ini->Export_html_zip || ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['opcao'] == "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['graf_pdf'] == "S"))
      {
          $this->grafico_pdf();
      }
      else
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'chart_diagnosa_top_config_pivot.php?nome_apl=chart_diagnosa_top&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=id&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   $nm_saida->saida("                  action=\"chart_diagnosa_top_iframe_prt.php\" \r\n");
   ...
   $nm_saida->saida("   </form> \r\n");
          ...
          $nm_saida->saida("               document.Fprint.action = \"chart_diagnosa_top_export_ctrl.php\";\r\n");
          $nm_saida->saida("           }\r\n");
          $nm_saida->saida("           else\r\n");
          $nm_saida->saida("           {\r\n");
   ...
   $nm_saida->saida("           document.Fexport.nmgp_tot_xls.value = tot_xls;\r\n");
          ...
          $nm_saida->saida("       document.Fexport.action = \"chart_diagnosa_top_export_ctrl.php\";\r\n");
      ...
      if (!extension_loaded("zip"))
      {
          ...
          $nm_saida->saida("       document.F3.SC_module_export.value = z;\r\n");
          $nm_saida->saida("  }\r\n");
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_diagnosa_top_iframe_params'] = array(
      ...
      $Md5_pdf    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@chart_diagnosa_top@SC_par@" . md5($prep_parm_pdf);
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['Md5_pdf'][md5($prep_parm_pdf)] = $prep_parm_pdf;
...
;
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_chart_diagnosa_top_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "chart_diagnosa_top/chart_diagnosa_top_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
 function grafico_pdf()
 {
   ...
   $this->Graf->Ini    = $this->Ini;
   $this->Graf->Lookup = $this->Lookup;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['skip_charts']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['skip_charts']))
   {
 ...
 { 
       ...
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts'] as $chart_index => $chart_data)
       {
 ...
 { 
           ...
           $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['this_chart_label'] = '';
           $this->Graf->monta_grafico($chart_index, 'pdf');
           if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['this_chart_label']) {
               $nm_saida->saida("<script type=\"text/javascript\">$(\"#$random_id\").html(\"{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['this_chart_label']}\")</script>\r\n");
           }
       ...
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['charts_html'])
       {
            $nm_saida->saida("<script type=\"text/javascript\">\r\n");
            $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['charts_html']}\r\n");
            $nm_saida->saida("</script>\r\n");
       }
   }
       $nm_saida->saida("</body>\r\n");
...
//--- 
 ...
 {
   global $nm_saida, $nm_lang;
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_list']))
   {
       $prim_graf = true;
       foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['chart_list'] as $arr_chart)
       {
           if (!$prim_graf)
           {
               $nm_saida->saida("<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>\r\n");
...
//--- 
 function grafico_bot()
 {
   ...
   if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts']))
   {
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['graf_bot'] = true;
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['graf_first'] = true;
       require_once($this->Ini->path_aplicacao . $this->Ini->Apl_grafico); 
       $this->Graf  = new chart_diagnosa_top_grafico();
       $this->Graf->Db     = $this->Db;
       $this->Graf->Erro   = $this->Erro;
       ...
       $nm_saida->saida("<TR>\r\n");
       $iChartCount = 1;
       $iChartTotal = sizeof($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['pivot_charts']);
       $sChartLang  = isset($this->Ini->Nm_lang['lang_pdff_pcht']) ? $this->Ini->Nm_lang['lang_pdff_pcht'] : 'Generating chart';
       if (!NM_is_utf8($sChartLang))
       {
           $sChartLang = sc_convert_encoding($sChartLang, "UTF-8", $_SESSION['scriptcase']['charset']);
      ...
      if ($bPDF && (!isset($this->progress_fp) || !$this->progress_fp))
      {
       ...
       $nm_saida->saida("</TD>\r\n");
       $nm_saida->saida("</TR>\r\n");
       $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['graf_bot'] = false;
       if ($bPDF && isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['charts_html']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['charts_html'])
       {
           $nm_saida->saida("<script type=\"text/javascript\">\r\n");
           $nm_saida->saida("{$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['charts_html']}\r\n");
           $nm_saida->saida("</script>\r\n");
...
//--- 
   ...
   //---- 
   function adiciona_registro($quebra_icd1, $quebra_icd1_orig)
   {
      $contr_arr = "";
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_cmp'] as $cmp_gb => $resto)
      {
          $Name_orig  = (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_orig'][$cmp_gb])) ? $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_orig'][$cmp_gb] : $cmp_gb;
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     { 
         $nmgp_select = "SELECT icd1 from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') nm_sel_esp"; 
     } 
     ...
     $nmgp_select .= " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['where_pesq']; 
     $campos_order = "";
     foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['SC_Gb_Free_sql'] as $cmp_var => $resto)
     {
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->icd1 = $rs_res->fields[0] ;  
            $this->icd1_orig = $this->icd1;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->icd1, $this->icd1_orig);
            }
   }
//
   ...
   function html_grid_search()
   {
       ...
       $tmp = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['cond_pesq'];
       $pos = strrpos($tmp, "##*@@");
       if ($pos !== false)
       {
       ...
       $str_display = empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq'])?'none':'';
       if(!$_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['ajax_nav'])
       {
          $nm_saida->saida("   <tr id=\"NM_Grid_Search\" ajax='' style='display:" . $str_display . "'>\r\n");
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['ajax_nav'] && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq']))
       { 
           $_SESSION['scriptcase']['saida_html'] = "";
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq'] as $cmp => $def)
           {
           ...
           if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['save_grid']))
           {
               return;
           }
           else
       ...
       $nm_saida->saida("            <input type=\"hidden\" name=\"parm\" value=\"\"/> \r\n");
            if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq']))
            {
                foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq'] as $cmp => $def)
                {
                    if (isset($def['label'])) {
       ...
       { 
           ...
           $_SESSION['scriptcase']['saida_html'] = "";
           if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['save_grid']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq']))
           { 
   ...
   function grid_search_add_tag()
   {
       ...
       if (count($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq']) < 1)
       {
           $lin_obj .= "<table id='id_grid_search_all_cmp' cellpadding=0 cellspacing=0>";
           foreach ($All_cmp_search as $cada_cmp)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_diagnosa_top']['grid_pesq'][$cada_cmp]))
               {
                   $lin_obj .= "  <tr>";
   ...
   function grid_search_icd1($ind, $ajax, $opc="", $val=array(), $label='')
   {
       ...
       $nm_comando = "select distinct icd1 from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') nm_sel_esp where icd1 = '$icd1_look'"; 
   ...
   function lookup_ajax_icd1($icd1)
   {
       ...
       $icd1_look = substr($this->Db->qstr($icd1), 1, -1); 
       $nmgp_def_dados = array(); 
       $nm_comando = "select distinct icd1 from (select a.icd1 from (select icd1 from tbadmrawatinap UNION ALL select icd1 from tbdetailigd UNION ALL select icd1 from tbdetailrawatjalan) a where a.icd1 != '') nm_sel_esp where  icd1 like '%" . $icd1 . "%'"; 
       $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
       $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->SelectLimit($nm_comando, 10, 0)) 
       { 
          ...
          { 
            ...
            $cmp1 = chart_diagnosa_top_pack_protect_string($cmp1);
            $nmgp_def_dados[] = array($cmp1 => $cmp1); 
             $rs->MoveNext(); 
          } 
          $rs->Close(); 
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/chart_diagnosa_top";
