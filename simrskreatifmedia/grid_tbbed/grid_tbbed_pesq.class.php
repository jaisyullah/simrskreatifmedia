...
class grid_tbbed_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_tbbed_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_tbbed_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_tbbed_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_idbed')
      {
          $idbed = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_idbed($idbed);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_nobed')
      {
          $nobed = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_nobed($nobed);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_ruang')
      {
          $ruang = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_ruang($ruang);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_idbed($idbed)
   {
      $idbed = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $idbed);
      $idbed = substr($this->Db->qstr($idbed), 1, -1);
            $idbed_look = substr($this->Db->qstr($idbed), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct idBed from " . $this->Ini->nm_tabela . " where   CAST (idBed AS TEXT)  like '%" . $idbed . "%' order by idBed"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct idBed from " . $this->Ini->nm_tabela . " where   CAST (idBed AS VARCHAR)  like '%" . $idbed . "%' order by idBed"; 
      }
      else
      {
          $nm_comando = "select distinct idBed from " . $this->Ini->nm_tabela . " where  idBed like '%" . $idbed . "%' order by idBed"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function lookup_ajax_nobed($nobed)
   {
      $nobed = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $nobed);
      $nobed = substr($this->Db->qstr($nobed), 1, -1);
            $nobed_look = substr($this->Db->qstr($nobed), 1, -1); 
      $nmgp_def_dados = array(); 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct noBed from " . $this->Ini->nm_tabela . " where   CAST (noBed AS TEXT)  like '%" . $nobed . "%' order by noBed"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct noBed from " . $this->Ini->nm_tabela . " where   CAST (noBed AS VARCHAR)  like '%" . $nobed . "%' order by noBed"; 
      }
      else
      {
          $nm_comando = "select distinct noBed from " . $this->Ini->nm_tabela . " where  noBed like '%" . $nobed . "%' order by noBed"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function lookup_ajax_ruang($ruang)
   {
      $ruang = substr($this->Db->qstr($ruang), 1, -1);
            $ruang_look = substr($this->Db->qstr($ruang), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct ruang from " . $this->Ini->nm_tabela . " where  ruang like '%" . $ruang . "%' order by ruang"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - tbbed</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_tbbed/grid_tbbed_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_tbbed_ajax_search.js"></script>
 <script type="text/javascript" src="grid_tbbed_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_tbbed_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbbed']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_tbbed']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  str_out += 'SC_idbed_cond#NMF#' + search_get_sel_txt('SC_idbed_cond') + '@NMF@';
  str_out += 'SC_idbed#NMF#' + search_get_text('SC_idbed') + '@NMF@';
  str_out += 'id_ac_idbed#NMF#' + search_get_text('id_ac_idbed') + '@NMF@';
  str_out += 'SC_nobed_cond#NMF#' + search_get_sel_txt('SC_nobed_cond') + '@NMF@';
  str_out += 'SC_nobed#NMF#' + search_get_text('SC_nobed') + '@NMF@';
  str_out += 'id_ac_nobed#NMF#' + search_get_text('id_ac_nobed') + '@NMF@';
  str_out += 'SC_ruang_cond#NMF#' + search_get_sel_txt('SC_ruang_cond') + '@NMF@';
  str_out += 'SC_ruang#NMF#' + search_get_text('SC_ruang') + '@NMF@';
  str_out += 'id_ac_ruang#NMF#' + search_get_text('id_ac_ruang') + '@NMF@';
   ...
   $("#id_ac_idbed").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          ...
          nmgp_parms: "NM_ajax_opcao?#?autocomp_idbed",
          max_itens: "10",
          cod_desc: "N",
   ...
   $("#id_ac_ruang").autocomplete({
     ...
     $.ajax({
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_ruang",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
     ...
     change: function (event, ui) {
       if (null == ui.item) {
          $("#SC_ruang").val( $(this).val() );
       }
     }
   });
   $("#id_ac_nobed").autocomplete({
     minLength: 1,
     source: function (request, response) {
     $.ajax({
       url: "index.php",
       ...
       data: {
          q: request.term,
          nmgp_opcao: "ajax_autocomp",
          nmgp_parms: "NM_ajax_opcao?#?autocomp_nobed",
          max_itens: "10",
          cod_desc: "N",
          script_case_init: <?php echo $this->Ini->sc_page ?>
        },
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - tbbed</div>
...
<?php
   ...
   function monta_form()
   {
      global 
             ...
             $idbed_cond, $idbed, $idbed_autocomp,
             $nobed_cond, $nobed, $nobed_autocomp,
             $ruang_cond, $ruang, $ruang_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $kelas = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['kelas']; 
          $kelas_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['kelas_cond']; 
          $idbed = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['idbed']; 
          $idbed_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['idbed_cond']; 
          $nobed = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['nobed']; 
          $nobed_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['nobed_cond']; 
          $ruang = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['ruang']; 
          $ruang_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['ruang_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['NM_operador']; 
      } 
      if (!isset($kelas_cond) || empty($kelas_cond))
      {
         $kelas_cond = "qp";
      }
      if (!isset($idbed_cond) || empty($idbed_cond))
      {
         $idbed_cond = "gt";
      }
      if (!isset($nobed_cond) || empty($nobed_cond))
      {
         $nobed_cond = "gt";
      }
      if (!isset($ruang_cond) || empty($ruang_cond))
      {
         $ruang_cond = "qp";
      }
      ...
      $str_hide_idbed = (in_array($idbed_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_nobed = (in_array($nobed_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_ruang = (in_array($ruang_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      ...
      if (!isset($idbed) || $idbed == "")
      {
          $idbed = "";
      }
      if (isset($idbed) && !empty($idbed))
      {
      ...
      if (!isset($nobed) || $nobed == "")
      {
          $nobed = "";
      }
      if (isset($nobed) && !empty($nobed))
      {
...
?>
<INPUT  type="text" id="SC_kelas" name="kelas" value="<?php echo NM_encode_input($kelas) ?>"  size=25 alt="{datatype: 'text', maxLength: 25, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_kelas" name="kelas_autocomp" size="25"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 25, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_idbed_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['idbed'])) ? $this->New_label['idbed'] : "Id Bed"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_idbed_cond" name="idbed_cond" onChange="nm_campos_between(document.getElementById('id_vis_idbed'), this, 'idbed')">
       <OPTION value="gt" <?php if ("gt" == $idbed_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $idbed_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $idbed_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_idbed" <?php echo $str_hide_idbed?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['idbed'])) ? $this->New_label['idbed'] : "Id Bed";
 $nmgp_tab_label .= "idbed?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct idBed from " . $this->Ini->nm_tabela . " where idBed = $idbed_look order by idBed"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct idBed from " . $this->Ini->nm_tabela . " where idBed = $idbed_look order by idBed"; 
      ...
      if (isset($nmgp_def_dados[0][$idbed]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$idbed];
      }
      else
      {
            nmgp_Form_Num_Val($idbed, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $idbed;
      }
?>
<INPUT  type="text" id="SC_idbed" name="idbed" value="<?php echo NM_encode_input($idbed) ?>" size=5 alt="{datatype: 'integer', maxLength: 5, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_idbed" name="idbed_autocomp" size="5"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 5, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_nobed_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['nobed'])) ? $this->New_label['nobed'] : "No Bed"; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_nobed_cond" name="nobed_cond" onChange="nm_campos_between(document.getElementById('id_vis_nobed'), this, 'nobed')">
       <OPTION value="gt" <?php if ("gt" == $nobed_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $nobed_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $nobed_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_nobed" <?php echo $str_hide_nobed?> valign="top">
        <TD class="scFilterFieldFontOdd">
           <?php
 $SC_Label = (isset($this->New_label['nobed'])) ? $this->New_label['nobed'] : "No Bed";
 $nmgp_tab_label .= "nobed?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct noBed from " . $this->Ini->nm_tabela . " where noBed = $nobed_look order by noBed"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct noBed from " . $this->Ini->nm_tabela . " where noBed = $nobed_look order by noBed"; 
      ...
      if (isset($nmgp_def_dados[0][$nobed]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$nobed];
      }
      else
      {
            nmgp_Form_Num_Val($nobed, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $nobed;
      }
?>
<INPUT  type="text" id="SC_nobed" name="nobed" value="<?php echo NM_encode_input($nobed) ?>" size=2 alt="{datatype: 'integer', maxLength: 2, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectOdd" type="text" id="id_ac_nobed" name="nobed_autocomp" size="2"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'integer', maxLength: 2, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}">
      ...
      <TD id='SC_ruang_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['ruang'])) ? $this->New_label['ruang'] : "Ruang"; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_ruang_cond" name="ruang_cond" onChange="nm_campos_between(document.getElementById('id_vis_ruang'), this, 'ruang')">
       <OPTION value="qp" <?php if ("qp" == $ruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $ruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $ruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $ruang_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_ruang" <?php echo $str_hide_ruang?> valign="top">
        <TD class="scFilterFieldFontEven">
           <?php
 $SC_Label = (isset($this->New_label['ruang'])) ? $this->New_label['ruang'] : "Ruang";
 $nmgp_tab_label .= "ruang?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $ruang_look = substr($this->Db->qstr($ruang), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct ruang from " . $this->Ini->nm_tabela . " where ruang = '$ruang_look' order by ruang"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$ruang]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$ruang];
      }
      else
      {
          $sAutocompValue = $ruang;
      }
?>
<INPUT  type="text" id="SC_ruang" name="ruang" value="<?php echo NM_encode_input($ruang) ?>"  size=25 alt="{datatype: 'text', maxLength: 25, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_ruang" name="ruang_autocomp" size="25"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 25, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
...
?>
<?php
   if (is_file("grid_tbbed_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_tbbed']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_tbbed']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.idbed_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_idbed'), document.F1.idbed_cond, 'idbed');
   document.F1.idbed.value = "";
   document.F1.idbed_autocomp.value = "";
   document.F1.nobed_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_nobed'), document.F1.nobed_cond, 'nobed');
   document.F1.nobed.value = "";
   document.F1.nobed_autocomp.value = "";
   document.F1.ruang_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_ruang'), document.F1.ruang_cond, 'ruang');
   document.F1.ruang.value = "";
   document.F1.ruang_autocomp.value = "";
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_tbbed";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function recupera_filtro($NM_filters)
   {
      ...
      $tp_fields['SC_idbed_cond'] = 'cond';
      $tp_fields['SC_idbed'] = 'text_aut';
      $tp_fields['id_ac_idbed'] = 'text_aut';
      $tp_fields['SC_nobed_cond'] = 'cond';
      $tp_fields['SC_nobed'] = 'text_aut';
      $tp_fields['id_ac_nobed'] = 'text_aut';
      $tp_fields['SC_ruang_cond'] = 'cond';
      $tp_fields['SC_ruang'] = 'text_aut';
      $tp_fields['id_ac_ruang'] = 'text_aut';
   ...
   {
      global $kelas_cond, $kelas, $kelas_autocomp,
             $idbed_cond, $idbed, $idbed_autocomp,
             $nobed_cond, $nobed, $nobed_autocomp,
             $ruang_cond, $ruang, $ruang_autocomp, $nmgp_tab_label;
      ...
      if (!empty($idbed_autocomp) && empty($idbed))
      {
          $idbed = $idbed_autocomp;
      }
      if (!empty($nobed_autocomp) && empty($nobed))
      {
          $nobed = $nobed_autocomp;
      }
      if (!empty($ruang_autocomp) && empty($ruang))
      {
          $ruang = $ruang_autocomp;
      }
      ...
      $idbed_cond_salva = $idbed_cond; 
      if (!isset($idbed_input_2) || $idbed_input_2 == "")
      {
          $idbed_input_2 = $idbed;
      }
      $nobed_cond_salva = $nobed_cond; 
      if (!isset($nobed_input_2) || $nobed_input_2 == "")
      {
          $nobed_input_2 = $nobed;
      }
      $ruang_cond_salva = $ruang_cond; 
      if (!isset($ruang_input_2) || $ruang_input_2 == "")
      {
          $ruang_input_2 = $ruang;
      }
      if ($idbed_cond != "in")
      {
          nm_limpa_numero($idbed, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      if ($nobed_cond != "in")
      {
          nm_limpa_numero($nobed, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['kelas'] = $kelas; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['kelas_cond'] = $kelas_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['idbed'] = $idbed; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['idbed_cond'] = $idbed_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['nobed'] = $nobed; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['nobed_cond'] = $nobed_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['ruang'] = $ruang; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['ruang_cond'] = $ruang_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca'] = $Temp_Busca;
      }
      if ($idbed_cond != "in" && $idbed_cond != "bw" && !empty($idbed) && !is_numeric($idbed))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Id Bed";
      }
      if ($idbed_cond == "bw" && ((!empty($idbed) && !is_numeric($idbed)) || (!empty($idbed_input_2) && !is_numeric($idbed_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Id Bed";
      }
      if ($nobed_cond != "in" && $nobed_cond != "bw" && !empty($nobed) && !is_numeric($nobed))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : No Bed";
      }
      if ($nobed_cond == "bw" && ((!empty($nobed) && !is_numeric($nobed)) || (!empty($nobed_input_2) && !is_numeric($nobed_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : No Bed";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['ruang'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['ruang'] = $ruang;
      }
      ...
      //----- $idbed
      $this->Date_part = false;
      if (isset($idbed) || $idbed_cond == "nu" || $idbed_cond == "nn" || $idbed_cond == "ep" || $idbed_cond == "ne")
      {
         $this->monta_condicao("idBed", $idbed_cond, $idbed, "", "idbed");
      }

      //----- $nobed
      $this->Date_part = false;
      if (isset($nobed) || $nobed_cond == "nu" || $nobed_cond == "nn" || $nobed_cond == "ep" || $nobed_cond == "ne")
      {
         $this->monta_condicao("noBed", $nobed_cond, $nobed, "", "nobed");
      }

      //----- $ruang
      $this->Date_part = false;
      if (isset($ruang) || $ruang_cond == "nu" || $ruang_cond == "nn" || $ruang_cond == "ep" || $ruang_cond == "ne")
      {
         $this->monta_condicao("ruang", $ruang_cond, $ruang, "", "ruang");
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_tbbed']['cond_pesq'] .= $this->NM_operador;
