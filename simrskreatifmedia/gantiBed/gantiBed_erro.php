...
$old_error_handler = set_error_handler("gantiBed_scriptcase_error_handler");

function gantiBed_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['gantiBed']['contr_erro']) && $_SESSION['scriptcase']['gantiBed']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_gantiBed->contr_gantiBed->Apl_com_erro = true;
            if ($inicial_gantiBed->contr_gantiBed->NM_ajax_flag)
            {
                $inicial_gantiBed->contr_gantiBed->NM_ajax_info['errList']['geral_gantiBed'][] = $msg;
                return;
            }
            gantiBed_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function gantiBed_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['gantiBed'])  && '' != $_SESSION['scriptcase']['error_icon']['gantiBed'])  ? $_SESSION['scriptcase']['error_icon']['gantiBed']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['gantiBed']) && '' != $_SESSION['scriptcase']['error_close']['gantiBed']) ? $_SESSION['scriptcase']['error_close']['gantiBed'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['gantiBed']) && '' != $_SESSION['scriptcase']['error_span_title']['gantiBed']) ? $_SESSION['scriptcase']['error_span_title']['gantiBed'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['gantiBed']) && '' != $_SESSION['scriptcase']['error_icon_title']['gantiBed']) ? $_SESSION['scriptcase']['error_icon_title']['gantiBed'] : '';
?>
