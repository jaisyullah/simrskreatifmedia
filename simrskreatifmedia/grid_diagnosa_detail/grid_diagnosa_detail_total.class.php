...
class grid_diagnosa_detail_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_diagnosa_detail']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_diagnosa_detail']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_regDate($res_limit=false)
   {
      global $nada, $nm_lang , $custcode, $icd1;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from (SELECT 	z.custCode, 	z.diagnosa1, 	z.icd1, 	a.sex, 	b.regDate  FROM 	( SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailrawatjalan UNION ALL SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailigd ) z 	LEFT JOIN tbcustomer a ON a.custCode = z.custCode 	LEFT JOIN tbantrian b ON b.struckNo = z.noStruk  WHERE 	z.diagnosa1 != '' and b.status != 'Batal') nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*) from (SELECT 	z.custCode, 	z.diagnosa1, 	z.icd1, 	a.sex, 	b.regDate  FROM 	( SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailrawatjalan UNION ALL SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailigd ) z 	LEFT JOIN tbcustomer a ON a.custCode = z.custCode 	LEFT JOIN tbantrian b ON b.struckNo = z.noStruk  WHERE 	z.diagnosa1 != '' and b.status != 'Batal') nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['contr_total_geral'] = "OK";
   } 

   //-----  sex
   function quebra_sex_regDate($sex, $arg_sum_sex) 
   {
      global $tot_sex, $custcode, $icd1;
      $tot_sex = array() ;  
      $tot_sex[0] = $sex ; 
   }

   //----- 
   function Calc_resumo_regDate($destino_resumo)
   {
      global $nm_lang, $custcode, $icd1;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('sex' => "sex");
      $cmps_quebra_atual = array("sex");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_regDate($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_regDate($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      ...
      { 
         $cmd_simp = "select count(*)#@#cmps_quebras#@# from (SELECT 	z.custCode, 	z.diagnosa1, 	z.icd1, 	a.sex, 	b.regDate  FROM 	( SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailrawatjalan UNION ALL SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailigd ) z 	LEFT JOIN tbcustomer a ON a.custCode = z.custCode 	LEFT JOIN tbantrian b ON b.struckNo = z.noStruk  WHERE 	z.diagnosa1 != '' and b.status != 'Batal') sc_sel_esp " . $where_ok;
         $comando  = "select count(*)#@#cmps_quebras#@# from (";
         $comando .= "select " . $cmps_quebras1 . " from (SELECT 	z.custCode, 	z.diagnosa1, 	z.icd1, 	a.sex, 	b.regDate  FROM 	( SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailrawatjalan UNION ALL SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailigd ) z 	LEFT JOIN tbcustomer a ON a.custCode = z.custCode 	LEFT JOIN tbantrian b ON b.struckNo = z.noStruk  WHERE 	z.diagnosa1 != '' and b.status != 'Batal') sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	z.custCode, 	z.diagnosa1, 	z.icd1, 	a.sex, 	b.regDate  FROM 	( SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailrawatjalan UNION ALL SELECT custCode, diagnosa1, icd1, noStruk FROM tbdetailigd ) z 	LEFT JOIN tbcustomer a ON a.custCode = z.custCode 	LEFT JOIN tbantrian b ON b.struckNo = z.noStruk  WHERE 	z.diagnosa1 != '' and b.status != 'Batal') sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_diagnosa_detail']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['sex']['reg'] = "S";
      $format_dimensions['sex']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'regDate', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "sex") {
                  $this->Lookup->lookup_regDate_sex($$Cada_dim); 
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_regDate($where_ok);
