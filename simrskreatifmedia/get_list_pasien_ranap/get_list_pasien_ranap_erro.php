...
$old_error_handler = set_error_handler("get_list_pasien_ranap_scriptcase_error_handler");

function get_list_pasien_ranap_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['get_list_pasien_ranap']['contr_erro']) && $_SESSION['scriptcase']['get_list_pasien_ranap']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_get_list_pasien_ranap->contr_get_list_pasien_ranap->Apl_com_erro = true;
            if ($inicial_get_list_pasien_ranap->contr_get_list_pasien_ranap->NM_ajax_flag)
            {
                $inicial_get_list_pasien_ranap->contr_get_list_pasien_ranap->NM_ajax_info['errList']['geral_get_list_pasien_ranap'][] = $msg;
                return;
            }
            get_list_pasien_ranap_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function get_list_pasien_ranap_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['get_list_pasien_ranap'])  && '' != $_SESSION['scriptcase']['error_icon']['get_list_pasien_ranap'])  ? $_SESSION['scriptcase']['error_icon']['get_list_pasien_ranap']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['get_list_pasien_ranap']) && '' != $_SESSION['scriptcase']['error_close']['get_list_pasien_ranap']) ? $_SESSION['scriptcase']['error_close']['get_list_pasien_ranap'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['get_list_pasien_ranap']) && '' != $_SESSION['scriptcase']['error_span_title']['get_list_pasien_ranap']) ? $_SESSION['scriptcase']['error_span_title']['get_list_pasien_ranap'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['get_list_pasien_ranap']) && '' != $_SESSION['scriptcase']['error_icon_title']['get_list_pasien_ranap']) ? $_SESSION['scriptcase']['error_icon_title']['get_list_pasien_ranap'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['get_list_pasien_ranap']['error_buffer'] .= ob_get_flush();
}
