...
$old_error_handler = set_error_handler("sendLIS_scriptcase_error_handler");

function sendLIS_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['sendLIS']['contr_erro']) && $_SESSION['scriptcase']['sendLIS']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_sendLIS->contr_sendLIS->Apl_com_erro = true;
            if ($inicial_sendLIS->contr_sendLIS->NM_ajax_flag)
            {
                $inicial_sendLIS->contr_sendLIS->NM_ajax_info['errList']['geral_sendLIS'][] = $msg;
                return;
            }
            sendLIS_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function sendLIS_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['sendLIS'])  && '' != $_SESSION['scriptcase']['error_icon']['sendLIS'])  ? $_SESSION['scriptcase']['error_icon']['sendLIS']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['sendLIS']) && '' != $_SESSION['scriptcase']['error_close']['sendLIS']) ? $_SESSION['scriptcase']['error_close']['sendLIS'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['sendLIS']) && '' != $_SESSION['scriptcase']['error_span_title']['sendLIS']) ? $_SESSION['scriptcase']['error_span_title']['sendLIS'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['sendLIS']) && '' != $_SESSION['scriptcase']['error_icon_title']['sendLIS']) ? $_SESSION['scriptcase']['error_icon_title']['sendLIS'] : '';
?>
