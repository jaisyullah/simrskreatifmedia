...
class grid_approvalusulanpengadaan_gizi_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_approvalusulanpengadaan_gizi']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_approvalusulanpengadaan_gizi']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_Status($res_limit=false)
   {
      global $nada, $nm_lang ;
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['where_pesq']; 
      } 
      ...
      while (!$rt->EOF) 
      {
         if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['tot_geral'][1])) 
         {
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['tot_geral'][1] = 1 ; 
         } 
         else
         {
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['tot_geral'][1] += 1 ; 
   ...
   //-----  selesai
   function quebra_selesai_Status($selesai, $arg_sum_selesai) 
   {
      global $tot_selesai;
      $tot_selesai = array() ;  
   ...
   //----- 
   function Calc_resumo_Status($destino_resumo)
   {
      global $nm_lang, $status;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('selesai' => "a.selesai");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_Status($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_Status($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('id' => 'N','total' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_approvalusulanpengadaan_gizi']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "selesai") {
                  $this->Lookup->lookup_Status_selesai($$Cada_dim); 
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
