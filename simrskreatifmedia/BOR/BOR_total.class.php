...
class BOR_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['BOR']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['BOR']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_Tgl($res_limit=false)
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['tot_geral'] = array() ;  
    if ($res_limit && isset($_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['sql_tot_res']))
    {
        $nm_comando = $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['sql_tot_res'];
    }
    else
    {
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(BOR) from (SELECT 	date_format( a.tglKeluar, '%Y-%m-%d' ) AS Tgl, 	SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) AS HP, 	( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) as jmlBed, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(BOR) from (SELECT 	date_format( a.tglKeluar, '%Y-%m-%d' ) AS Tgl, 	SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) AS HP, 	( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) as jmlBed, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['contr_total_geral'] = "OK";
   } 

   //-----  tgl
   function quebra_tgl_Tgl($tgl, $arg_sum_tgl) 
   {
      global $tot_tgl;
      $tot_tgl = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      ...
      { 
         $nm_comando .= " where Tgl" . $arg_sum_tgl ; 
      } 
      else 
      { 
         $nm_comando .= " and Tgl" . $arg_sum_tgl ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
   ...
   //----- 
   function Calc_resumo_Tgl($destino_resumo)
   {
      global $nm_lang, $tanggal;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tgl' => "Tgl");
      $cmps_quebra_atual = array("tgl");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_Tgl($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_Tgl($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('hp' => 'N','jmlbed' => 'N','bor' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(BOR)#@#cmps_quebras#@# from (SELECT 	date_format( a.tglKeluar, '%Y-%m-%d' ) AS Tgl, 	SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) AS HP, 	( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) as jmlBed, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select BOR as SC_metric1, " . $cmps_quebras1 . " from (SELECT 	date_format( a.tglKeluar, '%Y-%m-%d' ) AS Tgl, 	SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) AS HP, 	( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) as jmlBed, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	date_format( a.tglKeluar, '%Y-%m-%d' ) AS Tgl, 	SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) AS HP, 	( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) as jmlBed, 	( 		SUM( DATEDIFF( a.tglKeluar, b.tglMasuk ) ) / ( 			( SELECT count( * ) FROM tbbed WHERE kelas NOT LIKE 'BY%' AND kelas != 'PERINA' AND kelas != 'JENAZAH' ) * ( SELECT DAY ( LAST_DAY( a.tglKeluar ) ) )  		)  	) * 100 AS BOR  FROM 	tbdetailrawatinap a 	LEFT JOIN tbadmrawatinap b ON b.tranCode = a.tranCode 	LEFT JOIN tbbed c ON c.idBed = b.bed  WHERE 	a.tglKeluar IS NOT NULL  	AND c.kelas NOT LIKE 'BY%'  	AND c.kelas != 'PERINA'  	AND c.kelas != 'JENAZAH'  GROUP BY 	date_format( a.tglKeluar, '%Y-%m' )) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['BOR']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tgl']['reg'] = "S";
      $format_dimensions['tgl']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'Tgl', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          { 
              $this->nm_data->SetaData($$Cada_dim, "");
              $$Cada_dim = $this->nm_data->FormataSaida("mmaaaa");
          } 
              }
              if (null === $$Cada_dim)
              {
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_Tgl($where_ok);
