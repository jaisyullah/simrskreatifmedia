...
class fast_slow_moving_resumo
{
   ...
   function initializeButtons()
   {
      ...
      $this->nmgp_botoes['chart_exit'] = isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['dashboard_info']['under_dashboard']) && $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['dashboard_info']['under_dashboard'] ? "off" : "on";
   ...
   { 
	...
	function generateChartImages() {
		...
		$chartList  = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_charts'];
		$chartFiles = array();

		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['phantomjs_export_process'] = true;
		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_step']     = 'image';
		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_count']    = 0;
		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_total']    = 0;
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_total']++;
		}
		...
		foreach ($chartList as $chartKey => $chartData) {
			...
			$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['phantomjs_export_file'] = '';
			$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_count']++;

			$this->writeProgressFile();

			$this->Graf->generateChartImage($chartKey);

			if ('' != $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['phantomjs_export_file']) {
				$chartFiles[] = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['phantomjs_export_file'];
			}
		}

		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['phantomjs_export_process'] = false;
	...
	function zipChartImages($password = '') {
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_step'] = 'zip';

		$this->writeProgressFile();

		$zipFile = $this->zipFileList($chartImages, $password);

		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_step'] = 'end';
	...
	function zipFileList($fileList, $password = '') {
		$tempDir     = $this->Ini->root . $_SESSION['scriptcase']['fast_slow_moving']['glo_nm_path_imag_temp'] . '/';
	...
	function writeProgressFile() {
		if ('image' == $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_pdff_pcht'] . ": {$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_count']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_total']}###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('zip' == $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_step']) {
			$progress = floor($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_count'] * 100 / ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_total'] + 1));
			$content  = $this->Ini->Nm_lang['lang_chrt_zip_img'] . "###$progress";

			$content  = $_SESSION['scriptcase']['charset'] != 'UTF-8' ? sc_convert_encoding($content, 'UTF-8', $_SESSION['scriptcase']['charset']) : $content;
		}
		elseif ('end' == $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_step']) {
			$content = "ok###100";
		}
		else {
			$content = "init###0";
		...
		$f = @fopen("{$this->Ini->root}{$_SESSION['scriptcase']['fast_slow_moving']['glo_nm_path_imag_temp']}/{$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['export_progress_file']}", 'w');
		fwrite($f, $content);
		fclose($f);
	}
   ...
   function monta_resumo($b_export = false)
   {
     ...
     { 
      ...
      {
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['where_resumo']);
      }
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['res_hrz']))
      { 
          $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['res_hrz'] = $this->NM_totaliz_hrz;
      } 
      $this->NM_totaliz_hrz = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['res_hrz'];
      $dir_raiz          = strrpos($_SERVER['PHP_SELF'],"/") ;  
      $dir_raiz          = substr($_SERVER['PHP_SELF'], 0, $dir_raiz + 1) ;  
      $this->nm_location = $this->Ini->sc_protocolo . $this->Ini->server . $dir_raiz; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['array_graf_pdf'] = array();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "pdf")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['where_resumo'] = "";
      }
      $this->inicializa_vars();
      $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['SC_Ind_Groupby'];
      $this->Total->$Gb_geral();
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_array_resumo'] == "OK" && $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_total_geral'] == "OK")
      {
          $this->array_total_tgl = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total']['tgl'];
          $this->array_total_item = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total']['item'];
      }
      else
      {
          $this->array_total_tgl = array();
      ...
      $this->array_total_geral[0] = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][3];
      $this->array_total_geral[1] = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][2];
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][1]) || $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][1] == 0)
      {
      ...
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] == 'print' || $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] == 'pdf') && strpos(" " . $this->Ini->SC_module_export, "resume") === false)
      { }
      else
      {
          $this->drawMatrix();
      ...
      $this->resumo_final();
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_label_graf'] = array();
      if (isset($this->nmgp_label_quebras) && !empty($this->nmgp_label_quebras))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_label_graf'] = $this->nmgp_label_quebras;
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total']['tgl'] = $this->array_total_tgl;
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total']['item'] = $this->array_total_item;
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_array_resumo'] = "OK";
   }
   ...
   function completeMatrix()
   {
       ...
       //-----
       if ($this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['json_use_label']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['json_use_label'])
       {
          $Lab_tgl = "Tgl";
          $Lab_item = "item";
       }
       ...
       {
          $Lab_tgl = "Tgl Transaksi";
          $Lab_tgl =  sprintf("" . $this->Ini->Nm_lang['lang_othr_cons_title_YYYYMM'] . "", $Lab_tgl);
          $Lab_item = "Item";
       }
       $this->comp_field = array(
           $Lab_tgl,
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['item']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['item'] = "Item"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['tgl']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['tgl'] = "Tgl Transaksi"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['jumlah']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['jumlah'] = "Jumlah"; 
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['tot']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['labels']['tot'] = "Total"; 
       }
       //-----
       $this->comp_field_nm = array(
           'tgl' => 0,
       ...
       $this->comp_sum = array(
           ...
           3 => "Jumlah (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")",
       );
       ...
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_order']['namaItem']))
       {
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_order']['namaItem'][] = $i_sum;
           }
       }
       else
       {
           $this->comp_sum_order = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_order']['namaItem'];
       }
       ...
       $this->comp_sum_display = array(
           ...
           foreach ($this->comp_sum as $i_sum => $l_sum)
           {
               if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'][$i_sum]))
               {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'][$i_sum] = array('label' => $l_sum, 'display' => $this->comp_sum_display[$i_sum]);
               }
               else
               {
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'][$i_sum]['label']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'][$i_sum]['label'] = $l_sum;
                   }
                   if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'][$i_sum]['display']))
                   {
                       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'][$i_sum]['display'] = $this->comp_sum_display[$i_sum];
                   }
               }
           }
           foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['summarizing_fields_display']['namaItem'] as $i_sum => $d_sum)
           {
       ...
       //-----
       $this->comp_sum_lnk = array(
           2 => array('field' => "tot", 'show' => true),
           3 => array('field' => "jumlah", 'show' => true),
       );
       ...
       //-----
       foreach ($this->array_total_tgl as $i_tgl => $d_tgl) {
           if (!in_array($i_tgl, $this->comp_label[0], true)) {
               $this->comp_index[0][ $d_tgl[4] ] = $d_tgl[3];
               $this->comp_label[0][ $d_tgl[4] ] = $d_tgl[3];
           }
           foreach ($this->array_total_item[$i_tgl] as $i_item => $d_item) {
               if (!in_array($i_item, $this->comp_label[1][ $d_tgl[4] ], true)) {
                   $this->comp_index[1][ $d_item[4] ] = $d_item[3];
                   $this->comp_label[1][ $d_tgl[4] ][ $d_item[4] ] = $d_item[3];
               }
           }
       }
       ...
       //-----
       $this->comp_links_fl = array(
           array('name' => 'tgl', 'prot' => '@aspass@'),
           array('name' => 'item', 'prot' => '@aspass@'),
       );
       ...
       //-----
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_group_by'] = $this->comp_field;
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_x_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_x_axys'] = $this->comp_x_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_y_axys']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_y_axys'] = $this->comp_y_axys;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_fill']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_fill'] = $this->comp_fill;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order'] = $this->comp_order;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_start']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_start'] = $this->comp_order_start;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular'] = $this->comp_tabular;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_hover']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_hover'] = $this->comp_tabular && $this->comp_tab_hover;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_seq']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_seq'] = $this->comp_tabular && $this->comp_tab_seq;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_label']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_label'] = $this->comp_tabular && $this->comp_tab_label;
       }

       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_col']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_col'] = 3;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_level']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_level'] = -1;
       }
       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_sort']))
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_sort'] = 'd';
       }
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['ajax_nav'] && isset($_POST['parm']))
       { 
       ...
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_sort'] = $_POST['sort_ord'];
           if ('' == $_POST['sort_ord'])
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_col']  = 0;
           }
           else
           {
               $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_col']  = $_POST['sort_col'];
           }
       }
       if (isset($_POST['change_sort']) && 'NEW' == $_POST['change_sort']) {
           for ($i = 0; $i < sizeof($this->comp_label); $i++) {
               if ($i == $_POST['sort_col']) {
                   $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_start'][$i] = $_POST['sort_ord'];
               }
           }
       }

       $this->comp_x_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_x_axys'];
       $this->comp_y_axys      = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_y_axys'];
       $this->comp_fill        = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_fill'];
       $this->comp_order       = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order'];
       $this->comp_order_start = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_start'];
       $this->comp_order_col   = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_col'];
       $this->comp_order_level = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_level'];
       $this->comp_order_sort  = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_sort'];
       $this->comp_tabular     = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular'];
       $this->comp_tab_hover   = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_hover'];
       $this->comp_tab_seq     = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_seq'];
       $this->comp_tab_label   = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_tabular_label'];
       ...
       //-----
       foreach ($this->comp_label[0] as $i_tgl => $l_tgl) {
           if (isset($this->array_total_tgl[$i_tgl])) {
               $this->comp_group[$i_tgl] = array();
           }
           foreach ($this->comp_label[1][$i_tgl] as $i_item => $l_item) {
               if (isset($this->array_total_item[$i_tgl][$i_item])) {
                   $this->comp_group[$i_tgl][$i_item] = array();
               }
           }
       }
   ...
   function getCompData($level, $params = array()) {
       if (0 == $level) {
           $return = $this->array_total_tgl;
       }
       elseif (1 == $level) {
           $return = $this->array_total_item;
       }
   ...
   function getLabelLink($param, $i_tmp = -1, $bProtect = true)
   {
       ...
       $Md5_Res    = "@SC_par@" . NM_encode_input($this->Ini->sc_page) . "@SC_par@fast_slow_moving@SC_par@" . md5($Parms_Res);
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['LigR_Md5'][md5($Parms_Res)] = $Parms_Res;
       return $Md5_Res;
   }
   ...
   function buildChart()
   {
       ...
       $this->comp_chart_config = array(
           ...
           '0|2' => array(
               'title'    => $this->getChartText(strip_tags("Jumlah (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'show_sub' => true,
               'subtitle' => "",
               'field_x'  => 'Tgl',
               'field_y'  => $this->comp_sum_nm[3],
               'label_x'  => $this->getChartText(strip_tags($Lab_tgl)),
               'label_y'  => $this->getChartText(strip_tags("Jumlah (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
           ...
           '1|1' => array(
               ...
               'field_x'  => 'item',
               'field_y'  => $this->comp_sum_nm[2],
               'label_x'  => $this->getChartText(strip_tags($Lab_item)),
               'label_y'  => $this->getChartText(strip_tags("Total (" .  $this->Ini->Nm_lang['lang_btns_smry_msge_sum'] . ")")),
               'format'   => $this->formatChartValue(1),
           ),
           '1|2' => array(
       ...
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_charts']    = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_orig']      = $this->comp_chart_data;
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['comb_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['comb_table_def']  = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['anlt_table_data'] = array();
       $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['anlt_table_def']  = array();
	...
	function filterChartsByGroupbyLevel() {
		...
		if (isset($nmgp_opcao) && ('pdf' == $nmgp_opcao || 'pdf_res' == $nmgp_opcao) && isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['chart_level']) && '' !== $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['chart_level']) {
			$applyFilter = true;
		}
		elseif (isset($nmgp_opcao) && 'image_res' == $nmgp_opcao && isset($_POST['chart_level']) && '' !== $_POST['chart_level']) {
			$applyFilter = true;
			$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['chart_level'] = $_POST['chart_level'];
		}
		...
		$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_charts'] = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_orig'];

		if ($applyFilter) {
			foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_charts'] as $chartIndex => $chartData) {
				...
				if ($groupbyLevel > $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['chart_level']) {
					unset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_charts'][$chartIndex]);
				}
			}
		}

		$this->comp_chart_data = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_charts'];
	}
           ...
           foreach ($a_totals as $i_total => $values)
           {
               ...
               if (isset($this->comp_chart_config[$key_config]))
               {
                   ...
                   foreach ($a_keys as $tmp_key)
                   {
                       ...
                       $xml_links[]  = $this->getFusionLink('sc_fast_slow_moving_' . session_id() . '_!!!' . implode('_', array_merge(array($group + 1, $i_total), $link_index)) . '!!!.json');
                   }
                   ...
                   $this->comp_chart_data[$key_data]['xml']        = $this->getFusionLink('sc_fast_slow_moving_' . session_id() . '_!!!' . implode('_', array_merge(array($group, $i_total), $param)) . '!!!.json');
   ...
   function drawMatrix()
   {
       ...
       if ($this->NM_export)
       {
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_export']['label'] = $this->build_labels;
           $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_export']['data']  = $this->build_data;
           return;
       }
      ...
      $TD_padding = (!$this->Print_All && $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] == "pdf") ? " style=\"padding: 0px !important;\"" : "";
      ...
      if ($this->comp_tab_hover)
      {
       ...
       foreach ($this->build_data as $row_i => $lines)
       {
           ...
           foreach ($lines as $col_i => $columns)
           {
               ...
               if (0 <= $columns['level'])
               {
                   if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "pdf" && $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['embutida'])
                   {
               ...
               $chart   = (($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "print" && $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['embutida']) && isset($columns['chart']) && '' != $columns['chart'] && isset($this->comp_chart_data[ $columns['chart'] ]))
           ...
           if (!$b_display)
           {
               if ($this->comp_tabular)
               {
                   foreach ($this->comp_y_axys as $iYAxysIndex)
                   {
                       ...
                       if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_start'][$iYAxysIndex]))
                       {
                           $sInitialOrder   = '';
                           $sInitialDisplay = '; display: none';
                           $sInitialSrc     = '';
                       }
                       elseif ('asc' == $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['pivot_order_start'][$iYAxysIndex])
                       {
...
if($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['proc_pdf'] || $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['proc_pdf_vert']){ 
   ...
   {
       if (!$res_chart && $this->NM_export && isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['json_use_format']) && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['json_use_format'])
       {
           return $valor_campo;
       }
   ...
   function resumo_init()
   {
      ...
      {
         $this->monta_html_ini();
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "pdf" && !$_SESSION['scriptcase']['proc_mobile'])
         {
   ...
   function monta_css()
   {
      global $nm_saida, $nmgp_tipo_pdf, $nmgp_cor_print;
       ...
       if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['embutida'])
       {
          if (($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
           { 
   ...
   function inicializa_vars()
   {
      ...
      $this->Total   = new fast_slow_moving_total($this->Ini->sc_page);
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['LigR_Md5'] = array();
   }
   ...
   function totaliza()
   {
      $this->Total->Calc_resumo_namaItem("res");
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total'] as $cmp_gb => $resto)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $this->$Arr_tot_name = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total'][$cmp_gb];
      }
      ksort($this->array_total_tgl);
      ksort($this->array_total_item);
      if ("out" == $this->NM_tipo)
      {
         $Gb_geral = "quebra_geral_" . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['SC_Ind_Groupby'];
         $this->Total->$Gb_geral();
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['responsive_chart']['active']) {
$nm_saida->saida("<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"\r\n");
$nm_saida->saida("            \"http://www.w3.org/TR/1999/REC-html401-19991224/loose.dtd\">\r\n");
}
$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['dashboard_refresh_after_chart'] = 'resumo';
...
if (!$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['doc_word'])
{
   ...
   { 
      ...
      $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_fast_slow_moving_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['num_css'] . '.css', 'w');
      if (($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] == "print" && strtoupper($nmgp_cor_print) == "PB") || $nmgp_tipo_pdf == "pb")
      {
      ...
      @fclose($NM_css);
     $this->Ini->summary_css = $this->Ini->path_imag_temp . '/sc_css_fast_slow_moving_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['num_css'] . '.css';
     if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] == "print")
     {
         $nm_saida->saida("  <style type=\"text/css\">\r\n");
         $NM_css = file($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_fast_slow_moving_sum_' . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['num_css'] . '.css');
...
$nm_saida->saida("   <link rel=\"stylesheet\" type=\"text/css\" href=\"" . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) . ".css\" />\r\n");
           $nm_saida->saida("   <script type=\"text/javascript\">\r\n");
           ...
           $nm_saida->saida("     }\r\n");
           $nm_saida->saida("   </script>\r\n");
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['ajax_nav'] && !$this->Ini->sc_export_ajax && !$this->Ini->Export_html_zip && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['doc_word'])
      {
           $nm_saida->saida("   <script type=\"text/javascript\"> \r\n");
          ...
          if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['grid_pesq']))
          { 
...
if ($_SESSION['scriptcase']['proc_mobile'])
{
      ...
      if (!$this->Ini->Export_html_zip && $this->Print_All && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['doc_word']) 
      {
      ...
      {
          $remove_margin = isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['dashboard_info']['remove_margin']) && $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['dashboard_info']['remove_margin'] ? ' style="margin: 0"' : '';
          $nm_saida->saida(" <BODY class=\"" . $this->css_scGridPage . "\"" . $remove_margin . ">\r\n");
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "print" && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['embutida'])
      {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['opcao'] != "pdf" && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['embutida'] && !$_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['doc_word'])
      { 
          $nm_saida->saida("      <STYLE>\r\n");
   ...
   function monta_barra_top_normal()
   {
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['pdf'] == "on" && !$this->NM_res_sem_reg)
      {
         ...
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bpdf", "", "", "Rpdf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_config_pdf.php?nm_opc=pdf_res&nm_target=0&nm_cor=cor&papel=8&orientacao=1&bookmarks=1&largura=1200&conf_larg=S&conf_fonte=10&grafico=S&sc_ver_93=" . s . "&nm_tem_gb=s&nm_res_cons=s&nm_ini_pdf_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&summary_export_columns=S&nm_label_group=N&nm_all_cab=S&nm_all_label=S&pdf_zip=N&nm_orient_grid=2&origem=res&language=id&conf_socor=N&script_case_init=" . $this->Ini->sc_page . "&app_name=fast_slow_moving&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
         if ($NM_Gbtn)
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['word'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bword", "", "", "Rword_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_config_word.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=S&nm_cor=CO&nm_res_cons=s&nm_ini_word_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&origem=res&language=id&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      if (!$this->grid_emb_form && $this->nmgp_botoes['xls'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bexcel", "", "", "Rxls_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_config_xls.php?script_case_init=" . $this->Ini->sc_page . "&app_name=fast_slow_moving&nm_tp_xls=xlsx&nm_tot_xls=N&nm_tot_xls=N&nm_res_cons=s&nm_ini_xls_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&summary_export_columns=S&origem=res&language=&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
         if ($NM_Gbtn)
      ...
      if (!$this->grid_emb_form && $this->nmgp_botoes['csv'] == "on" && !$this->NM_res_sem_reg)
      {
          $nm_saida->saida("           <script type=\"text/javascript\">sc_itens_btgp_group_1_top = true;</script>\r\n");
          $nm_saida->saida("            <div class=\"scBtnGrpText scBtnGrpClick\">\r\n");
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bcsv", "", "", "Rcsv_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_config_csv.php?script_case_init=" . $this->Ini->sc_page . "&summary_export_columns=S&nm_res_cons=s&nm_ini_csv_res=grid,resume,chart&nm_all_modules=grid,resume,chart&password=n&nm_delim_line=1&nm_delim_col=1&nm_delim_dados=1&nm_label_csv=N&language=&origem=res&KeepThis=true&TB_iframe=true&modal=true", "group_1", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
          $nm_saida->saida("            </div>\r\n");
      }
      ...
      {
         $Cod_Btn = nmButtonOutput($this->arr_buttons, "bconf_graf", "", "", "Rgraf_top", "", "", "", "absmiddle", "", "0px", $this->Ini->path_botoes, "", "", "thickbox", "" . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_config_graf_flash.php?nome_apl=fast_slow_moving&campo_apl=nm_resumo_graf&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=id&tp_apl=cns_htm&KeepThis=true&TB_iframe=true&height=450&width=400&modal=true", "", "only_text", "text_right", "", "", "", "", "", "", "");
         $nm_saida->saida("           $Cod_Btn \r\n");
      }
      ...
      if ($this->nmgp_botoes['chart_detail'] == 'on' && !$this->grid_emb_form && !$this->NM_res_sem_reg)
      {
         ...
         if (is_file("fast_slow_moving_help.txt") && !$this->grid_emb_form && !$this->NM_res_sem_reg)
         {
   ...
   function monta_html_fim()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['print_all'])
      {
          $this->grafico_pdf();
      }
      $nm_saida->saida("</TABLE>\r\n");
      $nm_saida->saida("<link rel=\"stylesheet\" type=\"text/css\" href=\"../_lib/css/" . $this->Ini->str_schema_all . "_sweetalert.css\" />\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['fast_slow_moving']['glo_nm_path_prod'] . "/third/sweetalert/sweetalert2.all.min.js\"></script>\r\n");
      $nm_saida->saida("<script type=\"text/javascript\" src=\"" . $_SESSION['scriptcase']['fast_slow_moving']['glo_nm_path_prod'] . "/third/sweetalert/polyfill.min.js\"></script>\r\n");
...
{ 
      ...
      $nm_saida->saida("</TABLE>\r\n");
       ...
       $nm_saida->saida("  tb_show('', 'fast_slow_moving_config_pivot.php?nome_apl=fast_slow_moving&sc_page=" . NM_encode_input($this->Ini->sc_page) . "&language=id&TB_iframe=true&modal=true&height=300&width=500', '');\r\n");
       $nm_saida->saida("}\r\n");
   ...
   function monta_html_ini_pdf()
   {
      global $nm_saida;
       $tp_quebra = "";
       if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['num_css']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['num_css']))
       {
           $NM_css = @fopen($this->Ini->root . $this->Ini->path_imag_temp . '/sc_css_fast_slow_moving_grid_' . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['num_css'] . '.css', 'a');
           $NM_css_file = $this->Ini->root . $this->Ini->path_link . "fast_slow_moving/fast_slow_moving_res_" . strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']). ".css";
       ...
       if ($this->Print_All || $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['print_all'])
       {
       $tp_quebra = "<div style=\"page-break-after: always;\"><span style=\"display: none;\">&nbsp;</span></div>";
       }
...
//--- 
   ...
   //---- 
   function adiciona_registro($sum_jumlah, $sum_tot, $quebra_tgl, $quebra_item, $quebra_tgl_orig, $quebra_item_orig)
   {
      //----- " . $this->Ini->Nm_lang['lang_othr_cons_title_YYYYMM'] . "
      if (!isset($this->array_total_tgl[$quebra_tgl_orig]))
      {
         $this->array_total_tgl[$quebra_tgl_orig][0] = 1;
         $this->array_total_tgl[$quebra_tgl_orig][1] = $sum_jumlah;
         $this->array_total_tgl[$quebra_tgl_orig][2] = $sum_tot;
         $this->array_total_tgl[$quebra_tgl_orig][3] = $quebra_tgl;
         $this->array_total_tgl[$quebra_tgl_orig][4] = $quebra_tgl_orig;
      }
      else
      {
         $this->array_total_tgl[$quebra_tgl_orig][0]++;
         $this->array_total_tgl[$quebra_tgl_orig][1] += $sum_jumlah;
         $this->array_total_tgl[$quebra_tgl_orig][2] += $sum_tot;
      }
      //----- Item
      if (!isset($this->array_total_item[$quebra_tgl_orig][$quebra_item_orig]))
      {
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][0] = 1;
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][1] = $sum_jumlah;
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][2] = $sum_tot;
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][3] = $quebra_item;
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][4] = $quebra_item_orig;
      }
      else
      {
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][0]++;
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][1] += $sum_jumlah;
         $this->array_total_item[$quebra_tgl_orig][$quebra_item_orig][2] += $sum_tot;
      }
   }
...
//
   function nm_acumula_resumo($nm_tipo="resumo")
   {
     ...
     $campos_order = "";
     $format       = $this->Ini->Get_Gb_date_format('', 'tgl');
     $campos_order = $this->Ini->Get_date_order_groupby("Tgl", 'asc', $format, $campos_order);
     $format       = $this->Ini->Get_Gb_date_format('', 'item');
     $campos_order = $this->Ini->Get_date_order_groupby("item", 'asc', $format, $campos_order);
...
// 
   function nm_acum_res_unit($rs_res, $nm_tipo="resumo")
   {
            ...
            $this->tgl = $rs_res->fields[1] ;  
            $this->jumlah = $rs_res->fields[2] ;  
            $this->tot = $rs_res->fields[3] ;  
            ...
            $this->tgl_orig = $this->Ini->Get_arg_groupby($TP_Time . $this->tgl, $Format_tst);
            $this->tgl = $this->Ini->GB_date_format($TP_Time . $this->tgl, $Format_tst, $Prefix_dat, "N", "F/Y");
            $this->item_orig = $this->item;
            $this->item = $this->look_item ;
            if ($nm_tipo == "resumo")
            {
                $this->adiciona_registro($this->jumlah, $this->tot, $this->tgl, $this->item, $this->tgl_orig,  $this->item_orig);
            }
   }
//
