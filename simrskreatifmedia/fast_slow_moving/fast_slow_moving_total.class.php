...
class fast_slow_moving_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['fast_slow_moving']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['fast_slow_moving']['campos_busca']))
      { 
   ...
   //---- 
   function quebra_geral_namaItem($res_limit=false)
   {
      global $nada, $nm_lang , $item;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(jumlah), sum(tot) from (SELECT 	x.item, 	x.tot, 	x.jumlah, 	x.Tgl  FROM 	( 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ) , 0 ) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ), 0 ) AS tot, 		jumlah, 		 tranDate AS 'Tgl'  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		round( 				( jumlah * ( ( biaya / 128 ) * 138 ) ) + ( ( ( biaya / 128 ) * 138 ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatinap UNION ALL 	SELECT 		a.itemcode AS item, 		round( 			 				( a.rate * a.jumlah ) + ( ( a.rate * a.jumlah ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		a.jumlah AS jumlah, 		b.trandate AS 'Tgl'  	FROM 		tbitemdrugsell a 		LEFT JOIN tbdrugsell b ON b.trancode = a.trancode  	) x) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['where_pesq']; 
      } 
      else 
      { 
          $nm_comando = "select count(*), sum(jumlah), sum(tot) from (SELECT 	x.item, 	x.tot, 	x.jumlah, 	x.Tgl  FROM 	( 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ) , 0 ) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ), 0 ) AS tot, 		jumlah, 		 tranDate AS 'Tgl'  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		round( 				( jumlah * ( ( biaya / 128 ) * 138 ) ) + ( ( ( biaya / 128 ) * 138 ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatinap UNION ALL 	SELECT 		a.itemcode AS item, 		round( 			 				( a.rate * a.jumlah ) + ( ( a.rate * a.jumlah ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		a.jumlah AS jumlah, 		b.trandate AS 'Tgl'  	FROM 		tbitemdrugsell a 		LEFT JOIN tbdrugsell b ON b.trancode = a.trancode  	) x) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][2] = $rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $rt->fields[2] = (string)$rt->fields[2]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['tot_geral'][3] = $rt->fields[2]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['contr_total_geral'] = "OK";
   } 

   //-----  tgl
   function quebra_tgl_namaItem($tgl, $arg_sum_tgl) 
   {
      global $tot_tgl, $item;
      $tot_tgl = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      ...
      { 
         $nm_comando .= " where " .  $this->Sc_groupby_tgl . $arg_sum_tgl ; 
      } 
      else 
      { 
         $nm_comando .= " and " .  $this->Sc_groupby_tgl . $arg_sum_tgl ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_tgl[0] = NM_encode_input(sc_strip_script($tgl)) ; 
      $tot_tgl[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_tgl[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_tgl[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 

   //-----  item
   function quebra_item_namaItem($tgl, $item, $arg_sum_tgl, $arg_sum_item) 
   {
      global $tot_item, $item;
      $tot_item = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(jumlah), sum(tot) from (SELECT 	x.item, 	x.tot, 	x.jumlah, 	x.Tgl  FROM 	( 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ) , 0 ) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ), 0 ) AS tot, 		jumlah, 		 tranDate AS 'Tgl'  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		round( 				( jumlah * ( ( biaya / 128 ) * 138 ) ) + ( ( ( biaya / 128 ) * 138 ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatinap UNION ALL 	SELECT 		a.itemcode AS item, 		round( 			 				( a.rate * a.jumlah ) + ( ( a.rate * a.jumlah ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		a.jumlah AS jumlah, 		b.trandate AS 'Tgl'  	FROM 		tbitemdrugsell a 		LEFT JOIN tbdrugsell b ON b.trancode = a.trancode  	) x) nm_sel_esp " . $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['where_pesq']; 
      ...
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['where_pesq'])) 
      { 
         $nm_comando .= " where " .  $this->Sc_groupby_tgl . $arg_sum_tgl . " and item" . $arg_sum_item ; 
      } 
      else 
      { 
         $nm_comando .= " and " .  $this->Sc_groupby_tgl . $arg_sum_tgl . " and item" . $arg_sum_item ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_item[0] = sc_strip_script($item) ; 
      $tot_item[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_item[2] = (string)$rt->fields[1]; 
      $rt->fields[2] = str_replace(",", ".", $rt->fields[2]);
      $tot_item[3] = (string)$rt->fields[2]; 
      $rt->Close(); 
   } 


   //----- 
   function Calc_resumo_namaItem($destino_resumo)
   {
      global $nm_lang, $item;
      $this->nm_data = new nm_data("id");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('tgl' => "Tgl",'item' => "item");
      $cmps_quebra_atual = array("tgl","item");
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_namaItem($ind_qb, $cmp_gb, $arr_tots, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_namaItem($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('tot' => 'N','jumlah' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), sum(jumlah), sum(tot)#@#cmps_quebras#@# from (SELECT 	x.item, 	x.tot, 	x.jumlah, 	x.Tgl  FROM 	( 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ) , 0 ) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ), 0 ) AS tot, 		jumlah, 		 tranDate AS 'Tgl'  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		round( 				( jumlah * ( ( biaya / 128 ) * 138 ) ) + ( ( ( biaya / 128 ) * 138 ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatinap UNION ALL 	SELECT 		a.itemcode AS item, 		round( 			 				( a.rate * a.jumlah ) + ( ( a.rate * a.jumlah ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		a.jumlah AS jumlah, 		b.trandate AS 'Tgl'  	FROM 		tbitemdrugsell a 		LEFT JOIN tbdrugsell b ON b.trancode = a.trancode  	) x) sc_sel_esp " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1), sum(SC_metric2)#@#cmps_quebras#@# from (";
         $comando .= "select jumlah as SC_metric1,tot as SC_metric2, " . $cmps_quebras1 . " from (SELECT 	x.item, 	x.tot, 	x.jumlah, 	x.Tgl  FROM 	( 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ) , 0 ) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ), 0 ) AS tot, 		jumlah, 		 tranDate AS 'Tgl'  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		round( 				( jumlah * ( ( biaya / 128 ) * 138 ) ) + ( ( ( biaya / 128 ) * 138 ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatinap UNION ALL 	SELECT 		a.itemcode AS item, 		round( 			 				( a.rate * a.jumlah ) + ( ( a.rate * a.jumlah ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		a.jumlah AS jumlah, 		b.trandate AS 'Tgl'  	FROM 		tbitemdrugsell a 		LEFT JOIN tbdrugsell b ON b.trancode = a.trancode  	) x) sc_sel_esp1 " . $where_ok . ") SC_sel1 INNER JOIN (";
         $comando .= "select " . $cmps_quebras2 . " from (SELECT 	x.item, 	x.tot, 	x.jumlah, 	x.Tgl  FROM 	( 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ) , 0 ) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatjalan UNION ALL 	SELECT 		item, 		round(jumlah * biaya + ( biaya * ( 10 / 100 ) ), 0 ) AS tot, 		jumlah, 		 tranDate AS 'Tgl'  	FROM 		tbreseprawatigd UNION ALL 	SELECT 		item, 		round( 				( jumlah * ( ( biaya / 128 ) * 138 ) ) + ( ( ( biaya / 128 ) * 138 ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		jumlah, 		tranDate AS 'Tgl'  	FROM 		tbreseprawatinap UNION ALL 	SELECT 		a.itemcode AS item, 		round( 			 				( a.rate * a.jumlah ) + ( ( a.rate * a.jumlah ) * ( 10 / 100 ) ) , 			0  		) AS tot, 		a.jumlah AS jumlah, 		b.trandate AS 'Tgl'  	FROM 		tbitemdrugsell a 		LEFT JOIN tbdrugsell b ON b.trancode = a.trancode  	) x) sc_sel_esp2 " . $where_ok . " group by " . $group . $sc_having . ") SC_sel2 ";
         $comando .= " ON " . $join;
      } 
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['fast_slow_moving']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['tgl']['reg'] = "N";
      $format_dimensions['tgl']['msk'] = "F/Y";
      $format_dimensions['item']['reg'] = "S";
      $format_dimensions['item']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'namaItem', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "item") {
                  $this->Lookup->lookup_namaItem_item($$Cada_dim,  $item);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_namaItem($where_ok);
