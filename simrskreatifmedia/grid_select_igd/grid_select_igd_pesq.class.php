...
class grid_select_igd_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_select_igd_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_select_igd_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_select_igd_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
   ...
   function lookup_ajax_custcode($custcode)
   {
      ...
      $nm_comando = "select distinct custCode from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and  custCode like '%" . $custcode . "%' order by custCode"; 
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_doctor($doctor)
   {
      ...
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct doctor from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and  doctor like '%" . $doctor . "%' order by doctor"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   function lookup_ajax_queue($queue)
   {
      ...
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and   CAST (queue AS TEXT)  like '%" . $queue . "%' order by queue"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and   CAST (queue AS VARCHAR)  like '%" . $queue . "%' order by queue"; 
      }
      else
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and  queue like '%" . $queue . "%' order by queue"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
   ...
   function lookup_ajax_struckno($struckno)
   {
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and   CAST (struckNo AS TEXT)  like '%" . $struckno . "%' order by struckNo"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and   CAST (struckNo AS VARCHAR)  like '%" . $struckno . "%' order by struckNo"; 
      }
      else
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and  struckNo like '%" . $struckno . "%' order by struckNo"; 
      }
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . $op_all . $nm_ini_lower . "'%" . $campo . "%'" . $nm_fim_lower;
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - tbantrian</TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_select_igd/grid_select_igd_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
...
<SCRIPT type="text/javascript" src="<?php echo $this->Ini->path_js . "/browserSniffer.js" ?>"></SCRIPT>
 ...
 <script type="text/javascript" src="grid_select_igd_ajax_search.js"></script>
 <script type="text/javascript" src="grid_select_igd_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_select_igd_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_select_igd']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_select_igd']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
<?php
   ...
   {
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_othr_srch_titl'] ?> - tbantrian</div>
...
<?php
   ...
   function monta_form()
   {
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $custcode = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['custcode']; 
          $custcode_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['custcode_cond']; 
          $doctor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['doctor']; 
          $doctor_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['doctor_cond']; 
          $queue = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['queue']; 
          $queue_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['queue_cond']; 
          $struckno = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['struckno']; 
          $struckno_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['struckno_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['NM_operador']; 
      } 
...
<?php
      ...
      $custcode_look = substr($this->Db->qstr($custcode), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct custCode from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and custCode = '$custcode_look' order by custCode"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
...
<?php
      ...
      $doctor_look = substr($this->Db->qstr($doctor), 1, -1); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct doctor from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and doctor = '$doctor_look' order by doctor"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
...
<input class="sc-js-input scFilterObjectEven" type="text" id="id_ac_doctor" name="doctor_autocomp" size="8"  value="<?php echo NM_encode_input($sAutocompValue); ?>" alt="{datatype: 'text', maxLength: 8, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}">
      ...
      <TD id='SC_queue_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['queue'])) ? $this->New_label['queue'] : "Antrian"; ?></TD>
     <TD class="scFilterFieldOdd"> 
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and queue = $queue_look order by queue"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct queue from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and queue = $queue_look order by queue"; 
...
<?php
   ...
   { 
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and struckNo = $struckno_look order by struckNo"; 
      }
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct struckNo from " . $this->Ini->nm_tabela . " where (regDate = CURDATE( ) OR regDate = CURDATE( ) - INTERVAL 1 DAY) and poly = 'POL004' and (status = 'Antre' OR status = 'Mutasi RI') and struckNo = $struckno_look order by struckNo"; 
...
?>
<?php
   if (is_file("grid_select_igd_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_select_igd']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_select_igd']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
   ...
   <INPUT type="hidden" name="script_case_session" value="<?php echo NM_encode_input(session_id()); ?>"> 
<?php
   if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['orig_pesq'] == "grid")
   {
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "simrskreatifmedia/grid_select_igd";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_orig']))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_orig'] = "";
      }
      $this->comando        = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_orig'];
      $this->comando_sum    = "";
      $this->comando_filtro = "";
   ...
   function trata_campos()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['custcode'] = $custcode; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['custcode_cond'] = $custcode_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['doctor'] = $doctor; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['doctor_cond'] = $doctor_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['queue'] = $queue; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['queue_cond'] = $queue_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['struckno'] = $struckno; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['struckno_cond'] = $struckno_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca'] = $Temp_Busca;
      }
      if ($queue_cond != "in" && $queue_cond != "bw" && !empty($queue) && !is_numeric($queue))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : Antrian";
      }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_pesq_filtro'] = " (" . $this->comando_filtro . ")";
      }
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_pesq'] != $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['where_pesq_ant'])
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_select_igd']['cond_pesq'] .= $this->NM_operador;
