...
class lap_grid_logistik_single_xml
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function inicializa_vars()
   {
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['embutida'] && isset($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_label']))
      {
          $this->Xml_tag_label = $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_label'];
          $this->New_Format    = true;
      }
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_return']);
          if ($this->Tem_xml_res) {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "lap_grid_logistik_single.xml";
   ...
   function grava_arquivo()
   {
      ...
      $nm_raiz_img  = ""; 
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name'];
          $this->Arq_zip = $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name'];
          $Pos = strrpos($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name'], ".");
          if ($Pos !== false) {
              $this->Arq_zip = substr($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name'], 0, $Pos);
          }
          $this->Arq_zip .= ".zip";
          $this->Tit_zip  = $this->Arq_zip;
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         ...
         if ($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['embutida'])
         { 
             $this->xml_registro .= "<" . $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['embutida_tit'] . ">\r\n";
         }
         elseif ($this->New_Format)
         {
             $this->xml_registro = "<lap_grid_logistik_single>\r\n";
         }
         else
         {
             $this->xml_registro = "<lap_grid_logistik_single";
...
$_SESSION['scriptcase']['lap_grid_logistik_single']['contr_erro'] = 'off'; 
      ...
      { 
          ...
          { 
              ...
              $this->Res = new lap_grid_logistik_single_res_xml();
              $this->prep_modulos("Res");
              $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_res_grid'] = true;
              $this->Res->monta_xml();
          } 
          if (!$this->Ini->sc_export_ajax) {
          ...
          { 
              ...
              { 
                  $str_zip   = "";
                  $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_res_file']['xml'];
                  ...
                  unlink($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_res_file']['xml']);
              }
              if ($this->Grava_view)
              {
                  $str_zip    = "";
                  ...
                  { 
                      $str_zip   = "";
                      $Arq_res   = $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_res_file']['view'];
                      $Arq_input = (FALSE !== strpos($Arq_res, ' ')) ? " \"" . $Arq_res . "\"" :  $Arq_res;
                      if (FALSE !== strpos(strtolower(php_uname()), 'windows')) 
                      {
                          $str_zip = "zip.exe " . strtoupper($Parm_pass) . " -j -u " . $this->Xml_password . " " . $Zip_f . " " . $Arq_input;
                      ...
                      unlink($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_res_file']['view']);
                  }
              } 
              else 
              {
              ...
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_res_grid']);
          } 
      }
   ...
   function NM_export_jml()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->jml) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->jml) . "\"";
         }
   }
   //----- kemasan
   ...
   function NM_export_hrg()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->hrg) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->hrg) . "\"";
         }
   }
   //----- sc_field_2
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->b_status))
         {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->b_status) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->b_status) . "\"";
         }
   }
   //----- a_id
   function NM_export_a_id()
   {
         ...
         if ($this->New_Format)
         {
             $this->xml_registro .= " <" . $SC_Label . ">" . $this->trata_dados($this->a_id) . "</" . $SC_Label . ">\r\n";
         }
         else
         {
             $this->xml_registro .= " " . $SC_Label . " =\"" . $this->trata_dados($this->a_id) . "\"";
         }
   }
   ...
   {
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<INPUT type="hidden" name="nmgp_opcao" value="<?php echo NM_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['lap_grid_logistik_single']['xml_return']); ?>"> 
</FORM> 
</BODY>
</HTML>
<?php
